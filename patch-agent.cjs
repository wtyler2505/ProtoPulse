...
const content = `import type { Express, Request, Response, NextFunction } from 'express';
import { z } from 'zod';
  ...
  app.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (req.session?.userId) {
        const storedKey = await getApiKey(req.session.userId, 'gemini');
...
fs.writeFileSync('server/routes/agent.ts', content, 'utf8');
console.log("Patched server/routes/agent.ts");
