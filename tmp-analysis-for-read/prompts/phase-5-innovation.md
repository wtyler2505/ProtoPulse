...
Read the **EDA** section in `~/.claude/skills/product-analysis/references/domain-detection.md` for:
...
Adapt the URLs based on what WebSearch reveals and what's relevant to EDA.
...
Run the domain-specific tool commands from the `domain-detection.md` reference for EDA.
...
1. **Domain trends:** Search "EDA trends {year}" and "EDA emerging features {year}"
2. **AI/ML opportunities:** Search "AI features in EDA tools {year}" and "machine learning EDA applications"
...
4. **User needs research:** Search "EDA user pain points" and "what {persona} wish existed"
5. **Technology enablers:** Search "new {framework} capabilities {year}" and "{library} latest features"
6. **Industry reports:** Search "EDA market report {year}" and "EDA industry analysis"
7. **Viral features:** Search "EDA product hunt" and "EDA tools that went viral"
...
- What would make rest-express the OBVIOUS choice in its category?
...
- What would make rest-express go viral or get shared organically?
