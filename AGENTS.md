...
- **NEVER SIT IDLE WHILE BACKGROUND WORK RUNS.** When tests/builds/agents/teammates are running, you MUST find independent parallel work to progress. Options: (1) docs or knowledge notes about what you just built, (2) next-phase plan drafting or commit message prep, (3) research via Context7 or WebSearch for upcoming work, (4) gap-analysis scripts in `ops/queries/`, (5) Improvements Radar on nearby files, (6) verify assumptions the next phase depends on, (7) test stubs for the next phase. "Blocked on <specific thing>" in one sentence is acceptable — silent idle is NOT. Enforced at runtime by `.claude/hooks/never-idle-while-waiting.sh` PostToolUse hook on `Bash|Agent` when `run_in_background: true`. Rule reinforced 2026-04-11 after repeated violations.
