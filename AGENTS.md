...
- **NEVER SIT IDLE WHILE BACKGROUND WORK RUNS.** When tests/builds/agents/teammates are running, find independent parallel work — but ONLY from the SAFE list. Enforced at runtime by `.claude/hooks/never-idle-while-waiting.sh` PostToolUse hook on `Bash|Agent` when `run_in_background: true`. Full rule in `memory/feedback_never_idle_while_waiting.md`. Summary:
