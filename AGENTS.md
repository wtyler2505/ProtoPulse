...
- **NEVER SIT IDLE WHILE BACKGROUND WORK RUNS.** When tests/builds/agents/teammates are running, find independent parallel work sized to the DURATION of the wait. Enforced at runtime by `.claude/hooks/never-idle-while-waiting.sh` PostToolUse hook on `Bash|Agent` when `run_in_background: true`. The hook classifies every background task into SHORT (< 1 min: single file read, draft 1-2 sentences), MEDIUM (1-5 min: one knowledge note, next-phase prep, one gap-analysis script), or LONG (5 min+: full knowledge notes, next-phase plan draft, deep research arc, Improvements Radar). Full rule in `memory/feedback_never_idle_while_waiting.md`. Summary:
  - **SAFE during wait:** read any file (Read/Grep/Glob), research (Context7/WebSearch/WebFetch), write NEW files in `docs/` (not active plan), `knowledge/`, `ops/observations/`, `ops/queue/`, `ops/methodology/`, `reports/`; TaskCreate/Update/List; run read-only gap-analysis scripts in `ops/queries/`; draft the NEXT phase in a NEW scratch file.
  - **UNSAFE — NEVER during wait:** edit/write any file in `client/`, `server/`, `shared/`, `.claude/`, or any build config (`schema.ts`, `package.json`, `tsconfig.json`, `drizzle.config.ts`, `vite.config.ts`, `.env*`, `.mcp.json`); edit the active plan file being executed; `npm install/update/ci`, `npm run build`, `db:push`, migrations; `git commit/push/reset/rebase/merge/stash pop/cherry-pick/checkout/clean`; spawn new background tasks or agent-teams; `kill`/`pkill`; `prettier --write` / `eslint --fix`; run concurrent test suites; modify workspace state (`data/metrics.json`, `ops/sessions/*`, `dist/`, `coverage/`).
  - **Rule of thumb:** READ don't WRITE. "Blocked on <specific thing>" in one sentence is acceptable — silent idle is NOT. Rule reinforced 2026-04-11 after repeated violations.
...
- **All tests MUST run backgrounded.** Every `npm test`, `npm run test*`, `vitest run`, and `npx vitest` call MUST set `run_in_background: true`. Enforced at runtime by `.claude/hooks/enforce-test-background.sh` (PreToolUse, Bash matcher) — foreground test commands are hard-blocked with a rejection message. Rationale: (1) tests take minutes and foreground runs block the session and prevent the never-idle tier classifier from firing, (2) multi-agent sessions need persistent test logs via `tee logs/tests-*-latest.log`, (3) Tyler's rule 2026-04-11. Pipe output to a log: `npx vitest run --project server 2>&1 | tee logs/tests-server-latest.log | tail -10` with `run_in_background: true`.
