...
**ProtoPulse implication:** The bench coach should enforce a DRC rule: any schematic with a motor controller and an MCU must have an explicit ground connection between them. The rule is not "they share a ground rail" but "there is a dedicated signal ground wire." This applies to all motor controllers (L298N, TB6612, ESC), not just the ZS-X11H, but the high-current BLDC case makes it most critical because the ground offset is proportional to motor current.
