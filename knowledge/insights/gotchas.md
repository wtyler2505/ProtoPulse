...
- [[architecture-context-has-two-parallel-undo-systems-that-do-not-interact]] — global UndoRedoStack vs architecture-local useState snapshots; Ctrl+Z only triggers global, architecture undo only accessible via explicit UI buttons
- [[design-agent-hardcodes-confirmed-true-bypassing-destructive-tool-confirmation-enforcement]] — the agent loop bypasses tool confirmation; AI can delete nodes/clear BOM without any undo gate
- [[migration-0002-must-drop-and-recreate-check-constraints-because-drizzle-kit-cannot-model-them-creating-a-manual-maintenance-trap]] — every drizzle-kit migration drops CHECK constraints, requiring manual re-addition
- [[drc-engine-exports-two-completely-separate-rule-systems-from-one-file-creating-a-hidden-api-surface-split]] — shared/drc-engine.ts is two engines with different input types, naming conventions, and test files
- [[batch-analysis-tracking-lives-in-an-in-memory-map-that-does-not-survive-server-restarts]] — batch tracking lost on server restart; orphaned Anthropic batches can't be correlated to projects
