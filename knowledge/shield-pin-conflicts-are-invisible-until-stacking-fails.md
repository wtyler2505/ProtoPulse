...
The shield inventory reveals significant pin overlap that has no built-in detection mechanism. The HW-130 motor shield uses D3, D4, D5, D6, D8, D11, D12. The Ethernet W5100 uses SPI (ICSP) + D10 + D4. The TFT LCD shield uses SPI (ICSP) + D8 (reset) + D9 (DC) + D10 (CS) + A0-A3 (resistive touch XP, XM, YP, YM). The OSEPP Motor/Servo Shield V1 uses IN1-IN4 for motor direction and ENA/ENB for PWM speed — and crucially, the OSEPP datasheet explicitly warns "motor direction and PWM pins vary by shield revision," meaning the same-named shield from different production runs may occupy different Arduino pins. The OSEPP TB6612 motor shield uses AIN1, AIN2 (motor A direction), BIN1, BIN2 (motor B direction), PWMA, PWMB (speed control on PWM pins), and STBY (enable); this is 7 pins for two-channel motor control, fewer than the HW-130's 7 pins-plus-shift-register approach but still overlapping with any shield that uses the same PWM-capable pins. Attempting to stack the motor shield with either the Ethernet or TFT shield creates hard conflicts on D4 (motor + Ethernet) and D8 (motor + TFT) that manifest as silent malfunction, not an error message.
