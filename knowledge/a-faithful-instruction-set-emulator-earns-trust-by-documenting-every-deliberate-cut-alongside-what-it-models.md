...
This is why each cut is stated as a positive contract rather than an apology. The Xtensa core models 24-bit and 16-bit instructions but refuses windowed handler-only ops (L32E/S32E/RFWO/RFWU) and MOVSP outside its magic spill/fill path — refusal is safer than approximation because a wrong answer is worse than a halt. Memory is [[the-esp32-s3-maps-one-sram-block-at-both-an-iram-and-a-dram-address-so-an-emulator-can-model-it-as-a-single-window-aliased-to-two-bus-addresses|one 480 KB SRAM window aliased across the IRAM and DRAM buses]], with no cache, no SRAM0, single core, and 1 instruction = 1 cycle at 240 MHz; so any consumer measuring cycle-accurate timing or cache effects is outside the boundary and must not trust those numbers. The image loader accepts only SRAM-resident segments and refuses flash-mapped (0x42xxxxxx / 0x3Cxxxxxx) ones because the flash cache is unmodeled (a build that instead wants those windows can [[flash-mapped-irom-and-drom-segments-carry-post-mapping-virtual-addresses-so-an-emulator-serves-them-read-only-straight-from-the-image|serve them read-only straight from the image as the net effect of a warmed cache]], a disclosed-boundary cut of the same shape — 1-cycle XIP, no MMU model), and it [[the-esp-idf-app-image-checksum-is-the-xor-of-all-segment-data-bytes-seeded-with-0xef-stored-as-the-last-byte-of-the-16-aligned-image-body|verifies the XOR checksum but skips the SHA-256 trailer]] — each skip named so a user knows exactly what was and was not checked.
...
- [[the-esp32-s3-maps-one-sram-block-at-both-an-iram-and-a-dram-address-so-an-emulator-can-model-it-as-a-single-window-aliased-to-two-bus-addresses]] — the one-480 KB-window-aliased-across-IRAM/DRAM cut named in this note's body; modeling both buses with a single array is the disclosed boundary (no SRAM0, no cache)
...
- [[condition-derived-level-interrupts-cannot-be-cleared-by-intclear-until-the-underlying-condition-clears]] — the mirror-image of the timer-latch case: GPIO/UART level sources derive their status from live state, so modeling INTCLEAR as a no-op until the condition resolves is the faithful behavior — conflating it with a clearable sticky bit would be the same kind of undocumented divergence
...
- [[flash-mapped-irom-and-drom-segments-carry-post-mapping-virtual-addresses-so-an-emulator-serves-them-read-only-straight-from-the-image]] — the complement to the loader's refusal: serving IROM/DROM read-only from the image is the warmed-cache net effect, with 1-cycle XIP and an unmodeled MMU as its paired cuts
