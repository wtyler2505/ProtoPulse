...
The RioRand KJL-01 BLDC controller uses opposite logic polarities for its two motor-halting functions: STOP is active-low (connect to GND to stop, leave floating or HIGH to run) and BRAKE is active-high (connect to 5V to engage dynamic braking, leave floating for no brake). While this inverted-pair convention might seem standard, **polarity is actually vendor-specific, not a universal BLDC convention**. For example, the ZS-X11H controller uses an active-LOW CT (brake) pin, directly contradicting the KJL-01. Assuming a universal standard is a persistent source of wiring mistakes because the two signals look functionally similar ("both stop the motor") yet require opposite electrical treatment.

For the ZS-X11H specifically, the state table for the CT (Brake) pin through a level shifter is:
...
**Braking Operating Discipline:**
Always reduce speed before engaging brake. High-speed braking generates large back-EMF. Release brake within 2-3 seconds of motor stopping.
