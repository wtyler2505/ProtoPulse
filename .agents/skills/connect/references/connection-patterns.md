...
**If a task file exists** (pipeline execution): read the task file to see what the extraction phase discovered. The extract notes, semantic neighbor field, and classification provide critical context about why this {vocabulary.note} was extracted and what it relates to.
...
**Revisit Task Filtering:** when adding bidirectional links to older knowledge, apply the skip-filter table in SKILL.md (hub >5 incoming links, tension type, just-revisited, topic map) before flagging for revisit — that table is a queue contract and lives in SKILL.md.
...
Remove gaps that are now filled. Add new gaps discovered during connection.
