---
name: connect
description: Find connections between knowledge and update MOCs. Requires semantic judgment to identify genuine relationships. Use after /extract creates knowledge, when exploring connections, or when a topic needs synthesis. Triggers on "/connect", "/connect [note]", "find connections", "update MOCs", "connect these knowledge".
user-invocable: true
...
8. If task file in context: fork
...
**If a task file exists** (pipeline execution): read the task file to see what the extraction phase discovered. The extract knowledge, semantic neighbor field, and classification provide critical context about why this {vocabulary.note} was extracted and what it relates to.
...
- What do agent knowledge reveal about navigation?
...
2. Do NOT create the synthesis {vocabulary.note} during connect — flag it for future work
...
When you edit an older {vocabulary.note} to add a reverse link, you MAY flag it for full reconsideration via revisit. But SKIP revisit flagging if ANY of these apply:
...
| Note was reweaved in current batch | Do not re-revisit what was just reweaved |
| Note is a {vocabulary.topic_map} | {vocabulary.topic_map_plural} are navigation, not claims to reconsider |

**Check incoming links:**
```bash
grep -r '\[\[note name\]\]' {vocabulary.knowledge}/*.md | wc -l
```
...
```bash
grep -c '^\- \[\[' "{vocabulary.knowledge}/[moc-name].md"
```
...
- [[composability]] demands small knowledge, but [[context limits]] means traversal has overhead. [[new note]] suggests the tradeoff depends on expected traversal depth.
```
...
Agent knowledge are breadcrumbs for future navigation.

**Add agent knowledge when:**
...
ls {vocabulary.knowledge}/"target name.md" 2>/dev/null
```
...
- Agent knowledge reveal non-obvious paths
...
- Add agent knowledge when navigation insights emerge
...
```
=== RALPH HANDOFF: {vocabulary.connect} ===
Target: [[note name]]
...
Files Modified:
- {vocabulary.knowledge}/[note name].md (inline links added)
- {vocabulary.knowledge}/[moc-name].md (Core Ideas updated)
- [task file path] ({vocabulary.connect} section)
...
Queue Updates:
- Advance phase: {vocabulary.connect} -> {vocabulary.revisit}
...
When running in handoff mode via /ralph, the prompt includes the task file path. After completing the workflow, update the `## {vocabulary.connect}` section of that task file with:
...
jq '(.tasks[] | select(.id=="TASK_ID")).current_phase = "{vocabulary.revisit}" |
    (.tasks[] | select(.id=="TASK_ID")).completed_phases += ["{vocabulary.connect}"]' \
...
- **suggested:** Output next step AND advance task queue entry to `current_phase: "{vocabulary.revisit}"`
