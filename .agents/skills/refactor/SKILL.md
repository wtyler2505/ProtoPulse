...
Artifact: [file path]
  ...
  Content impact: [does this affect existing knowledge? how many?]
```

### Content Impact Assessment

Some changes affect only infrastructure (skills, templates, context file). Others affect existing content (knowledge, MOCs):

| Change Type | Content Impact | Action Required |
|-------------|---------------|----------------|
| Template field addition | New knowledge get field, old knowledge don't | Add field to old knowledge (schema migration) |
| Template field removal | Old knowledge have unused field | Remove field from old knowledge (optional) |
| Folder restructure | Notes must move | Move files, update all wiki links |
| topic map tier change | topic map hierarchy restructured | Merge/split MOCs, update Topics footers |
| Enum value change | Old knowledge have invalid values | Update values in affected knowledge |

For content-impacting changes:
- Count affected knowledge: `grep -rl '[old value]' {vocabulary.knowledge}/*.md | wc -l`
...
|-----------------|---------------------|
| Granularity | /extract, /verify, /validate |
| Processing | /extract, /connect, /revisit, /verify, /ralph, /pipeline |
| Linking | /connect, /revisit |
| Navigation | /connect (topic map update logic) |
...
4. **topic map restructuring:** Merge/split MOCs as needed
   ...
   - For merges: combine Core Ideas, remove redundant topic map, update knowledge' Topics footers
...
| Schema compliance | Check all knowledge against template _schema blocks | All required fields present, enum values valid |
| topic map hierarchy | Verify hub links to domain MOCs, domains link to topics | All tiers connected |
| Session orient | Simulate session start — can the context file be loaded? | No file-not-found errors |
| Three-space boundaries | Check knowledge vs ops vs self boundaries | No content in wrong space |
...
### Large Content Migration (100+ knowledge)

For large migrations:
1. Show the count and estimated time
2. Offer batch processing: "This affects 150 knowledge. Process all at once or in batches of 50?"
