---
name: pipeline
description: End-to-end source processing -- seed, extract, process all claims through connect/revisit/verify, archive. The full pipeline in one command. Triggers on "/pipeline", "/pipeline [file]", "process this end to end", "full pipeline".
version: "1.0"
...
Invoke /seed on the target file — it validates the source, detects duplicates, creates the archive folder, moves the source from {DOMAIN:inbox}/, creates the extract task, and updates the queue.
...
Check how many pending tasks exist for this batch. The extract phase creates 1 queue entry per claim and 1 per enrichment.
...
$ Processing {DOMAIN:note} 1/{total}: {title}
  ...
  $ revisit... done (2 {DOMAIN:note_plural} updated)
  $ verify... done (PASS)
```
...
Processing:
  ...
  Older {DOMAIN:note_plural} updated via revisit: {R}
...
Files Modified:
- {DOMAIN:knowledge}/ ({N} new {DOMAIN:note_plural})
...
| After seed, before extract | /ralph 1 --batch {id} --type extract |
| After extract, during claims | /ralph --batch {id} (picks up from failed phase) |
