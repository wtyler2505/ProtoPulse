---
name: pp-view-arduino
description: ProtoPulse page intelligence for Arduino. Use when working on Arduino view behavior, layout, workflow, tests, or page-specific UX.
---

# ProtoPulse Arduino

Use this for page-level Arduino work: compile/upload, board selection, serial workflow, device safety.

## When To Use

- Compile/Upload work in the Arduino area.
- Board Selection work in the Arduino area.
- Serial Workflow work in the Arduino area.
- Device Safety work in the Arduino area.
- Bugs where Arduino routing, state, layout, or tests are unclear.
- Any task where a future agent would otherwise need to rediscover Arduino file locations.

## Fast Workflow

1. Run `node .agents/skills/pp-view-arduino/scripts/inspect-arduino.mjs`.
...
Warnings count as defects. Do not call Arduino work clean while known test, runtime, or console warnings remain.
