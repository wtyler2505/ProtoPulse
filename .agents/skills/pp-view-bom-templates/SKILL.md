---
name: pp-view-bom-templates
description: ProtoPulse page intelligence for BOM Templates. Use when working on BOM Templates view behavior, layout, workflow, tests, or page-specific UX.
---

# ProtoPulse BOM Templates

Use this for page-level BOM Templates work: BOM templates, reuse, template editing, apply flow.

## When To Use

- BOM Templates work in the BOM Templates area.
- Reuse work in the BOM Templates area.
- Template Editing work in the BOM Templates area.
- Apply Flow work in the BOM Templates area.
- Bugs where BOM Templates routing, state, layout, or tests are unclear.
- Any task where a future agent would otherwise need to rediscover BOM Templates file locations.

## Fast Workflow

1. Run `node .agents/skills/pp-view-bom-templates/scripts/inspect-bom-templates.mjs`.
...
Warnings count as defects. Do not call BOM Templates work clean while known test, runtime, or console warnings remain.
