...
```bash
grep -rl "{key terms from source title}" {DOMAIN:knowledge}/ 2>/dev/null | head -5
```
...
- Duplicate check against {DOMAIN:knowledge}/ during extraction
...
```
GEMINI.md -> stays as GEMINI.md (no move)
