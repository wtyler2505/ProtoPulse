...
Read `ops/derivation-manifest.md` (or fall back to `ops/derivation.md`) for domain vocabulary mapping. All output must use domain-native terms. If neither file exists, use universal terms (knowledge, inbox, topic map, etc).
...
|-----------|------------------|
| orphan_notes | For each note in {vocabulary.knowledge}/, count incoming [[links]]. Zero = orphan. |
...
| moc_oversize | For each topic map, count linked knowledge. |
| stale_notes | Notes not modified in 30+ days with < 2 links. |
| low_link_density | Average link count across all knowledge. |
...
| **Note count** | Count `*.md` in {vocabulary.knowledge}/ | Total knowledge for context |
| **Orphan knowledge** | For each note, grep for `[[filename]]` across all files — zero hits = orphan | Count, first 5 names |
| **Dangling links** | Extract all `[[links]]` from knowledge/, verify each target file exists | Count, first 5 targets |
| **Stale knowledge** | Notes not modified recently AND with low link density (< 2 links) | Count |
...
- orphan_notes: "{N} knowledge invisible to traversal"
...
**When recommending inbox processing:** Choose the specific inbox item that aligns best with current goals or has the most connection potential to existing knowledge. Recommend a concrete file, not "process some inbox."
...
**When recommending reweaving:** Choose the most-connected stale note (highest link density + oldest modification). Reweaving high-connectivity knowledge has the highest ripple effect.
...
next
  ...
  or revisit older {DOMAIN:knowledge} to deepen the graph.
```
...
| Good | Bad |
|------|-----|
| `/{DOMAIN:extract} inbox/article-on-spaced-repetition.md` | "process some inbox items" |
| `/ralph 5` | "work on the queue" |
| `/{DOMAIN:rethink}` | "review your observations" |
| `/{DOMAIN:revisit} [[note title here]]` | "update some old knowledge" |
...
### Empty Vault (0-5 knowledge)

Recommend capturing or reducing content. Maintenance is premature with < 5 knowledge — the graph does not have enough nodes for meaningful analysis.
...
next
  ...
  Recommended: Capture or /{DOMAIN:extract} content
  Rationale: Your graph has [N] knowledge. At this stage, adding
  content matters more than maintaining structure. Health checks,
  reweaving, and rethink become valuable after ~10 knowledge.
```

### Everything Clean

Say so explicitly. Recommend exploratory work aligned with goals, or reflective work on older knowledge:

```
  No urgent work detected. Consider:
  - Exploring a research direction from goals.md
  - Reweaving older {DOMAIN:knowledge} to deepen connections
