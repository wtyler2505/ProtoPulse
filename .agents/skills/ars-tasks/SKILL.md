...
Discoveries are captured during pipeline work (e.g., /extract knowledge a connection opportunity, /connect notices a split candidate). They accumulate here until the user decides to convert them to tasks or discard them.
...
- `/extract` adds tasks to the queue AND knowledge discoveries in tasks.md
...
- topic map [[topic]] might need splitting (40+ knowledge observed during /connect)
```
