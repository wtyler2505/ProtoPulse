...
3. **`ops/methodology/`** — existing methodology knowledge (read all to understand current system self-knowledge)
...
Every rule in the context file, every workflow in a skill, every assumption baked into the architecture was a hypothesis at some point. Hypotheses need testing against reality. Observation knowledge in `ops/observations/` capture friction from actual use. Tension knowledge in `ops/tensions/` capture unresolved conflicts. Rethink first triages these individually (some become {DOMAIN:knowledge}, some become methodology updates, some get archived), then compares remaining evidence against what the system assumes and proposes changes when patterns emerge.
...
```bash
# Get all methodology knowledge with their metadata
...
Read all methodology knowledge fully. Extract:
...
- The context file (GEMINI.md) — current behavioral instructions
...
- Maintenance conditions (methodology knowledge about when maintenance triggers?)
- Session rhythm (methodology knowledge about session workflow?)
- Domain-specific behaviors (methodology knowledge about domain vocabulary and patterns?)
...
- Are there other methodology knowledge that contradict this one?
...
Drift Check:
  ...
  Coverage gaps: [N features without methodology knowledge]
...
Read each pending item fully. These are small atomic knowledge — load all of them. Understanding the full content is required for accurate triage. If zero pending items, report clean state and exit early.

Also read `ops/methodology/` to understand existing methodology knowledge — this prevents creating duplicates and informs whether new observations should extend existing methodology rather than create new knowledge.
...
|-------------|---------|---------------|--------|
| PROMOTE | Reusable insight worth keeping as a permanent {DOMAIN:note} | General principle across sessions. Would work as a claim note. Crystallized insight, not operational guidance. | Create {DOMAIN:note} in {vocabulary.knowledge}/, set observation `status: promoted`, add `promoted_to: [[title]]` |
...
- Tension reveals a genuine conflict between two {DOMAIN:knowledge} → PROMOTE (create a tension {DOMAIN:note} or resolution {DOMAIN:note})
...
**For PROMOTE items:**
1. Create {DOMAIN:note} with prose-as-title in {vocabulary.knowledge}/
...
**Update MOCs:** After triage execution, update `ops/observations.md` and `ops/tensions.md` to connect status changes. Move entries between Pending/Promoted/Archived/Resolved/Dissolved sections as appropriate.

---

## Phase 2: Methodology Folder Updates

For items triaged as METHODOLOGY, create or update knowledge in `ops/methodology/`.
...
After creating or updating methodology knowledge, update `ops/methodology.md`:
- Add new knowledge to the appropriate category section
- Update context phrases for modified knowledge
...
4. **Methodology knowledge** — patterns in `ops/methodology/` by category
...
5. **Compare methodology knowledge against context file:** If `ops/methodology/` has 3+ knowledge in the same category that are not reflected in the context file, the methodology has converged enough for elevation.
...
--=={ {DOMAIN:rethink} — Complete }==--
  ...
  Triaged: [N] observations, [M] tensions
    ...
    Promoted to {DOMAIN:knowledge}:  [count]
...
If any observations were promoted to {DOMAIN:knowledge}:

```
  [count] {DOMAIN:knowledge} were promoted from observations.
  Run /{DOMAIN:connect} on promoted knowledge to find connections.
...
- Add each promoted {DOMAIN:note} to the queue with `current_phase: "connect"` (the note already exists, so skip create)
...
- Create {DOMAIN:knowledge} directly from observations without going through standard pipeline (PROMOTE adds to queue)
