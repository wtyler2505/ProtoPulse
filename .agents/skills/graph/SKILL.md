...
If no derivation file exists, use universal terms (knowledge, MOCs, etc.).
...
```bash
# Count total knowledge (excluding MOCs)
...
# Find orphan knowledge (zero incoming links)
...
# topic map coverage: % of knowledge appearing in at least one topic map's Core Ideas
COVERED=0
for f in "$NOTES_DIR"/*.md; do
  NAME=$(basename "$f" .md)
...
- `ops/scripts/graph/orphan-knowledge.sh` for orphan detection
...
--=={ graph bridges }==--
  ...
  [If no bridges: "No bridge knowledge found. The graph has redundant paths between
...
2. Traverse all reachable knowledge via wiki links (bidirectional)
3. Mark as one component
4. Repeat until all knowledge visited
...
--=={ graph hubs }==--
  ...
  [If no clear synthesizers: "No knowledge score high on both metrics.
...
3. Track visited knowledge to avoid cycles
...
Find all knowledge that link TO this {vocabulary.note} (hop 1).
...
|-------|----------------|---------|
| `topics [[X]]` | `rg '^topics:.*\[\[X\]\]'` | Find knowledge in a topic |
| `type tension` | `rg '^type: tension'` | Find knowledge by type |
| `methodology X` | `rg '^methodology:.*X'` | Find knowledge by tradition |
| `status open` | `rg '^status: open'` | Find knowledge by status |
| `created 2026-02` | `rg '^created: 2026-02'` | Find knowledge by date range |
| `source [[X]]` | `rg '^source:.*\[\[X\]\]'` | Find knowledge from a source |
...
| "What are my most important knowledge?" | hubs | Authority/hub ranking |
...
| "Show me knowledge about [topic]" | query topics [[topic]] | Schema query |
...
Use universal vocabulary (knowledge, MOCs, etc.). All operations work identically.
