---
name: validate
description: Schema validation for knowledge. Checks against domain-specific templates. Validates required fields, enum values, description quality, and link health. Non-blocking — warns but doesn't prevent capture. Triggers on "/validate", "/validate [note]", "check schema", "validate note", "validate all".
user-invocable: true
...
- If target is "all" or "knowledge": validate all knowledge in {DOMAIN:knowledge}/ directory
...
Good (adds mechanism):
- Title: `vector proximity measures surface overlap not deep connection`
- Description: "Two knowledge about the same concept with different vocabulary score high, while genuinely related ideas across domains score low"
...
| Relevant knowledge links | Each note in `relevant_notes` must exist | Verify each wiki link in relevant_notes resolves |
...
When validating all knowledge (target is "all" or "knowledge"):

1. Discover all .md files in {DOMAIN:knowledge}/ directory
...
### Top WARN Patterns
- Description restates title: N knowledge
- Missing context phrases in relevant_notes: N knowledge
- Enum value not in template: N knowledge
