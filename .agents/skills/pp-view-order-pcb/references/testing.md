...
- `npm run test -- client/src/components/views/__tests__/PcbOrderingView.test.tsx` passed after naming the board quantity and dimensions inputs.
- `npm run test:keyboard-nav -- --reporter=dot --workers=1 --grep "ordering \\(Order PCB\\)"` passed with Ordering at 19/20 reachable stops and 29 distinct targets.
- Full `npm run test:keyboard-nav -- --reporter=dot --workers=1` passed later in R23 with Ordering at 20/20 reachable stops and 29 distinct targets.
