...
ops/derivation-manifest.md  — domain vocabulary (inbox folder, extract skill name)
```
...
Format for downstream extract extraction — findings as clear propositions, not raw dumps:
...
should be a clear proposition the extract phase can extract as an atomic insight.]
...
Research complete
  ...
  Next: /[extract-skill-name] [inbox-folder]/[filename]
```

Append based on mode:
- **manual:** (nothing extra)
- **suggested:** `Ready for processing when you are.`
- **automatic:** Replace "Next" line with `Queued for /[extract-skill-name] -- processing will begin automatically.`
...
| Phase | Skill | Purpose |
|-------|-------|---------|
| Extract insights | /[extract-name] | Mine research for atomic propositions |
| Find connections | /[connect-name] | Link new insights to existing graph |
| Update old knowledge | /[revisit-name] | Backward pass on touched knowledge |
