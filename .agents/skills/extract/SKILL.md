...
1. **`ops/derivation-manifest.md`** — vocabulary mapping, extraction categories, platform hints
   ...
   - Use `vocabulary.extract` for the process verb in output
...
|---------|---------------|---------|
| **Having knowledge** | The vault contains information | "We store knowledge in folders" |
...
Extract composable {vocabulary.note_plural} from source material into {vocabulary.knowledge}/.
...
|---------|---------|-----------------|
| **We DO this** | "We tag knowledge with topics" | — (not sufficient) |
...
```bash
# Get descriptions from existing knowledge
...
IMPLEMENTATION IDEAS (methodology knowledge):
1. [feature/pattern] — what it enables, why it matters
...

TENSIONS (tension knowledge):
...
Write to: `{vocabulary.knowledge}/[title].md`
...
- Rejecting implementation ideas as "not claims" (they ARE extractable as methodology knowledge)
- Rejecting tensions as "not claims" (they become tension knowledge)
...
- Tension knowledge (sections: Quick Test, When Each Pole Wins, Dissolution Attempts, Practical Applications)
- {vocabulary.topic_map} knowledge (sections: Synthesis, Core Ideas, Tensions, Explorations Needed, Agent Notes)
...
- The chain: research query -> inbox file -> /{vocabulary.extract} -> {vocabulary.knowledge}
...
- 6 implementation ideas -> methodology knowledge
- 4 tensions -> tension knowledge
...
|-----------|---------------|-----|
| New content to process | /{vocabulary.extract} | Extraction requires quality gates |
...
- **manual:** Output "Next: {vocabulary.cmd_reflect} [created knowledge]" — user decides when to proceed
