...
1. **`ops/derivation-manifest.md`** — vocabulary mapping, extraction categories, platform hints
   ...
   - Use `vocabulary.cmd_revisit` for the backward-pass command name
...
- knowledge folder: `knowledge/` (the manifest maps `vocabulary.knowledge` to `knowledge/` — there is no other fallback)
- inbox folder: `inbox/`
...
| {vocabulary.note} just created | /{vocabulary.cmd_connect} | New {vocabulary.note_plural} need connections |
| After connecting | /{vocabulary.cmd_revisit} | Old {vocabulary.note_plural} need updating |
...
- **manual:** Output "Next: {vocabulary.cmd_connect} [created notes]" — user decides when to proceed
