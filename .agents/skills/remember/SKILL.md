---
name: remember
description: Capture friction as methodology knowledge. Three modes — explicit description, contextual (review recent corrections), session mining (scan transcripts for patterns). Triggers on "/remember", "/remember [description]".
version: "1.0"
...
3. **`ops/methodology/`** — read existing methodology knowledge before creating new ones (prevents duplicates)
...
User provides a description: `/remember "don't process personal knowledge like research"` or `/remember always check for duplicates before creating`
...
|----------|-------------|
| processing | How to extract, extract, or handle content |
| capture | How to record, file, or organize incoming material |
| connection | How to find, evaluate, or add links between knowledge |
...
| quality | Standards for knowledge, descriptions, titles |
...
| Check Result | Action |
|-------------|--------|
| No existing knowledge in this area | Create new methodology note |
...
**Filename:** Convert the prose title to kebab-case. Example: "don't process personal knowledge like research" becomes `dont-process-personal-knowledge-like-research.md`.
...
**Writing quality for methodology knowledge:**
- Be specific enough that a fresh agent session could follow this guidance without additional context
- Use concrete examples where possible — "when processing therapy knowledge" not "when processing certain types of content"
- State both the DO and the DON'T — methodology knowledge that only say what to do miss the anti-pattern that triggered them
- Keep scope explicit — unbounded methodology knowledge get applied where they should not be
...
Count methodology knowledge in the same category:

```bash
grep -rl "^category: [CATEGORY]" ops/methodology/ 2>/dev/null | wc -l | tr -d ' '
```

If 3+ knowledge exist in the same category, this is a signal for /{DOMAIN:rethink}:
...
The agent reviews the current conversation to find corrections the user made that should become methodology knowledge.
...
|------------|-------------------|---------|
| Direct correction | "no", "that's wrong", "not like that", "incorrect" | "No, don't split that into separate knowledge" |
...
**Wait for user confirmation.** Do not create knowledge from inferred corrections without approval — the agent might misinterpret what the user meant.
...
```
  ...
  Capture all as methodology knowledge? (all / select numbers / none)
```
...
Before creating any knowledge:

1. Read all existing methodology knowledge in `ops/methodology/`
...
--=={ remember — mine }==--
  ...
  Methodology knowledge created: [count]
  ...
  Category "[category]" now has [N] methodology knowledge.
...
Work happens
  ...
  → agent reads methodology knowledge at session start (via context file reference)
  → agent behavior improves
  → fewer corrections needed
  → when methodology knowledge accumulate (3+ in same category)
...
The loop is healthy when methodology knowledge accumulate slowly (friction is being addressed) and /rethink elevates patterns to context-level changes when thresholds are exceeded.

The loop is unhealthy when the same category keeps getting methodology knowledge without elevation (the system is capturing friction but not learning from it).
...
- Future /rethink sessions will compare system behavior against what methodology knowledge declare. Vague knowledge create unmeasurable specs.
...
- When methodology knowledge are stale (older than config changes), the system surfaces this as a maintenance condition.
...
| "processing too aggressive" | "differentiate personal knowledge from research in processing depth" |

The title is what the agent reads at session start. It should be immediately actionable as a behavioral directive.

### Body Quality

Methodology knowledge are operational guidance, not essays. They should be:
...
| connection | Friction during /connect, link evaluation, topic map updates |
...
```markdown
---
description: Methodology knowledge capturing how this system has learned to operate
type: moc
---

# methodology

Methodology knowledge organized by category. Each note captures a specific behavioral learning.
...
In contextual mode, flag these as lower-confidence findings and always confirm before creating methodology knowledge from implicit signals.
