...
# Connection count (all wiki links across knowledge/)
...
```bash
# Orphan count (knowledge with zero incoming links)
ORPHAN_COUNT=0
...
# Schema compliance (% of knowledge with required fields: description, topics)
...
# Processed ratio (knowledge vs inbox)
...
```bash
# This week's growth (knowledge with created: date within last 7 days)
...
# This week's connections (approximate — count links in recently created knowledge)
...
| PROCESSED_PCT < 50 | "More content in inbox than in {vocabulary.knowledge}/ — consider processing backlog" |
| THIS_WEEK_NOTES == 0 | "No new {vocabulary.note_plural} this week" |

Only show interpretation knowledge when conditions are notable. A healthy vault gets just the stats, no warnings.
...
Use universal vocabulary (knowledge, MOCs, etc.). All metrics work identically.

### Very Large Vault (500+ knowledge)

The orphan and topic map coverage checks may be slow for large vaults. If {vocabulary.knowledge}/ has >200 files:
