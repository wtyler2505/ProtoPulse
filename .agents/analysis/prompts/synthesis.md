...
Read `~/.agents/skills/product-analysis/references/cross-phase-analysis.md` for the full methodology. Apply these 10 cross-phase connections:
...
- Search "EDA / Electronics Prototyping Platform (breadboard, schematic, PCB, 3D, simulation, digital twin, inventory, procurement) industry benchmarks {year}" — contextualize the project's metrics (is 50 test files good or bad for a project this size?)
- Search "EDA / Electronics Prototyping Platform (breadboard, schematic, PCB, 3D, simulation, digital twin, inventory, procurement) product maturity model" — frame the project's current stage
- Search "technical debt benchmarks {language}" — put the `lizard`/`scc` numbers in industry perspective
- Search "EDA / Electronics Prototyping Platform (breadboard, schematic, PCB, 3D, simulation, digital twin, inventory, procurement) startup success factors" — help prioritize the checklist based on what matters most for market success
