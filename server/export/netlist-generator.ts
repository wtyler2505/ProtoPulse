...
export function generateKicadNetlist(input: NetlistInput): string {
  ...
  netNameOrder.forEach((name) => { netNodes.set(name, []); });
  ...
  netNameOrder.forEach((netName, idx) => {
    const code = idx + 1;
...
export function generateCsvNetlist(input: NetlistInput): string {
  ...
  const resolvePinDisplay = (instanceId: number, pinRef: string): string => {
    ...
    const byId = part.connectors.find((c) => c.id === pinRef);
    if (byId) return byId.name;
    const byName = part.connectors.find((c) => c.name === pinRef);
