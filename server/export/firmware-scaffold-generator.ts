...
import { detectBoard, getPinsByCapability } from './firmware-templates';
...
function generateConfigH(
  ...
  if (boardTemplate) {
    lines.push(` * Board: ${boardTemplate.displayName}`);
    lines.push(` * MCU: ${boardTemplate.mcu} (${boardTemplate.pins.length} pins)`);
    lines.push(` * PlatformIO: ${boardTemplate.platformioBoard} / ${boardTemplate.platformioPlatform}`);
  } else {
  ...
  // Board-specific pin defines from template
  if (boardTemplate) {
    const pwmPins = getPinsByCapability(boardTemplate, 'pwm');
    const adcPins = getPinsByCapability(boardTemplate, 'adc');
    const dacPins = getPinsByCapability(boardTemplate, 'dac');

    lines.push('// ---------------------------------------------------------------------------');
    lines.push(`// Board: ${boardTemplate.displayName} — Pin Summary`);
    lines.push('// ---------------------------------------------------------------------------');
    if (pwmPins.length > 0) {
      lines.push(`// PWM pins: ${pwmPins.map((p) => p.name).join(', ')}`);
    }
    if (adcPins.length > 0) {
      lines.push(`// ADC pins: ${adcPins.map((p) => p.name).join(', ')}`);
    }
    if (dacPins.length > 0) {
      lines.push(`// DAC pins: ${dacPins.map((p) => p.name).join(', ')}`);
    }
    lines.push('');

    // Board-specific notes
    if (boardTemplate.notes.length > 0) {
      lines.push('// Board notes:');
      for (const note of boardTemplate.notes) {
        lines.push(`//   - ${note}`);
      }
      lines.push('');
    }
...
function generateMainCpp(
  ...
  lines.push('/**');
  if (boardTemplate) {
    lines.push(` * main.cpp — Firmware scaffold for ${boardTemplate.displayName}`);
    lines.push(` * MCU: ${boardTemplate.mcu}`);
  } else {
  ...
  if (boardTemplate && boardTemplate.defaultLibraries.length > 0) {
    lines.push(' *');
    lines.push(` * Recommended libraries: ${boardTemplate.defaultLibraries.join(', ')}`);
  }
  lines.push(' */');
  lines.push('');
...
function generatePlatformioIni(
  ...
  // Prefer board template values when available (more accurate than MCU profile)
  const platform = boardTemplate?.platformioPlatform ?? mcu.platform;
  const board = boardTemplate?.platformioBoard ?? mcu.board;
  const framework = boardTemplate?.platformioFramework ?? mcu.framework;
  const displayTarget = boardTemplate ? `${boardTemplate.displayName} (${board})` : `${mcu.family} (${board})`;

  lines.push('; platformio.ini');
  lines.push(`; Generated by ProtoPulse EDA on ${now}`);
  lines.push(`; Target: ${displayTarget}`);
  lines.push(';');
...
export function generateFirmwareScaffold(data: {
  ...
  // 1b. Try to resolve a detailed board template from node labels
  const boardTemplate = mcuNode ? detectBoard(mcuNode.label) : undefined;
