...
export function registerCircuitImportRoutes(app: Express, storage: IStorage): void {
  ...
  app.post('/api/projects/:projectId/import/fzz', requireProjectOwnership, payloadLimit(10 * 1024 * 1024), async (req, res) => {
  ...
  app.post('/api/projects/:projectId/import/kicad', requireProjectOwnership, payloadLimit(10 * 1024 * 1024), async (req, res) => {
    const projectId = parseIdParam(req.params.projectId);
