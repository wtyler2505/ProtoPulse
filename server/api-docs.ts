...
export const apiDocs: RouteDoc[] = [
  ...
  { method: "POST", path: "/api/settings/api-keys", description: "Store an API key for a provider", body: "{ provider: 'gemini'|'jlcpcb'|'pcbway'|'oshpark'|'google_workspace', apiKey: string }", response: "{ message: string }" },
  { method: "DELETE", path: "/api/settings/api-keys/:provider", description: "Delete a stored API key", params: { provider: "gemini|jlcpcb|pcbway|oshpark|google_workspace" }, response: "204 No Content" },
