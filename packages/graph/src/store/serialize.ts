...
export interface GraphSnapshotJson {
  ...
  pcb: { placements: unknown[]; traces: unknown[]; vias: unknown[]; zones?: unknown[]; outline?: unknown };
  annotations: unknown[];
...
export function graphToJson(graph: DesignGraph): GraphSnapshotJson {
  return {
    ...
    pcb: {
      ...
      ...(graph.pcb.outline ? { outline: graph.pcb.outline } : {}),
    },
...
export function graphFromJson(json: GraphSnapshotJson): DesignGraph {
  return {
    ...
    sheets: new Map((json.sheets ?? []) as [string, never][]),
    ...
    pcb: {
      ...
      ...(json.pcb.outline !== undefined ? { outline: json.pcb.outline as never } : {}),
