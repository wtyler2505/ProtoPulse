...
describe('Esp32s3Core', () => {
  ...
  it('routes the SYSTIMER COMP1 alarm (driven by UNIT1) to its own interrupt source', () => {
    // The second comparator, COMP1: TARGET1 registers (0x24/0x28/0x38), COMP1_LOAD
    // 0x54, INT bit 1, CONF enable bit 8, matrix map 0x0e8 (source 58). TARGET1_CONF
    // bit 31 (TIMER_UNIT_SEL) drives it from UNIT1 instead of UNIT0 — exercised here.
    const SCRATCH = ESP32S3_DRAM_BASE + 0x9a0;
    const image = assembleXtensa(
      ESP32S3_IRAM_BASE,
      [UART, 0x60023000, INTMTX, SCRATCH, ESP32S3_IRAM_BASE, 0x80000000],
      [
        L32R(13, 3), // scratch
        MOVI(14, 0),
        S32I(14, 13, 0), // ISR counter = 0
        ...
        L32R(3, 1), // SYSTIMER
        MOVI(4, 50),
        S32I(4, 3, 0x28), // TARGET1_LO = 50
        MOVI(4, 0),
        S32I(4, 3, 0x24), // TARGET1_HI = 0
        L32R(4, 5),
        S32I(4, 3, 0x38), // TARGET1_CONF = TIMER_UNIT_SEL (drive COMP1 from UNIT1)
        MOVI(4, 1),
        S32I(4, 3, 0x54), // COMP1_LOAD
        MOVI(4, 2),
        S32I(4, 3, 0x64), // INT_ENA = TARGET1 (bit 1)
        MOVI(4, 0x104),
        S32I(4, 3, 0x00), // CONF = UNIT1_WORK_EN (bit 2) | TARGET1_WORK_EN (bit 8)

        L32R(15, 2), // interrupt matrix
        MOVI(4, 0),
        S32I(4, 15, 0xe8), // SYSTIMER_TARGET1 (source 58) -> CPU line 0
        MOVI(4, 1),
        WSR(4, SR.INTENABLE),
        RSIL(12, 0),
        ...
        MOVI(11, 1),
        L32I(4, 13, 0),
        BNE(4, 11, BR(-2)), // spin until the COMP1 ISR has run
        ...
        L32R(3, 1),
        MOVI(4, 1),
        S32I(4, 3, 0x24), // TARGET1_HI = 1 (reprogram forward)
        MOVI(4, 0),
        S32I(4, 3, 0x28), // TARGET1_LO = 0
        MOVI(4, 1),
        S32I(4, 3, 0x54), // COMP1_LOAD
        MOVI(4, 2),
        S32I(4, 3, 0x6c), // INT_CLR = TARGET1
        L32I(3, 2, 8),
        RSR(2, SR.EXCSAVE1),
        RFE(),
      ],
  ...
  it('wakes from deep sleep via a GPIO high-level source by resetting the core', () => {
    // Deep sleep (DIG_PWC.DG_WRAP_PD_EN) powers the digital core down, so ANY armed
    // wake source — not just the RTC timer — boots the chip fresh with DEEPSLEEP_RESET
    // rather than resuming WAITI. Here the GPIO high-level source (gpio_reg.h GPIO_PINn
    // WAKEUP_ENABLE bit 10 + INT_TYPE [9:7]=5, soc/rtc.h RTC_GPIO_TRIG_EN = BIT(2)) drives
    // the reset. GPIO_PIN7 = GPIO + 0x90. Same DIG_PWC/STATE0/RESET_STATE map as the timer
    // deep-sleep slice.
    const RTCCNTL = 0x60008000;
    const dgWrapPdEn = 0x80000000;
    const wakeupGpio = (1 << 2) << 15; // RTC_GPIO_TRIG_EN in the WAKEUP_ENA field
    const gpioPin7HighWake = (1 << 10) | (5 << 7); // WAKEUP_ENABLE + high-level INT_TYPE
    const c = core(
      assembleXtensa(ESP32S3_IRAM_BASE, [RTCCNTL, UART, dgWrapPdEn, wakeupGpio, gpioPin7HighWake, GPIO], [
        L32R(2, 0), // RTC_CNTL
        L32R(7, 5), // GPIO
        L32R(6, 1), // UART
        L32I(3, 2, 0x38), // RESET_STATE
        ...
        MOVI(5, 5), // DEEPSLEEP_RESET
        BNE(3, 5, BR(1)), // first boot (cause != 5) -> deep-sleep setup
        J(BR(9)), // woke from deep sleep -> emit marker
        ...
        L32R(4, 2),
        S32I(4, 2, 0x18), // STATE0.SLEEP_EN -> enter deep sleep
        J(BR(-1)), // spin until the GPIO wake resets the chip
    ...
    c.step(2_000); // boot 1: configure deep sleep, then spin armed
    expect([...c.drainUart()]).toEqual([]); // still asleep, no reset yet
    c.setPin('IO7', 1); // high level on the armed RTC GPIO -> wake -> deep-sleep reset
    c.step(2_000); // consume the pending reset: the core reboots and step() returns
    c.step(2_000); // boot 2: reboot reads DEEPSLEEP_RESET and emits the marker
    expect([...c.drainUart()]).toEqual([0xcd]);
  });

  it('wakes from deep sleep via an EXT0 RTC-IO source by resetting the core', () => {
    // Same source-agnostic deep-sleep path as the GPIO/timer cases, but driven by
    // the EXT0 RTC-IO wake: EXT_WAKEUP_CONF.EXT_WAKEUP0_LV (RTC_CNTL + 0x64, bit 30
    // = high) plus RTC_IO_EXT_WAKEUP0_SEL (RTC_IO + 0xdc, GPIO num in [31:27]),
    // armed via RTC_EXT0_TRIG_EN = BIT(0) in the WAKEUP_ENA field. RTC GPIO7 -> IO7.
    const RTCCNTL = 0x60008000;
    const RTCIO = 0x60008400;
    const dgWrapPdEn = 0x80000000;
    const wakeupExt0 = (1 << 0) << 15; // RTC_EXT0_TRIG_EN in the WAKEUP_ENA field
    const ext0LvHigh = 1 << 30; // EXT_WAKEUP0_LV = wake on high level
    const ext0Gpio7 = 7 << 27; // EXT_WAKEUP0_SEL = RTC GPIO7
    const c = core(
      assembleXtensa(ESP32S3_IRAM_BASE, [RTCCNTL, UART, dgWrapPdEn, wakeupExt0, ext0LvHigh, ext0Gpio7, RTCIO], [
        L32R(2, 0), // RTC_CNTL
        L32R(8, 6), // RTC_IO
        L32R(6, 1), // UART
        L32I(3, 2, 0x38), // RESET_STATE
        ...
        L32R(4, 2),
        S32I(4, 2, 0x18), // STATE0.SLEEP_EN -> enter deep sleep
        J(BR(-1)), // spin until the EXT0 wake resets the chip
    ...
    c.step(2_000); // boot 1: configure deep sleep, then spin armed
    expect([...c.drainUart()]).toEqual([]); // still asleep, no reset yet
    c.setPin('IO7', 1); // high level on the armed EXT0 RTC IO -> wake -> deep-sleep reset
    c.step(2_000); // consume the pending reset: the core reboots and step() returns
    c.step(2_000); // boot 2: reboot reads DEEPSLEEP_RESET and emits the marker
    expect([...c.drainUart()]).toEqual([0xce]);
  });

  it('wakes from deep sleep via an EXT1 RTC-GPIO-mask source by resetting the core', () => {
    // EXT1 deep-sleep wake through the same source-agnostic reset path: a RTC-GPIO
    // bitmask in EXT_WAKEUP1_SEL (RTC_CNTL + 0xe0) plus EXT_WAKEUP_CONF.EXT_WAKEUP1_LV
    // (RTC_CNTL + 0x64, bit 31 = any-high), armed via RTC_EXT1_TRIG_EN = BIT(1) in the
    // WAKEUP_ENA field. Bit 7 of the mask selects RTC GPIO7 -> IO7.
    const RTCCNTL = 0x60008000;
    const dgWrapPdEn = 0x80000000;
    const wakeupExt1 = (1 << 1) << 15; // RTC_EXT1_TRIG_EN in the WAKEUP_ENA field
    const ext1LvHigh = 1 << 31; // EXT_WAKEUP1_LV = wake on high level
    const ext1Gpio7 = 1 << 7; // EXT_WAKEUP1_SEL mask bit for RTC GPIO7
    const c = core(
      assembleXtensa(ESP32S3_IRAM_BASE, [RTCCNTL, UART, dgWrapPdEn, wakeupExt1, ext1LvHigh, ext1Gpio7], [
        L32R(2, 0), // RTC_CNTL
        L32R(6, 1), // UART
        L32I(3, 2, 0x38), // RESET_STATE
        MOVI(4, 0x3f),
        ...
        L32R(4, 5),
        S32I(4, 2, 0xe0), // EXT_WAKEUP1_SEL = RTC GPIO7 mask
        L32R(4, 2),
        S32I(4, 2, 0x18), // STATE0.SLEEP_EN -> enter deep sleep
        J(BR(-1)), // spin until the EXT1 wake resets the chip
    ...
    c.step(2_000); // boot 1: configure deep sleep, then spin armed
    expect([...c.drainUart()]).toEqual([]); // still asleep, no reset yet
    c.setPin('IO7', 1); // high level on the armed EXT1 RTC GPIO -> wake -> deep-sleep reset
    c.step(2_000); // consume the pending reset: the core reboots and step() returns
    c.step(2_000); // boot 2: reboot reads DEEPSLEEP_RESET and emits the marker
  ...
  it('hashes a one-block message through the SHA-1 mode', () => {
    // SHA-1 (SHA_MODE = 0) reuses the same message/digest registers but runs the
    // 80-round SHA-1 compression onto a 5-word digest. The padded "abc" block is
    // identical to the SHA-256 case; the digest is the NIST SHA-1("abc") =
    // a9993e36 4706816a ba3e2571 7850c26c 9cd0d89d. The guest emits each digest
    // word's low byte (5 words).
    const SHA = 0x6003b000;
    const c = core(
      assembleXtensa(ESP32S3_IRAM_BASE, [SHA, UART, 0x61626380], [
        L32R(2, 0), // SHA base
    ...
    c.step(200);
    expect([...c.drainUart()]).toEqual([0x36, 0x6a, 0x71, 0x6c, 0x9d]);
  });

  it('hashes a one-block message through the SHA-224 mode', () => {
    // SHA-224 (SHA_MODE = 1) runs the SHA-256 compression with the SHA-224 initial
    // vector and a truncated 7-word (224-bit) digest. Padded "abc" block as before;
    // digest = NIST SHA-224("abc") = 23097d22 3405d822 8642a477 bda255b3 2aadbce4
    // bda0b3f7 e36c9da7. The guest emits each of the 7 digest words' low byte.
    const SHA = 0x6003b000;
    const c = core(
      assembleXtensa(ESP32S3_IRAM_BASE, [SHA, UART, 0x61626380], [
        L32R(2, 0), // SHA base
        L32R(6, 1), // UART
        MOVI(4, 1),
        S32I(4, 2, 0x00), // SHA_MODE = SHA-224
        L32R(4, 2),
        S32I(4, 2, 0x80), // M[0]
        MOVI(4, 0),
        S32I(4, 2, 0x84),
  ...
  it('encrypts a block through the AES-192 ECB accelerator', () => {
    ...
    const c = core(
      assembleXtensa(
        ESP32S3_IRAM_BASE,
        [AES, UART, 0x00010203, 0x04050607, 0x08090a0b, 0x0c0d0e0f, 0x10111213, 0x14151617, 0x00112233, 0x44556677, 0x8899aabb, 0xccddeeff],
        [
          L32R(2, 0),
          L32R(6, 1),
          L32R(4, 2),
          ...
          S32I(4, 2, 0x10),
          L32R(4, 7),
          S32I(4, 2, 0x14), // KEY[0..5]
          MOVI(4, 1),
          S32I(4, 2, 0x40), // MODE = AES-192
          L32R(4, 8),
          S32I(4, 2, 0x20),
          L32R(4, 9),
          S32I(4, 2, 0x24),
    ...
    c.step(400);
    expect([...c.drainUart()]).toEqual([2, 0xdd, 0xa4, 0x86, 0xe0, 0x6e, 0xa0, 0xec, 0x91]);
  });

  it('encrypts a block through the AES-256 ECB accelerator', () => {
    // AES-256 (AES_MODE = 2): 8 key words. FIPS-197 vector: key 000102..1f,
    // plaintext 00112233..ff -> ciphertext 8ea2b7ca 516745bf eafc4990 4b496089.
    const AES = 0x6003a000;
    const c = core(
      assembleXtensa(
        ESP32S3_IRAM_BASE,
        [AES, UART, 0x00010203, 0x04050607, 0x08090a0b, 0x0c0d0e0f, 0x10111213, 0x14151617, 0x18191a1b, 0x1c1d1e1f, 0x00112233, 0x44556677, 0x8899aabb, 0xccddeeff],
        [
          L32R(2, 0),
          L32R(6, 1),
          L32R(4, 2),
          ...
          S32I(4, 2, 0x18),
          L32R(4, 9),
          S32I(4, 2, 0x1c), // KEY[0..7]
          MOVI(4, 2),
          S32I(4, 2, 0x40), // MODE = AES-256
          L32R(4, 10),
          S32I(4, 2, 0x20),
          L32R(4, 11),
          S32I(4, 2, 0x24),
  ...
  it('writes console bytes through the USB-Serial-JTAG TX FIFO', () => {
    // USB-Serial-JTAG (DR_REG_USB_DEVICE_BASE 0x60038000): EP1_REG (+0x00) is the
    // byte FIFO (RDWR_BYTE [7:0]); EP1_CONF_REG (+0x04) carries SERIAL_IN_EP_DATA_FREE
    // (bit1, RO, reads 1 when the TX FIFO has room) and WR_DONE (bit0, write 1 to
    // flush the staged bytes to the host). The guest first reads EP1_CONF (expects
    // the data-free bit, 0b10 = 2) and emits it over UART0, then pushes 'H','i' into
    // the FIFO and flushes — the two bytes land in drainUsbSerialJtag().
    const USJ = 0x60038000;
    const c = core(
      assembleXtensa(ESP32S3_IRAM_BASE, [USJ, UART, 0x48, 0x69], [
        L32R(2, 0), // USB-Serial-JTAG base
        L32R(6, 1), // UART
        L32I(4, 2, 0x04), // EP1_CONF (data-free bit1 set -> 2)
        S32I(4, 6, 0), // emit status over UART0
        L32R(4, 2),
        S32I(4, 2, 0x00), // EP1 <- 'H'
        L32R(4, 3),
        S32I(4, 2, 0x00), // EP1 <- 'i'
        MOVI(4, 1),
        S32I(4, 2, 0x04), // EP1_CONF WR_DONE -> flush to host
        J(BR(-1)),
      ]),
    );
    c.step(400);
  ...
  it('encrypts a CBC block through the AES-DMA path (GDMA-fed)', () => {
    // On the S3, AES-CBC runs through GDMA: a TX (mem->AES) descriptor feeds the
    // plaintext, an IN (AES->mem) descriptor receives the ciphertext. The guest sets
    // AES_BLOCK_MODE (+0x94) = CBC, the IV (+0x50), AES_BLOCK_NUM (+0x98) = 1,
    // AES_DMA_ENABLE (+0x90) = 1, then TRIGGER. NIST SP 800-38A F.2.1 vector:
    // key 2b7e1516…, IV 000102…0f, plaintext 6bc1bee2… -> ciphertext 7649abac…12e9197d.
    // The guest emits ciphertext word[0] and word[3] MSB-first.
    const AES = 0x6003a000;
    ...
    const c = core(
      assembleXtensa(
        ...
        [
          ...
          S32I(3, 2, 0), // dw0
          L32R(3, 3),
          S32I(3, 2, 4), // dw1 = ctBuf
          MOVI(3, 0),
          S32I(3, 2, 8), // dw2 = 0

          L32R(6, 6), // GDMA
          MOVI(7, 6), // GDMA_PERI_AES
          S32I(7, 6, 0xa8), // OUT_CH0_PERI_SEL
          L32R(7, 7),
          S32I(7, 6, 0x80), // OUT_CH0_LINK = start
          MOVI(7, 6),
          S32I(7, 6, 0x48), // IN_CH0_PERI_SEL
          L32R(7, 8),
          S32I(7, 6, 0x20), // IN_CH0_LINK = start
    ...
    c.step(600);
    expect([...c.drainUart()]).toEqual([0x76, 0x49, 0xab, 0xac, 0x12, 0xe9, 0x19, 0x7d]);
  });

  it('decrypts a CBC block through the AES-DMA path (GDMA-fed)', () => {
    // The inverse of the previous test (AES_MODE = 4 = AES-128 decrypt): the NIST
    // ciphertext 7649abac…12e9197d with IV 000102…0f decrypts back to the plaintext
    // 6bc1bee2…7393172a. The guest emits plaintext word[0] and word[3] MSB-first.
    const AES = 0x6003a000;
    const txDesc = ESP32S3_DRAM_BASE + 0x1200;
    const inBuf = ESP32S3_DRAM_BASE + 0x1240;
    const rxDesc = ESP32S3_DRAM_BASE + 0x1280;
    const outBuf = ESP32S3_DRAM_BASE + 0x12c0;
    const txDw0 = (GDMA_DESC_OWNER_DMA | GDMA_DESC_SUC_EOF | (16 << 12) | 16) >>> 0;
    const rxDw0 = (GDMA_DESC_OWNER_DMA | 16) >>> 0;
    const outLinkStart = ((txDesc & 0x000f_ffff) | GDMA_OUT_LINK_START) >>> 0;
    const inLinkStart = ((rxDesc & 0x000f_ffff) | GDMA_INLINK_AUTO_RET | GDMA_INLINK_START) >>> 0;
    const c = core(
      assembleXtensa(
        ESP32S3_IRAM_BASE,
        [
          txDesc, inBuf, rxDesc, outBuf, txDw0, rxDw0, GDMA, outLinkStart, inLinkStart, AES, UART,
          0x7649abac, 0x8119b246, 0xcee98e9b, 0x12e9197d, // ciphertext
          0x2b7e1516, 0x28aed2a6, 0xabf71588, 0x09cf4f3c, // key
          0x00010203, 0x04050607, 0x08090a0b, 0x0c0d0e0f, // IV
        ],
        [
          ...
          S32I(4, 9, 0x0c),
          MOVI(4, 4),
          S32I(4, 9, 0x40), // MODE = AES-128 decrypt
          L32R(4, 19),
          S32I(4, 9, 0x50),
          L32R(4, 20),
          S32I(4, 9, 0x54),
  ...
  it('encrypts two CFB-128 blocks through the AES-DMA path (GDMA-fed)', () => {
    // AES-CFB128 (AES_BLOCK_MODE = 5): keystream = E(feedback) from the IV, out = in XOR
    // keystream, feedback = the ciphertext (the output when encrypting). Two blocks so the
    // chaining (block 2 feeds back C_1) is exercised. NIST SP 800-38A F.3.13: key 2b7e1516…,
    // IV 000102…0f, plaintext 6bc1bee2… / ae2d8a57… -> ciphertext 3b3fd92e…e83cfb4a /
    // c8a64537…9f1ce58b (independently confirmed via OpenSSL aes-128-cfb). The hardware
    // enc/dec direction convention (AES_MODE bit) was verified against esp-idf's
    // esp_aes_crypt_cfb128 DMA path. The guest emits word0 and word3 of each block.
    const AES = 0x6003a000;
    const txDesc = ESP32S3_DRAM_BASE + 0x1200;
    const ptBuf = ESP32S3_DRAM_BASE + 0x1240;
    const rxDesc = ESP32S3_DRAM_BASE + 0x1280;
    ...
    const c = core(
      assembleXtensa(
        ...
        [
          ...
          S32I(4, 9, 0x5c),
          MOVI(4, 5),
          S32I(4, 9, 0x94), // BLOCK_MODE = CFB128
          MOVI(4, 2),
          S32I(4, 9, 0x98), // BLOCK_NUM = 2
          MOVI(4, 1),
          S32I(4, 9, 0x90), // DMA_ENABLE = 1
    ...
    c.step(800);
    expect([...c.drainUart()]).toEqual([
      0x3b, 0x3f, 0xd9, 0x2e, 0xe8, 0x3c, 0xfb, 0x4a, 0xc8, 0xa6, 0x45, 0x37, 0x9f, 0x1c, 0xe5, 0x8b,
    ]);
  });

  it('decrypts two CFB-128 blocks through the AES-DMA path (GDMA-fed)', () => {
    // AES-CFB128 decrypt (AES_BLOCK_MODE = 5, AES_MODE = 4 = AES-128 decrypt): the block
    // cipher still runs forward (E); the decrypt direction feeds back the INPUT ciphertext.
    // Inverse of the previous test — input is the ciphertext 3b3fd92e… / c8a64537…, output
    // is the plaintext 6bc1bee2… / ae2d8a57… (round-trip confirmed via OpenSSL aes-128-cfb).
    const AES = 0x6003a000;
    const txDesc = ESP32S3_DRAM_BASE + 0x1200;
    const ctBuf = ESP32S3_DRAM_BASE + 0x1240;
    const rxDesc = ESP32S3_DRAM_BASE + 0x1280;
    const ptBuf = ESP32S3_DRAM_BASE + 0x12c0;
    const txDw0 = (GDMA_DESC_OWNER_DMA | GDMA_DESC_SUC_EOF | (32 << 12) | 32) >>> 0;
    const rxDw0 = (GDMA_DESC_OWNER_DMA | 32) >>> 0;
    const outLinkStart = ((txDesc & 0x000f_ffff) | GDMA_OUT_LINK_START) >>> 0;
    const inLinkStart = ((rxDesc & 0x000f_ffff) | GDMA_INLINK_AUTO_RET | GDMA_INLINK_START) >>> 0;
    const c = core(
      assembleXtensa(
        ESP32S3_IRAM_BASE,
        [
          txDesc, ctBuf, rxDesc, ptBuf, txDw0, rxDw0, GDMA, outLinkStart, inLinkStart, AES, UART,
          0x3b3fd92e, 0xb72dad20, 0x333449f8, 0xe83cfb4a, // ciphertext block 1
          0xc8a64537, 0xa0b3a93f, 0xcde3cdad, 0x9f1ce58b, // ciphertext block 2
          0x2b7e1516, 0x28aed2a6, 0xabf71588, 0x09cf4f3c, // key
          0x00010203, 0x04050607, 0x08090a0b, 0x0c0d0e0f, // IV
        ],
        [
    ...
    c.step(800);
    expect([...c.drainUart()]).toEqual([
      0x6b, 0xc1, 0xbe, 0xe2, 0x73, 0x93, 0x17, 0x2a, 0xae, 0x2d, 0x8a, 0x57, 0x45, 0xaf, 0x8e, 0x51,
    ]);
  });

  it('encrypts two CFB-8 blocks through the AES-DMA path (GDMA-fed)', () => {
    // AES-CFB8 (AES_BLOCK_MODE = 4): byte (8-bit segment) cipher feedback. For each input
    // byte the engine encrypts the 128-bit shift register, XORs the most-significant
    // keystream byte with the input byte, then shifts the register left one byte and
    // appends the ciphertext byte at the LSB end. Two blocks so the byte-feedback chain is
    // exercised across the block boundary (block 2 byte 0 depends on block 1's last byte).
    // Key 2b7e1516…, IV 000102…0f, plaintext 6bc1bee2… / ae2d8a57… -> ciphertext
    // 3b79424c…d4586a4f / 32b9ded5…67fb5052 (NIST SP 800-38A F.3.7 first byte 0x3b;
    // independently confirmed via OpenSSL aes-128-cfb8). esp-idf's esp_aes_crypt_cfb8 drives
    // this exact DMA path — aes_hal_mode_init(ESP_AES_BLOCK_MODE_CFB8) over full blocks. The
    // guest emits word0 and word3 of each output block.
    const AES = 0x6003a000;
    const txDesc = ESP32S3_DRAM_BASE + 0x1200;
    const ptBuf = ESP32S3_DRAM_BASE + 0x1240;
    const rxDesc = ESP32S3_DRAM_BASE + 0x1280;
    ...
    c.step(800);
    expect([...c.drainUart()]).toEqual([
      0x3b, 0x79, 0x42, 0x4c, 0xd4, 0x58, 0x6a, 0x4f, 0x32, 0xb9, 0xde, 0xd5, 0x67, 0xfb, 0x50, 0x52,
    ]);
  });

  it('decrypts two CFB-8 blocks through the AES-DMA path (GDMA-fed)', () => {
    // AES-CFB8 decrypt (AES_BLOCK_MODE = 4, AES_MODE = 4 = AES-128 decrypt): the block cipher
    // still runs forward (E); the decrypt direction feeds back the INPUT ciphertext byte.
    // Inverse of the previous test — input is the ciphertext 3b79424c… / 32b9ded5…, output is
    // the plaintext 6bc1bee2… / ae2d8a57… (round-trip confirmed via OpenSSL aes-128-cfb8).
    const AES = 0x6003a000;
    const txDesc = ESP32S3_DRAM_BASE + 0x1200;
    const ctBuf = ESP32S3_DRAM_BASE + 0x1240;
    const rxDesc = ESP32S3_DRAM_BASE + 0x1280;
    ...
    const c = core(
      assembleXtensa(
        ...
        [
          txDesc, ctBuf, rxDesc, ptBuf, txDw0, rxDw0, GDMA, outLinkStart, inLinkStart, AES, UART,
          0x3b79424c, 0x9c0dd436, 0xbace9e0e, 0xd4586a4f, // ciphertext block 1
          0x32b9ded5, 0x0ae3ba69, 0xd472e882, 0x67fb5052, // ciphertext block 2
          0x2b7e1516, 0x28aed2a6, 0xabf71588, 0x09cf4f3c, // key
          0x00010203, 0x04050607, 0x08090a0b, 0x0c0d0e0f, // IV
        ],
        [
  ...
  it('hashes a block through the SHA-DMA path (GDMA-fed)', () => {
    // The S3 SHA accelerator can take its (pre-padded) message over GDMA: a TX
    // descriptor feeds the blocks, the digest stays in the H registers. The guest
    // sets SHA_MODE (+0x00) = SHA-256, SHA_DMA_BLOCK_NUM (+0x0c) = 1, then SHA_DMA_START
    // (+0x1c). The padded "abc" block (0x61626380, 0×14, 0x18) hashes to NIST
    // ba7816bf…f20015ad. The guest emits digest word[0] and word[7] MSB-first.
    const SHA = 0x6003b000;
    const txDesc = ESP32S3_DRAM_BASE + 0x1200;
    ...
    const c = core(
      assembleXtensa(
        ...
        [
          ...
          S32I(3, 4, 48),
          S32I(3, 4, 52),
          S32I(3, 4, 56), // words 1..14 = 0
          L32R(3, 8),
          S32I(3, 4, 60), // word15 = 0x18 (bit length of "abc")

          L32R(6, 3), // GDMA
          MOVI(7, 7), // GDMA_PERI_SHA
          S32I(7, 6, 0xa8), // OUT_CH0_PERI_SEL = SHA
          L32R(7, 4),
          S32I(7, 6, 0x80), // OUT_CH0_LINK = start

          L32R(9, 5), // SHA base
          MOVI(4, 2),
          S32I(4, 9, 0x00), // SHA_MODE = SHA-256
          MOVI(4, 1),
  ...
  it('encrypts and authenticates AES-256-GCM through the AES-DMA path', () => {
    // AES-256 GCM (AES_MODE = 2 = 256-bit encrypt, AES_BLOCK_MODE = 6) over NIST GCM
    // Test Case 15: 256-bit all-zero key, 96-bit all-zero IV -> J0 = 0…0 || 0x00000001,
    // one all-zero plaintext block, no AAD. Independently confirmed via OpenSSL
    // (aes-256-gcm): ciphertext cea7403d 4d606b6e 074ec5d3 baf39d18, tag d0d1c8a7
    // 99996bf0 265b98b5 d48ab919. The guest emits CT word0, CT word3, tag word0,
    // tag word3 (first/last word of each), MSB-first.
    const AES = 0x6003a000;
    const txDesc = ESP32S3_DRAM_BASE + 0x1200;
    const ptBuf = ESP32S3_DRAM_BASE + 0x1240;
    const rxDesc = ESP32S3_DRAM_BASE + 0x1300;
    ...
    const c = core(
      assembleXtensa(
        ...
        [
          ...
          S32I(4, 9, 0x1c),
          MOVI(4, 2),
          S32I(4, 9, 0x40), // MODE = AES-256 encrypt
          MOVI(4, 0),
          S32I(4, 9, 0x70), // J0[0]
          S32I(4, 9, 0x74), // J0[1]
          S32I(4, 9, 0x78), // J0[2]
    ...
    c.step(900);
    expect([...c.drainUart()]).toEqual([
      0xce, 0xa7, 0x40, 0x3d, 0xba, 0xf3, 0x9d, 0x18, 0xd0, 0xd1, 0xc8, 0xa7, 0xd4, 0x8a, 0xb9, 0x19,
    ]);
  });
  ...
  it('authenticates AES-GCM additional data (AAD) through the AES-DMA path', () => {
    ...
    // ciphertext. Same key/IV/plaintext as Test Case 3 plus a 16-byte AAD block; the
    // ciphertext is unchanged (42831ec2…) but the tag now folds in the AAD:
    // e5d06dc2…88d624ee. The guest emits CT word[0] and tag word[0]/word[3] MSB-first.
    const AES = 0x6003a000;
    const txDesc = ESP32S3_DRAM_BASE + 0x1200;
    const msgBuf = ESP32S3_DRAM_BASE + 0x1240;
    const rxDesc = ESP32S3_DRAM_BASE + 0x1300;
    const ctBuf = ESP32S3_DRAM_BASE + 0x1340;
    const txDw0 = (GDMA_DESC_OWNER_DMA | GDMA_DESC_SUC_EOF | (80 << 12) | 80) >>> 0;
    const rxDw0 = (GDMA_DESC_OWNER_DMA | 64) >>> 0;
    const outLinkStart = ((txDesc & 0x000f_ffff) | GDMA_OUT_LINK_START) >>> 0;
    const inLinkStart = ((rxDesc & 0x000f_ffff) | GDMA_INLINK_AUTO_RET | GDMA_INLINK_START) >>> 0;
    const msgStores: (number | ReturnType<typeof L32R>)[] = [];
    for (let i = 0; i < 20; i++) {
      msgStores.push(L32R(3, 11 + i));
      msgStores.push(S32I(3, 4, i * 4));
    }
    const c = core(
      assembleXtensa(
        ESP32S3_IRAM_BASE,
        [
          txDesc, msgBuf, rxDesc, ctBuf, txDw0, rxDw0, GDMA, outLinkStart, inLinkStart, AES, UART,
          0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210, // AAD block (idx 11-14)
          0xd9313225, 0xf88406e5, 0xa55909c5, 0xaff5269a, 0x86a7a953, 0x1534f7da, 0x2e4c303d, 0x8a318a72,
          0x1c3c0c95, 0x95680953, 0x2fcf0e24, 0x49a6b525, 0xb16aedf5, 0xaa0de657, 0xba637b39, 0x1aafd255, // plaintext (idx 15-30)
          0xfeffe992, 0x8665731c, 0x6d6a8f94, 0x67308308, // key (idx 31-34)
          0xcafebabe, 0xfacedbad, 0xdecaf888, 0x00000001, // J0 (idx 35-38)
        ],
        [
          L32R(2, 0),
          L32R(3, 4),
          ...
          S32I(3, 2, 8),
          L32R(4, 1), // msgBuf
          ...msgStores, // AAD block + 16 plaintext words
          L32R(2, 2),
          L32R(3, 5),
          S32I(3, 2, 0),
          L32R(3, 3),
          ...
          S32I(4, 9, 0x7c),
          MOVI(4, 1),
          S32I(4, 9, 0xa0), // AAD_BLOCK_NUM = 1
          MOVI(4, 6),
          S32I(4, 9, 0x94), // BLOCK_MODE = GCM
          MOVI(4, 5),
          S32I(4, 9, 0x98), // BLOCK_NUM = 5 (1 AAD + 4 data)
          MOVI(4, 1),
          S32I(4, 9, 0x90), // DMA_ENABLE
          MOVI(4, 1),
          S32I(4, 9, 0x48), // TRIGGER
    ...
    c.step(900);
    expect([...c.drainUart()]).toEqual([0x42, 0x83, 0x1e, 0xc2, 0xe5, 0xd0, 0x6d, 0xc2, 0x88, 0xd6, 0x24, 0xee]);
  });
