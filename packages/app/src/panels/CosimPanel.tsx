...
import { addBinding, candidateNets, candidatePins, removeBinding, validateSpec } from '../cosim/bindings.js';
import { runCosim } from '../cosim/runner.js';
...
import type { CosimOutcome } from '../cosim/runner.js';
import type { CosimWindowSpec, PinBinding } from '../cosim/types.js';
...
/** Defaults: a 500 µs window solved at 1 µs steps. */
...
export function CosimPanel() {
  ...
  const onRun = async () => {
    ...
    setFormError(null);
    setBusy(true);
    const outcome = await runCosim(getGraph(useSession.getState()), partDb, spec);
  ...
  return (
    <div className="panel-body cosim-panel">
      <h3 className="panel-subtitle">Pin → net bindings</h3>
      {bindings.length === 0 ? (
