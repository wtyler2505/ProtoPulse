...
describe('DrcScriptEngine', () => {
  ...
  describe('run', () => {
    it('returns passed:true when script reports no violations', () => {
      const script = makeScript('/* no-op */');
      const result = engine.run(script, designData());
    ...
    it('collects violations from report()', () => {
      const script = makeScript('report("r1", "bad thing", "error", ["n1"]);');
      const result = engine.run(script, designData([node('n1')]));
    ...
    it('report() uses default severity error and empty nodeIds', () => {
    ...
    it('collects messages from info()', () => {
  ...
  describe('context API', () => {
    it('exposes nodes array to scripts', () => {
    ...
    it('exposes edges array to scripts', () => {
    ...
    it('exposes bomItems array to scripts', () => {
    ...
    it('getConnected returns connected node IDs', () => {
    ...
    it('getConnected returns empty array for unconnected node', () => {
      const script = makeScript(`
    ...
    it('getConnected includes bidirectional connections', () => {
      const script = makeScript(`
    ...
    it('hasProperty returns false for unknown node', () => {
      const script = makeScript(`
  ...
  describe('sandboxing', () => {
    ...
    it('prevents mutation of design data', () => {
      const script = makeScript(`
        try {
  ...
  describe('error handling', () => {
    it('catches runtime errors and reports as violation', () => {
  ...
  describe('max violations', () => {
    it('stops collecting violations after limit (500)', () => {
      const code = `
        for (var i = 0; i < 600; i++) {
  ...
  describe('edge cases', () => {
    ...
    it('handles script that does nothing', () => {
      const result = engine.run(makeScript(''), designData());
      expect(result.passed).toBe(true);
      expect(result.violations).toHaveLength(0);
    });
  });
  ...
  describe('runAll', () => {
    it('runs only enabled scripts', () => {
      ...
      const results = engine.runAll([s1, s2, s3], designData());
      expect(results).toHaveLength(2);
      expect(results[0].scriptId).toBe(s1.id);
      expect(results[1].scriptId).toBe(s3.id);
    });

    it('returns empty array when no scripts are enabled', () => {
      const s1 = makeScript('report("s1", "s1", "info");', { enabled: false });
      const results = engine.runAll([s1], designData());
...
describe('BUILTIN_TEMPLATES', () => {
  ...
  it('Trace Width Minimum flags narrow traces', () => {
  ...
  it('Component Spacing flags close components', () => {
  ...
  it('Net Naming Convention flags non-prefixed nets', () => {
  ...
  it('Power Rail Consistency flags mismatched voltages', () => {
  ...
  it('Unconnected Pin Detection flags floating nodes', () => {
  ...
  it('Duplicate Component Labels flags duplicates', () => {
...
describe('useDrcScripts', () => {
  ...
  it('addScript creates and persists a script', () => {
    ...
    act(() => {
      script = result.current.addScript('Hook Rule', 'desc', 'report("hr", "m");');
    });
  ...
  it('runScript executes a specific script and stores result', () => {
    const { result } = renderHook(() => useDrcScripts());
    let script: DrcScript;
    act(() => {
    ...
    act(() => {
      res = result.current.runScript(script!.id, designData([node('a')]));
    });
  ...
  it('runScript returns null for unknown script', () => {
    const { result } = renderHook(() => useDrcScripts());
    let res: DrcScriptResult | null = null;
    act(() => {
      res = result.current.runScript('nonexistent', designData());
    });
    expect(res).toBeNull();
  });

  it('runAllEnabled executes all enabled scripts', () => {
    const { result } = renderHook(() => useDrcScripts());
    act(() => {
    ...
    act(() => {
      allResults = result.current.runAllEnabled(designData());
    });
    expect(allResults).toHaveLength(1);
    expect(result.current.results).toHaveLength(1);
  });

  it('deleteScript clears associated results', () => {
    const { result } = renderHook(() => useDrcScripts());
    let script: DrcScript;
    act(() => {
    ...
    act(() => {
      result.current.runScript(script!.id, designData());
    });
    expect(result.current.results).toHaveLength(1);
    act(() => {
      result.current.deleteScript(script!.id);
