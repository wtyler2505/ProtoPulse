...
export default function CoachPanel() {
  ...
  return (
    <section
      ...
      className="mx-2.5 mb-1.5 rounded-sm border border-primary/15 bg-card/60 shadow-[0_2px_8px_rgba(0,0,0,0.12)] backdrop-blur-xl"
      aria-label="Coach panel"
    >
      <div className="flex items-start justify-between gap-2 border-b border-border/60 px-2 py-1.5">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <GraduationCap className="h-3.5 w-3.5 text-primary" />
            <p className="text-xs font-semibold text-foreground">Coach Panel</p>
          </div>
          <p data-testid="coach-role-description" className="mt-0.5 text-[11px] text-muted-foreground">
        ...
        <button
          ...
          className="rounded-sm p-0.5 text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground"
      ...
      {!collapsed && (
        <div className="max-h-[15rem] space-y-2 overflow-y-auto px-2 py-2">
          <div data-testid="coach-mode-section" className="space-y-1.5">
          ...
          <div
            ...
            className="rounded-sm border border-primary/15 bg-primary/5 p-1.5"
          >
          ...
          <div data-testid="coach-checklist-section" className="space-y-2">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">
                  Next Steps
              ...
              {checklist.dismissed ? (
                <button
                  ...
                  className="rounded-full border border-border bg-background/60 px-2 py-0.5 text-[10px] font-medium text-muted-foreground transition-colors hover:text-foreground"
                >
                  Show Again
                </button>
              ) : (
            ...
            {checklist.dismissed ? (
              <div
                ...
                className="rounded-md border border-dashed border-border/70 bg-background/40 px-2.5 py-2 text-[11px] leading-relaxed text-muted-foreground"
              >
                ...
                <ul className="space-y-0.5">
                  {checklist.items.map((item) => (
                    <li
                      ...
                      className={cn(
                        'flex items-start gap-2 rounded-sm px-2 py-1 text-xs transition-colors',
          ...
          <div data-testid="coach-actions" className="space-y-1">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">
              Helpful Shortcuts
            </p>
            <div className="grid grid-cols-1 gap-1">
              {COACH_ACTIONS.map(({ label, view, icon: Icon }) => (
                <button
                  ...
                  className="flex items-center gap-2 rounded-sm border border-border bg-background/60 px-2 py-1 text-left text-[11px] font-medium text-foreground transition-colors hover:border-primary/30 hover:bg-primary/5"
                >
          ...
          {checklist.completedCount === checklist.totalCount && (
            <div
              ...
              className="rounded-sm border border-emerald-500/25 bg-emerald-500/10 px-2 py-1.5"
            >
              <p className="text-xs font-medium text-emerald-300">You are ready for more advanced tools.</p>
