    ...
    <div className="h-14 border-b border-border bg-card/30 backdrop-blur flex items-center justify-between px-4 shrink-0">
      ...
      <div className="flex gap-1 items-center">
        <StyledTooltip content="Search chat" side="bottom">
            <button data-testid="chat-search-toggle" onClick={onSearch} aria-label="Search chat" className={cn("p-1.5 hover:bg-muted transition-colors", showSearch && "text-primary bg-primary/10")}>
              <Search className="w-4 h-4" />
            </button>
        </StyledTooltip>
        <StyledTooltip content="Export chat" side="bottom">
            <button data-testid="chat-export" onClick={onExport} aria-label="Export chat" className="p-1.5 hover:bg-muted transition-colors text-muted-foreground hover:text-foreground">
        ...
        <StyledTooltip content="Close (Esc)" side="bottom">
            <button data-testid="chat-close" aria-label="Close chat" className="p-1.5 hover:bg-muted text-muted-foreground hover:text-foreground transition-colors ml-1 md:hidden" onClick={onClose}>
              <X className="w-4 h-4" />
            </button>
        </StyledTooltip>
      </div>
