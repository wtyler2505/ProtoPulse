  ...
  const sortedBom = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      switch (sortField) {
        ...
        case 'manufacturer':
          return dir * a.manufacturer.localeCompare(b.manufacturer);
        case 'stock':
          return dir * (a.stock - b.stock);
        case 'quantity':
          return dir * (a.quantity - b.quantity);
        case 'unitPrice':
          return dir * (Number(a.unitPrice) - Number(b.unitPrice));
        case 'totalPrice':
          return dir * (Number(a.totalPrice) - Number(b.totalPrice));
        default:
          return 0;
      }
    });
        ...
        <thead className="bg-muted/50 text-muted-foreground font-medium uppercase text-[10px] tracking-wider">
          <tr>
            <th className="w-8 px-1 py-3">
              <StyledTooltip content="Drag to reorder" side="right">
                ...
                {virtualizer.getVirtualItems().map((virtualRow) => {
                  ...
                  return (
                    <SortableBomRow
                      ...
                      toast={toast}
                      highlighted={highlightedItemId === Number(item.id)}
                      onHighlight={handleHighlightItem}
                      onAssessDamage={onAssessDamage}
                      isDuplicate={duplicatePartNumbers.has(item.partNumber.toLowerCase().trim())}
                      onFindAlternates={onFindAlternates}
                      shortfall={shortfallsByPartNumber?.get(item.partNumber) ?? null}
                    />
                  );
                })}
              </tbody>
...
const SortableBomRow = memo(function SortableBomRow({
  item,
  editingId,
  editValues,
  setEditValues,
      ...
      <ContextMenuTrigger asChild>
        <tr
          ...
          className={cn(
            ...
            highlighted &&
              'ring-1 ring-inset ring-[var(--color-editor-accent)]/60 bg-[var(--color-editor-accent)]/5 animate-pulse',
        ...
        >
          <td className="w-8 px-1 py-3 text-center">
            {isEditing ? (
              <Pencil
                className="w-3.5 h-3.5 text-primary mx-auto animate-pulse"
                data-testid={`edit-indicator-${item.id}`}
              />
            ) : (
              <button
                type="button"
          ...
          <td className="px-4 py-3">
            <span
              className={cn(
                'inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium border uppercase tracking-wide',
          ...
          {isEditing ? (
            <>
              <td className="px-4 py-1">
                <input
                  data-testid={`edit-part-number-${item.id}`}
                  className="w-full bg-primary/5 border border-primary/30 px-2 py-1 text-xs font-mono focus-visible:outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-cyan-400/50"
                  value={editValues.partNumber}
                  onChange={(e) => setEditValues((v) => ({ ...v, partNumber: e.target.value }))}
              ...
              <td className="px-4 py-1">
                <input
                  ...
                  className="w-full bg-primary/5 border border-primary/30 px-2 py-1 text-xs focus-visible:outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-cyan-400/50"
              ...
              <td className="px-4 py-1">
                <NumberInput
                  ...
                  min={1}
                  max={999999}
                  className="w-16 bg-primary/5 border border-primary/30 px-2 py-1 text-xs font-mono text-right focus-visible:outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-cyan-400/50"
              ...
              <td className="px-4 py-1">
                <NumberInput
                  ...
                  max={99999.99}
                  step={0.01}
                  className="w-24 bg-primary/5 border border-primary/30 px-2 py-1 text-xs font-mono text-right focus-visible:outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-cyan-400/50"
            ...
            <>
              <td
                className="px-4 py-3 font-mono font-medium text-foreground text-xs"
                data-testid={`text-part-number-${item.id}`}
              >
                <span className="inline-flex items-center gap-1.5">
                  {item._isEsd && (
                    <StyledTooltip
                      content="ESD Sensitive — Handle with anti-static precautions. Use ESD wrist strap and grounded work surface."
                      side="right"
                    >
                  ...
                  {isDuplicate && (
                    <StyledTooltip content="Duplicate part number detected in BOM" side="right">
                      <span
                        className="inline-flex items-center px-1 py-0 text-[9px] font-medium bg-amber-500/10 text-amber-500 border border-amber-500/20 uppercase tracking-wider"
              ...
              <td className="px-4 py-3 text-muted-foreground" data-testid={`text-manufacturer-${item.id}`}>
                {item.manufacturer}
              </td>
              <td
              ...
              <td className="px-4 py-3 text-muted-foreground" data-testid={`text-supplier-${item.id}`}>
                {item.supplier}
              </td>
              <td className="px-4 py-3 text-right font-mono text-xs" data-testid={`text-stock-${item.id}`}>
                <span className="inline-flex items-center gap-1.5 justify-end">
                  ...
                  {shortfall && shortfall.shortfall > 0 && (
                    <StyledTooltip
                      content={`Need ${shortfall.shortfall} more (BOM wants ${shortfall.quantityNeeded}, on hand ${shortfall.quantityOnHand})`}
                      side="left"
                    >
                      <span
                        className="inline-flex items-center gap-0.5 px-1 py-0 text-[9px] font-medium bg-red-500/10 text-red-500 border border-red-500/20 uppercase tracking-wider"
                        data-testid={`shortfall-badge-${item.id}`}
                      >
                        <PackageX className="w-2.5 h-2.5" />-{shortfall.shortfall}
                      </span>
                    </StyledTooltip>
                  )}
                </span>
              </td>
              <td className="px-4 py-3 text-right font-mono text-xs" data-testid={`text-quantity-${item.id}`}>
                {item.quantity}
              </td>
              <td
                className="px-4 py-3 text-right font-mono text-xs text-muted-foreground"
                data-testid={`text-unit-price-${item.id}`}
              >
                ${(Math.round(Number(item.unitPrice) * 100) / 100).toFixed(2)}
              </td>
              <td
                className="px-4 py-3 text-right font-mono text-xs font-bold text-foreground"
              ...
              <td className="px-4 py-3 text-right flex gap-1">
                <StyledTooltip content="Edit item" side="left">
                  <button
                    ...
                    className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted/30 opacity-0 group-hover:opacity-100 transition-opacity"
                ...
                <StyledTooltip content="Assess damage" side="left">
                  <button
                    type="button"
                    aria-label="Assess damage"
                    className="p-1.5 text-amber-500 hover:bg-amber-500/10 opacity-0 group-hover:opacity-100 transition-opacity"
                    onClick={() => onAssessDamage(item)}
                    data-testid={`button-damage-${item.id}`}
                  >
                    <Shield className="w-4 h-4" />
                  </button>
                </StyledTooltip>
                {onFindAlternates && (
                  <StyledTooltip content="Find alternate parts" side="left">
                    <button
                      type="button"
                      aria-label="Find alternates"
                      className="p-1.5 text-primary hover:bg-primary/10 opacity-0 group-hover:opacity-100 transition-opacity"
                      onClick={() => onFindAlternates(item.partNumber)}
                      data-testid={`button-alternates-${item.id}`}
                    >
      ...
      <ContextMenuContent className="bg-card/90 backdrop-blur-xl border-border min-w-[180px]">
        <ContextMenuItem
          onSelect={() => {
            ...
            addOutputLog('[BOM] Copied details: ' + item.partNumber);
          }}
        >
          Copy Details
        </ContextMenuItem>
        ...
        <ContextMenuItem
          onSelect={() =>
            window.open(
              'https://www.google.com/search?q=' + encodeURIComponent(item.partNumber + ' alternative equivalent'),
              '_blank',
              'noopener,noreferrer',
            )
          }
        ...
        <ContextMenuItem
          ...
          onSelect={(e) => {
            e.preventDefault();
            if (window.confirm(`Remove "${item.partNumber}" from the BOM?`)) {
