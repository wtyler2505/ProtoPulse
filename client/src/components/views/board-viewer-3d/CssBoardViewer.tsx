/**
 * CssBoardViewer — the original CSS-3D-transform PCB board visualization.
 *
 * Extracted verbatim from the former `BoardViewer3DView.tsx` (PP3D-1) so the
 * latter can become a thin feature-flag switch between this CSS implementation
 * and the new WebGL one. Behavior and DOM/`data-testid` output are unchanged —
 * this is the default engine and the path the existing component tests exercise.
 *
 * Renders the board, components (with real footprint data), traces, vias,
 * camera-angle buttons, layer-visibility toggles, and editable dimensions
 * using CSS `transform-style: preserve-3d` rather than WebGL.
 */
...
export default function CssBoardViewer() {
  const {
    board,
    setBoard,
    components,
