...
import type { Measurement } from './RulerOverlay';
...
export default function ShapeCanvas({ view, drcViolations = [] }: ShapeCanvasProps) {
  ...
  const [editingNodeDrag, setEditingNodeDrag] = useState<{ nodeIndex: number; handle: 'point' | 'cp1' | 'cp2' } | null>(
    null,
  );
          ...
          dispatch({
            ...
            payload: {
              ...
              shape: {
                ...
                rotation: 0,
                text,
                style: { fill: '#ccc', fontSize: 14, fontFamily: 'sans-serif' },
              } as TextShape,
            },
          });
        }
  ...
  const handleShapeMouseDown = useCallback(
    (e: React.MouseEvent, shapeId: string) => {
      ...
      if (targetShape && isLayerLocked(targetShape.layer)) {
        return;
      }
      if (e.shiftKey) {
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (drawStart) {
        setDrawCurrent(toPartSpace(e.clientX, e.clientY));
        return;
      }
  ...
  const handleToggleNodeType = useCallback(
    (i: number) => {
      if (!editingPathNodes || !selectedPathShape) {
        return;
      }
  ...
  const handleSimplifyPath = useCallback(() => {
    if (!editingPathNodes || !selectedPathShape || editingPathNodes.length < 4) {
      return;
    }
  ...
  const preview = useMemo(
    () => buildDrawPreview(drawStart, drawCurrent, activeTool),
  ...
  const toolbar = useMemo(
    () => (
      <div
        className="flex items-center gap-1 px-2 py-1 border-b border-border bg-card"
      ...
      >
        ...
        {refImage && (
          <>
            ...
            <label
              className="flex items-center gap-1 text-xs text-muted-foreground ml-1"
              title="Reference image X offset"
            >
              <span>X</span>
              <NumberInput
                data-testid="ref-image-x"
            ...
            <button
              data-testid="button-ref-lock"
              title={refImage.locked ? 'Unlock reference image' : 'Lock reference image'}
              className={`p-1.5 rounded transition-colors ${
                refImage.locked
                  ? 'bg-primary/20 text-primary'
                  : 'text-muted-foreground hover:text-foreground hover:bg-muted'
            ...
            <button
              data-testid="button-ref-calibrate"
              title={isCalibrating ? 'Calibrating... click 2 points' : 'Calibrate reference image'}
              className={`p-1.5 rounded transition-colors ${
                isCalibrating
  ...
  return (
    <div className="flex flex-col flex-1 h-full relative" data-testid="shape-canvas-container">
      ...
      <svg
        ref={svgRef}
        data-testid="shape-canvas-svg"
        className="flex-1 w-full"
        role="application"
        aria-label={`Component editor canvas - ${view} view`}
      ...
      >
        ...
        <g transform={`translate(${panX}, ${panY}) scale(${zoom})`}>
          ...
          <RulerOverlay
            ...
            pendingStart={pendingMeasureStart}
            cursorPos={measureCursorPos}
            zoom={zoom}
          />
          {preview}
          {marqueeRect}
