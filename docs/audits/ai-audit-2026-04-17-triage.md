...
5. ~~**01-CORE-HIGH-05** Gemini fast/standard tiers identical~~ — **RESOLVED 2026-04-18**. `server/ai.ts:165-167` now maps fast→`gemini-2.5-flash-lite`, standard→`gemini-2.5-flash`, premium→`gemini-2.5-pro` (three distinct model IDs). Landed in commit 74b250ee. Regression test `server/__tests__/ai.test.ts` "routeToModel — Gemini tier distinctness" locks the invariant.
