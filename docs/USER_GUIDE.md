...
The editor has a full PCB mode (toggle in the toolbar): place footprints from the tray, route copper with the trace tool — in **manual** (hand-drawn), **walk** (auto-detour around copper at the rule deck's clearance), or **shove** mode (your trace goes straight; traces in the way are pushed aside, and they *spring back* to their original paths if you later delete yours) — drop vias, follow the ratsnest, and run DRC against the JLC rule deck. Gerber/Excellon/pick-and-place exports are byte-exact tested.

### What's coming

The new editor grows in stages. Still ahead: copper zones/pours, panelization, real-time collaboration (sync relay), the fuller AI design crew (Architect/Router/Buyer), WebSerial flashing of real boards, and community/fab features. The full current status always lives in [ROADMAP.md](../ROADMAP.md).
