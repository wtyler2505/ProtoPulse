...
- [ ] FG-12: Add design review and commenting system (threaded comments, @mentions, resolve/unresolve) | Effort: L | Priority: P1
- [ ] FG-13: Implement AC small-signal analysis in circuit solver | Effort: L | Priority: P1
...
- [ ] UI-36: Add theme customization beyond light/dark | Effort: M | Priority: P3
...
- [ ] TD-04: Decompose ShapeCanvas.tsx (CCN=381 aggregate, 1,275 lines, 6 functions >CCN 20) into: ShapeRenderer, HitTester, DragManager, SnapGuideEngine, PathEditor, CanvasTransforms | Effort: L | Priority: P0
...
- [ ] TD-25: Build unified undo/redo stack across all views | Effort: L | Priority: P2
...
- [ ] EN-08: Import IBIS/SPICE model files for simulation | Effort: M | Priority: P2
...
- [ ] IN-05: Intelligent component suggestion engine ("you need decoupling caps") | Effort: M | Priority: P1 | **Prerequisite: TD-09 (split ai-tools)**
- [ ] IN-06: One-click PCB ordering with DFM pre-check (JLCPCB API) | Effort: L | Priority: P1
- [ ] IN-07: Visual diff and design version history | Effort: M | Priority: P1 | **Prerequisite: TD-03 (migrations)**
- [ ] IN-08: Multi-model AI routing with design-phase awareness | Effort: L | Priority: P1 | **Prerequisite: TD-11 (prompt optimization)**
...
- [ ] IN-11: Component lifecycle and supply chain dashboard | Effort: L | Priority: P2
- [ ] IN-12: Multimodal AI input (image/photo to circuit) | Effort: M | Priority: P2
- [ ] IN-13: Interactive design tutorials and guided workflows | Effort: M | Priority: P2
...
- [ ] IN-18: AI-powered comprehensive design review | Effort: M | Priority: P2
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [ ] | CAPX-SEC-01 | Add project ownership model and enforce per-project authorization | NEW | P0 | L | Backend + DB | DB migration readiness | `projects` includes owner field; all `/api/projects/:id*` endpoints reject cross-owner access; authorization tests pass |
...
| [ ] | CAPX-TEST-01 | Add authorization and abuse-control regression test suite | NEW | P1 | M | Backend QA | CAPX-SEC-01, CAPX-REL-02 | Tests cover cross-user project access, admin purge auth, stream rate limits/origin protections |
...
| [ ] | CAPX-OPS-03 | Add backup/restore automation and documented recovery runbook (RPO/RTO) | NEW | P2 | M | DevOps + Backend | CAPX-DATA-01 | Automated backup schedule exists, restore drill succeeds, runbook is versioned |
...
| [ ] | CAPX-ARCH-02-EXP | Add transactions to `updateBomItem` and `updateComponentPart` to prevent race conditions | EXPANDED | P1 | M | Backend | CAPX-ARCH-02 | Concurrent update requests to same resource produce correct results; version increment is atomic; integration test with parallel writes passes |
...
| [ ] | CAPX-OBS-04 | Propagate X-Request-Id to client error context | NEW | P2 | S | Frontend + Backend | CAPX-ERR-11 | Client reads `X-Request-Id` from response headers; included in error messages, toast notifications, and Sentry breadcrumbs; users can reference in bug reports |
