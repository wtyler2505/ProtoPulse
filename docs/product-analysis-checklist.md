...
- [x] FG-01: ~~Implement production-quality PCB layout with copper routing, layer stack management, and trace editing~~ | Effort: XL | Priority: P0 | **DONE 2026-03-06 Waves 42-46 — 5-phase implementation: Phase 1 footprint library (27 packages, pad renderer, net-pad mapper), Phase 2 via model (4 types, IPC validation, SVG renderer, layer stack panel), Phase 3 interactive routing (45° trace router, trace editor, net classes Default/Power/High-Speed, PCB DRC spatial checker), Phase 4 autorouter (A* maze router, rip-up retry, server endpoint), Phase 5 integration (Gerber export to real data, copper pour bridge, 3D viewer real footprints/traces/vias). ~510 tests across 17 test files.**
- [x] FG-02: ~~Add multi-project support -- remove hardcoded PROJECT_ID=1; add project picker, project CRUD in UI~~ | Effort: M | Priority: P0 | **DONE 2026-03-04 — ProjectPickerPage.tsx (476 lines): project list/search/create/delete UI, last-project persistence via localStorage, project cards with metadata. App.tsx: wouter Route `/projects/:projectId` → ProjectWorkspace, `/` → ProjectPickerPage. ProjectProvider accepts `projectId` prop, uses ProjectIdContext (project-id-context.tsx). No hardcoded PROJECT_ID — fully dynamic via URL params.**
...
- [x] FG-04: ~~Build or integrate PCB autorouter (FreeRouting WASM, topological, or custom A* with DRC)~~ | Effort: XL | Priority: P0 | **DONE 2026-03-06 Wave 45 — MazeRouter class (maze-router.ts): A* on discretized grid (0.25mm default), 8-directional movement, obstacle inflation by clearance, via cost for layer changes, path simplification, progressive blocking. Rip-up retry: net criticality ordering, max 3 iterations per failed net. Server endpoint upgraded: POST /api/circuits/:designId/autoroute fetches real circuit data, Manhattan routing with overlap avoidance, saves wires to storage. 93 tests (42 maze-router + 21 rip-up + 30 server).**
- [x] FG-05: ~~Expand built-in component library to 10K+ parts with symbols, footprints, 3D models~~ | Effort: L | Priority: P0 | **DONE 2026-03-04 Wave 39 — `shared/standard-library.ts`: 100 standard components across 14 categories (Logic ICs, Resistors, Capacitors, Inductors, MCUs, Power ICs, Op-Amps, Transistors, Diodes, LEDs, Connectors, Displays, Sensors, Comm). Auto-seeds on startup via `server/index.ts` seedStandardLibrary(). `POST /api/admin/seed-library`. 39 tests.**
...
- [x] FG-07: ~~Connect Gerber export to actual PCB layout data~~ | Effort: L | Priority: P1 | **DONE 2026-03-06 Wave 46 — gerber-generator.ts upgraded: GerberVia type, buildGerberInput() bridge from DB types, via pad flashing on F.Cu/B.Cu, soldermask openings for non-tented vias, drill file from vias + THT pads, backward compatible. 47 tests.**
...
- [x] FG-10: ~~Add one-click PCB ordering (JLCPCB, PCBWay, OSHPark APIs)~~ | Effort: M | Priority: P1 | **DONE 2026-03-06 Wave 47 — Server-side PCB ordering infrastructure: pcb_orders table (14 columns, 2 indexes), OrderingStorage class with LRU cache, 6 REST endpoints (list/get/create/update/submit/delete orders), Zod validation. Client-side engine already existed (Wave 35 IN-06: 5 fab profiles, DFM validation, pricing, 5-step UI workflow). 39 tests.**
- [x] FG-11: ~~Implement push-and-shove interactive routing for PCB layout~~ | Effort: XL | Priority: P1 | Prerequisite: FG-01 | **DONE 2026-03-06 Wave 48 — push-shove-engine.ts (~800 lines): PushShoveEngine class with grid-based spatial index, swept rectangle collision detection, segment push maintaining 45-degree angles, cascade push with cycle detection (max depth 10), via displacement with connected segment updates, spring relaxation to minimize total displacement, snapshot-based undo stack, solve() for full push-and-shove resolution. 66 tests.**
...
- [x] FG-16: ~~Add differential pair routing support in PCB layout~~ | Effort: L | Priority: P2 | Prerequisite: FG-01, FG-11 | **DONE 2026-03-06 Wave 49 — 3 modules: diff-pair-router.ts (centerline-to-pair geometry, miter join offsets, via pair generation, path length/skew calculation), diff-pair-manager.ts (pair assignments, 7 protocol presets USB2/USB3/HDMI/LVDS/PCIe/Ethernet/Custom with impedance/width/gap/skew targets, 4 DRC violation types gap/width/skew/uncoupled, singleton+subscribe, JSON serialization), diff-pair-meander.ts (trombone + sawtooth serpentine meander generator, coordinate transform routing, fitMeander auto-insertion into longest segment). 96 tests.**
- [x] FG-17: ~~Implement multi-layer PCB support (32+ layers)~~ | Effort: L | Priority: P2 | **DONE 2026-03-06 Wave 47 — layer-utils.ts (standard layer names F.Cu/In1-30.Cu/B.Cu, legacy normalization, index mapping, HSL color generation, getCopperLayers), ActiveLayer widened from 'front'\|'back' to string, maze-router N-layer grids (blockedLayers[] + netOccLayers[]), trace-router layer-index support, via-model multi-layer validation (blind/buried/micro span checks, getOppositeLayer), board-stackup 8/10/16/32-layer presets, LayerStackPanel any-copper-layer selection, gerber-generator inner copper layer export (In1_Cu.gbr...), PCB DRC layer-aware checks. 89 tests (45 layer-utils + 44 multi-layer-pcb).**
- [x] FG-18: ~~Add ECAD-MCAD integration (STEP export)~~ | Effort: L | Priority: P2 | **DONE 2026-03-06 Wave 47 — step-generator.ts (~1200 lines): ISO-10303-21 AP214 STEP file generation, rectangular box solids (8 vertices, 12 edges, 6 faces), cylinder solids for vias, component placement with rotation + back-side flipping, board outline with layer stack, assembly hierarchy with product definitions. POST /api/projects/:id/export/step route. ExportPanel "3D & CAD" category. 65 tests.**
...
- [x] FG-20: ~~Add signal integrity analysis (impedance, crosstalk, eye diagrams)~~ | Effort: XL | Priority: P2 | Prerequisite: FG-01, FG-17 | **DONE 2026-03-06 Wave 48 — 4 modules: transmission-line.ts (microstrip/stripline Z0, skin effect loss, dielectric loss, insertion/return loss, differential impedance), crosstalk-solver.ts (mutual C/L extraction, NEXT/FEXT coupling, guard trace reduction, critical length), eye-diagram.ts (PRBS-7 generator, NRZ waveform, channel impulse response, ISI convolution, jitter injection, eye overlay, eye height/width/opening measurement, bathtub curve), si-advisor.ts (termination recommendation, impedance match check, trace width suggestion, length match analysis, SI report generation). 137 tests.**
...
- [x] FG-23: ~~Implement nonlinear device models (diodes, BJTs, MOSFETs) in circuit solver~~ | Effort: XL | Priority: P2 | **DONE 2026-03-03 — client/src/lib/simulation/device-models.ts (589 lines): Diode (Shockley equation, Is/n/T/Vbr/Rs parameters, derivative for NR), BJT (Ebers-Moll, npn/pnp, betaF/betaR/Vaf, cutoff/active/saturation regions, gm/gpi/go extraction), MOSFET (Level 1 Shichman-Hodges, nmos/pmos, Vth/Kp/lambda/W/L, cutoff/linear/saturation, gm/gds). Newton-Raphson companion model solver for nonlinear circuits. 81 tests.**
...
- [x] FG-28: ~~Add rigid-flex PCB design support~~ | Effort: XL | Priority: P3 | **DONE 2026-03-06 Wave 48 — flex-zone-manager.ts (~600 lines): FlexZoneManager class with polygon-based flex zone model, 4 flex material properties (Kapton HN/FN, coverlay, adhesive with er/tanD/elongation/modulus/bendCycles), IPC-2223 bend radius validation (6x static, 12x dynamic), 5 flex-specific DRC rules (via exclusion, solid copper restriction, component height limits, trace direction checks, transition stress), point-in-polygon ray-casting, zone CRUD with singleton+subscribe pattern, JSON export/import. 79 tests.**
...
- [x] FG-35: ~~Implement copper pour / zone fill~~ | Effort: L | Priority: P3 | **DONE 2026-03-03 — client/src/lib/copper-pour.ts: CopperPourEngine singleton with zone CRUD, obstacle management (pad/trace/via), fill computation (polygon subtraction, clearance expansion, thermal relief spokes), geometry helpers (shoelace area, ray-casting point-in-polygon, polygon offset, circle subtraction), hatched fill, priority-based filling, keepout zones, conflict detection, JSON export/import, localStorage persistence, useCopperPour hook; 78 tests**
...
- [x] UI-11: ~~Add DRC constraint visualization during PCB trace routing~~ | Effort: L | Priority: P1 | **DONE 2026-03-04 Wave 40 — `client/src/components/views/pcb-layout/DrcConstraintOverlay.tsx` (290 lines): SVG overlay layer with clearance rings (amber dashed, 0.15 fill), violation highlights (red pulsing), net highlighting for selected component (neon cyan), board edge clearance zone. `DrcConstraintToggle.tsx` (50 lines): Eye/EyeOff toolbar button. Integrated into PCBBoardRenderer.tsx. 40 tests.**
...
- [x] UI-29: ~~Add interactive tutorial / guided walkthrough per view~~ | Effort: L | Priority: P3 | **DONE 2026-03-03 — client/src/lib/tutorial-system.ts: TutorialSystem singleton with 5 built-in tutorials (Welcome, Architecture, BOM, Simulation, Export), step navigation (next/prev/complete/skip), progress tracking with completion percentage, prerequisites, recommendations, event history, JSON export/import, localStorage persistence, useTutorialSystem hook; 120 tests**
...
- [x] TD-02: ~~Remove PROJECT_ID=1 hardcoding; implement multi-project routing, context switching, data isolation~~ | Effort: L | Priority: P0 | **DONE 2026-03-04 — ProjectProvider takes `projectId: number` prop; `useProjectId()` hook via ProjectIdContext; ProjectWorkspace reads `params.projectId` from wouter. Zero hardcoded project IDs remain.**
...
- [x] TD-07: ~~Split ProjectProvider monolithic context into domain-specific contexts with memoized selectors~~ | Effort: L | Priority: P1 | **DONE 2026-03-03 — project-context.tsx refactored into 8 domain-specific context providers: ProjectIdProvider, OutputProvider, ChatProvider, HistoryProvider, BomProvider, ValidationProvider, ProjectMetaProvider, ArchitectureProvider. Each in client/src/lib/contexts/ with own state + hooks. SeededProviders nests all providers. ArchitectureBridge passes setActiveView cross-context. All consumers migrated to domain hooks (useProjectId, useProjectMeta, useBom, useChat, useHistory, useValidation, useArchitecture, useOutput). Deprecated useProject() re-composition hook retained for backward compat with zero consumers.**
...
- [x] TD-27: ~~Update major outdated dependencies: drizzle-orm 0.39->0.45, date-fns 3->4~~ | Effort: M | Priority: P2 | **DONE 2026-03-04 Wave 39 (partial) — date-fns 3.6→4.1.0 updated (no API breakage). drizzle-kit 0.31.4→0.31.9 updated. drizzle-orm/drizzle-zod pinned at 0.39.3/0.7.0 — upgrade to 0.45 blocked by Zod v3→v4 migration (drizzle-zod 0.8.x requires Zod v4 internals). Tracked as future major migration.**

### P3 -- Nice-to-Have (Tech Debt)

- [x] TD-28: ~~Reduce overall file count >500 lines -- target max 800 lines via decomposition~~ | Effort: L | Priority: P3 | **DONE 2026-03-05 Wave 40 — storage.ts 1915→162 lines (13 domain modules in server/storage/), ProcurementView.tsx 1713→335 lines (15 sub-components in client/src/components/views/procurement/)**
...
- [x] EN-02: ~~Build unified undo/redo stack across all views (currently fragmented per-view)~~ | Effort: L | Priority: P1 | **DONE 2026-03-03 — See TD-25. client/src/lib/undo-redo.ts: UndoRedoStack class (command pattern, max 50). undo-redo-commands.ts: 8 command factories. undo-redo-context.tsx: UndoRedoProvider + useUndoRedo() hook with Ctrl+Z/Ctrl+Shift+Z. 36 tests.**
...
- [x] EN-04: ~~Implement actual PCB auto-router (currently stub AI tool with no routing engine)~~ | Effort: XL | Priority: P1 | **DONE 2026-03-06 Wave 45 — See FG-04. MazeRouter A* engine + rip-up retry + server endpoint upgrade. 93 tests.**
...
- [x] EN-06: ~~Add interactive manual PCB trace routing (currently AI-only)~~ | Effort: L | Priority: P1 | **DONE 2026-03-06 Wave 44 — TraceRouter class (trace-router.ts ~440 lines): 45° angle snapping, via insertion mid-route with layer switching, real-time DRC preview, state machine (IDLE→ROUTING→IDLE), grid snapping, multi-segment traces. TraceEditor class (~140 lines): segment hit testing, vertex/segment move, segment delete with trace splitting, net selection. 104 tests.**
...
- [x] EN-13: ~~Add E2E tests for critical user flows~~ | Effort: L | Priority: P1 | **DONE 2026-03-04 Wave 39 — Playwright setup: `playwright.config.ts` (Chromium, port 5000, webServer config, screenshot-on-failure). `e2e/` directory: project-picker.spec.ts, project-workspace.spec.ts, navigation.spec.ts, accessibility.spec.ts. `npm run test:e2e` / `test:e2e:ui` scripts. Covers project list, creation dialog, workspace layout, sidebar nav, chat panel presence, accessibility structure.**
...
- [x] EN-16: ~~Add component tests for high-complexity UI: PCBLayoutView, ShapeCanvas, BreadboardView~~ | Effort: L | Priority: P1 | **DONE 2026-03-03 — 162 new component tests: pcb-modules.test.ts (72 tests for 7 PCB layout modules), shape-modules.test.ts (72 tests for ShapeCanvas modules: primitives, toolbar, snap engine, connector rendering, grid, inspector, shape interactions), BreadboardView.test.tsx (18 tests covering rendering, toolbar, keyboard shortcuts, wires, empty/loading states).**
...
- [x] EN-22: ~~Automated alternate part cross-referencing with real equivalence databases~~ | Effort: L | Priority: P2 | **DONE 2026-03-03 — client/src/lib/alternate-parts.ts: AlternatePartsEngine singleton with part/rule/equivalence databases, parametric matching engine (exact/functional/pin-compatible/similar/upgrade levels), confidence scoring, 5 built-in cross-ref rules (resistor/capacitor/LED/regulator/op-amp), 30 sample parts, 15 explicit equivalences (LM7805↔L7805CV, NE555↔LM555), batch BOM search, search history, JSON export/import, localStorage persistence, useAlternateParts hook; 102 tests**
...
- [x] IN-01: ~~AI Design Agent -- natural language to complete circuit (chain architecture, BOM, validation, DFM)~~ | Effort: XL | Priority: P0 | **DONE 2026-03-04 Wave 40 — `server/routes/agent.ts` (230 lines): POST /api/projects/:id/agent with SSE streaming agentic loop, rate limit 2/min/IP, Anthropic multi-turn tool-use, up to 15 steps, terminates on no-tool response. `DesignAgentPanel.tsx` (230 lines): textarea + maxSteps slider + live step log + complete/error states. Chat panel gains Chat/Design Agent tab switcher. 26 server tests.**
...
| [x] | CAPX-WF-02 | ~~Remove hardcoded `/projects/1` redirects and add project selection entry flow~~ | EXPANDED | P1 | M | Frontend | CAPX-SEC-01 | ~~Root route opens project picker/last-project; invalid IDs no longer silently redirect to project 1~~ **DONE 2026-03-04 — ProjectPickerPage.tsx: project grid with search/filter, create new project, last-visited auto-redirect via localStorage, empty state; App.tsx: root `/` renders ProjectPickerPage instead of redirect to /projects/1; 68 tests** |
...
| [x] | CAPX-ARCH-02 | ~~Implement optimistic concurrency controls for mutable APIs~~ | NEW | P1 | M | Backend + API | CAPX-SEC-01 | ~~Conflicting writes return 409 with retry guidance; concurrency tests pass~~ **DONE 2026-03-03 — shared/schema.ts: version integer columns (default 1) on projects, architecture_nodes, architecture_edges, bom_items, circuit_designs. server/storage.ts: VersionConflictError class (httpStatus=409), all 5 update methods accept optional expectedVersion with WHERE version=N, auto-increment on success, follow-up SELECT to distinguish stale-version (409) from not-found (undefined). server/routes/architecture.ts, bom.ts, projects.ts, circuit-routes/designs.ts: ETag header on responses, If-Match header parsing, 409 Conflict responses. Fully backward-compatible (omit If-Match = skip check). 42 tests in server/__tests__/optimistic-concurrency.test.ts.** |
...
| [x] | CAPX-ARCH-03 | ~~Introduce async job queue/worker model for long-running AI/export tasks~~ | NEW | P2 | L | Backend + Platform | CAPX-REL-02 | **DONE 2026-03-04 — server/job-queue.ts: In-process async job queue with priority scheduling, configurable concurrency (default 3), retry with exponential backoff, AbortController cancellation, progress reporting, TTL-based cleanup. 5 job types (ai_analysis/export_generation/batch_drc/report_generation/import_processing). server/routes/jobs.ts: 5 API routes (POST/GET/GET:id/cancel/DELETE). 63 tests.** |
...
| [x] | CAPX-FFI-02 | ~~Web Serial API hardware communication — connect to Arduino/ESP32/serial devices directly from browser, send/receive data, serial monitor~~ | circuitmind-ai + multi-controller-app | P0 | L | None | **DONE 2026-03-04 — client/src/lib/web-serial.ts: WebSerialManager singleton with connection state machine, auto-reconnect with exponential backoff (1s→30s max), DTR/RTS signal control, 15 known board USB VID/PID filters (Arduino/ESP32/STM32/RPi Pico/Adafruit/SparkFun/Teensy), serial monitor buffer, connection profiles, localStorage persistence, useWebSerial() hook. 84 tests.** |
...
| [x] | CAPX-FFI-06 | ~~Transient / time-domain simulation — step response, switching waveforms, startup behavior~~ | Wild | P0 | L | CAPX-FFI-05, FG-23 | **DONE 2026-03-03 — client/src/lib/simulation/transient-analysis.ts (~720 lines): Full transient engine with MNA solver, Backward Euler + Trapezoidal integration, reactive companion models (C/L), time-varying sources (DC/Pulse/Sine/PWL), Newton-Raphson nonlinear support (diode/BJT/MOSFET via device-models.ts companion models), adaptive timestep with LTE estimation and breakpoint handling. 62 tests (RC/RL/RLC circuits, source types, integration comparison, diode rectifier, adaptive step, edge cases).** |
| [x] | CAPX-FFI-07 | ~~Camera/vision-based component identification — snap a photo, AI identifies component (markings, package type, color bands), adds to BOM~~ | PartScout + Partsnap | P0 | L | None | **DONE 2026-03-03 — server/ai-tools/vision.ts: registerVisionTools() with identify_component_from_image tool (base64 image + context, 10-point structured analysis). client/src/components/panels/CameraComponentId.tsx: camera capture UI with state machine (idle→streaming→captured→identifying→result), getUserMedia, canvas capture, permission handling, results display with confidence badges, "Add to BOM" action. AI tool count: 84. 25 tests.** |

### P1 — High Impact for Makers

| Done | ID | Label | Source | Priority | Effort | Dependencies | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| [x] | CAPX-FFI-08 | ~~Component pinout hover reference — hover any component to see pinout diagram and key datasheet specs without leaving the tool~~ | Wild | P1 | M | FG-05 (component library) | **DONE 2026-03-03 — client/src/lib/pinout-data.ts: 13 built-in component pinouts (ATmega328P, ESP32, NE555, LM7805, LM317, LM358, 2N2222, IRF540N, ATtiny85, L293D, 74HC595, Arduino Uno/Mega). lookupPinout() fuzzy match, getGenericPinout() fallback, searchPinouts(). PinoutHoverCard.tsx: SVG pin diagram (DIP/TO-220/TO-92/module renderers), color-coded pin table (power/ground/io/analog/special), key specs, datasheet link, escape/click-outside dismiss. 94 tests.** |
| [x] | CAPX-FFI-09 | ~~Design pattern library — documented, reusable circuit patterns with explanations~~ | Wild | P1 | L | None | **DONE 2026-03-03 — 10 educational circuit design patterns (decoupling network, voltage divider, pull-up resistor, H-bridge, voltage regulator, RC filter, LED driver, level shifter, USB-C power, crystal oscillator). client/src/lib/design-patterns/: types.ts + 10 pattern files + index.ts barrel with getAllPatterns(), getPatternsByCategory(), searchPatterns(). DesignPatternsView.tsx: card grid grouped by category, difficulty badges, expandable detail panels with whyItWorks, components, tips, common mistakes. Search/filter by category + difficulty + keyword. Tests validate all patterns.** |
| [x] | CAPX-FFI-10 | ~~Voice AI interface — live audio interaction with AI assistant using WebSocket streaming + PCM audio encoding/decoding~~ | circuitmind-ai + OmniTrek | P1 | L | None | ~~Hands-free while soldering/wiring. Use Gemini Live or similar real-time voice API.~~ **DONE 2026-03-04 — VoiceAIManager singleton (voice-ai.ts): audio capture via getUserMedia + AudioContext, PCM encoding Float32→Int16, energy-based VAD with configurable threshold/hold time, push-to-talk + hands-free modes, state machine (idle→requesting→ready→listening→processing); useVoiceAI hook; VoiceInput component with mic button + audio level visualization + mode toggle; 80+ tests** |
...
| [x] | CAPX-FFI-12 | ~~Automatic datasheet lookup — AI finds and links datasheets, manufacturer pages, and distributor links for components~~ | PartScout | P1 | M | None | **DONE 2026-03-03 — shared/schema.ts: datasheetUrl + manufacturerUrl columns on bom_items. server/ai-tools/bom.ts: lookup_datasheet AI tool (reads BOM item, returns structured search data for AI to suggest URLs). server/routes/bom.ts: POST lookup-datasheet endpoint. client/src/components/ui/DatasheetLink.tsx: external link component with "Find" button for missing datasheets. Tests for component rendering.** |
| [x] | CAPX-FFI-13 | ~~Physical storage location tracking — record where each part lives in your workshop~~ | PartScout | P1 | M | None | **DONE 2026-03-03 — shared/schema.ts: storageLocation, quantityOnHand, minimumStock columns on bom_items. server/storage.ts: getLowStockItems() + getStorageLocations() methods. server/routes/bom.ts: GET low-stock + storage-locations endpoints. StorageManagerPanel.tsx: grouped by location, stock warnings (green/yellow/red badges), search/filter. Integrated as 'Inventory' tab in ProjectWorkspace.** |
| [x] | CAPX-FFI-14 | ~~Minimum stock alerts — quantity tracking with configurable minimum thresholds and notification when stock drops below~~ | PartScout | P1 | S | CAPX-FFI-13 | **DONE 2026-03-03 — Built into CAPX-FFI-13 implementation. quantityOnHand + minimumStock columns. getLowStockItems() returns items where qty <= min. StorageManagerPanel shows color-coded badges: green (OK, qty > 2x min), yellow (low, qty > min but < 2x), red (critical, qty <= min), gray (not tracked).** |
| [x] | CAPX-FFI-15 | ~~RAG-enhanced AI context — retrieval-augmented generation from datasheets, documentation, and electronics knowledge bases~~ | circuitmind-ai | P1 | L | None | ~~AI assistant that can actually read and reference datasheets. Dramatically improves answer quality for specific component questions.~~ **DONE 2026-03-04 — RAGEngine (rag-engine.ts): TF-IDF indexing + cosine similarity, text chunking with overlap, stop word removal, relevance ranking with score threshold, localStorage persistence; 24 built-in knowledge entries (ATmega328P, ESP32, NE555, LM7805, Arduino, motor drivers, op-amps, etc.); useRAG hook; server routes POST/GET/DELETE /api/rag/documents; 124 tests** |
...
| [x] | CAPX-FFI-17 | ~~Design parameterization / variables — define parameters as variables (VOUT=3.3V) and have component values auto-calculate; change VOUT to 5V, watch resistors update~~ | Wild | P1 | L | None | ~~Powerful learning tool: "what happens if I change the voltage?" Also useful for design iteration.~~ **DONE 2026-03-03 — shared/design-variables.ts (1013 lines): Full expression parser (recursive descent), SI prefix parsing (p/n/u/m/k/M/G), math functions (sqrt/log/exp/sin/cos/abs/min/max), variable references, dependency graph with topological sort, circular dependency detection, VariableStore class. 45+ tests in shared/__tests__/design-variables.test.ts.** |
| [x] | CAPX-FFI-18 | ~~AI real-time design validation (Design Gateway)~~ | Wild/CR-8000 | P1 | L | None | **DONE 2026-03-03 — client/src/lib/design-gateway.ts: DesignGateway singleton with 12 proactive validation rules (decoupling cap proximity, power rail naming, floating pins, bus width mismatch, clock fanout, thermal pad, etc.). Singleton+Subscribe+Hook pattern. Validates on node/edge changes, returns severity-ranked issues. 63 tests.** |
| [x] | CAPX-FFI-19 | ~~Arduino/firmware code generation improvements — board-specific templates, pin mapping from schematic~~ | OmniTrek | P1 | M | IN-14 (firmware scaffold) | **DONE 2026-03-03 — server/export/firmware-templates/: BoardTemplate type system (PinDefinition, BusDefinition, PinCapability). 4 board templates: ESP32 DevKit (30+ GPIO with ADC/DAC/touch/PWM), Arduino Mega (70+ pins, 4 UARTs), Arduino Uno (20 pins), STM32 Blue Pill (37 pins). index.ts: detectBoard() fuzzy matcher, getPinsByCapability(), getBusesByType(). firmware-scaffold-generator.ts: integrated board detection, generates board-specific #define macros and library suggestions. Tests validate all boards.** |
...
| Done | ID | Label | Source | Priority | Effort | Dependencies | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| [x] | CAPX-FFI-21 | ~~ESD sensitivity flagging — mark static-sensitive components with handling warnings and ESD-safe workflow guidance~~ | PartScout | P2 | S | None | ~~Safety feature for beginners who don't know about ESD. Flag ICs, MOSFETs, etc.~~ **DONE 2026-03-03 — shared/schema.ts: esdSensitive boolean column on bom_items. ProcurementView.tsx: Auto-detect ESD-sensitive components from description (ICs, MOSFETs, op-amps, FPGAs, microcontrollers, CMOS, JFET). Zap icon (⚡) ESD warning badges with tooltip. ESD filter toggle in toolbar. 84 tests in server/__tests__/bom-assembly.test.ts.** |
| [x] | CAPX-FFI-22 | ~~Barcode/QR code scanning~~ | PartScout | P2 | M | CAPX-FFI-07 (camera) | **DONE 2026-03-03 — client/src/lib/barcode-scanner.ts: BarcodeScanner class with algorithmic EAN-13, UPC-A, Code128 decoders (no external deps). ProtoPulse QR label parsing (PP: prefix + base64url JSON). ScanSession with configurable dedup window. useBarcodeScanner React hook. 72 tests.** |
| [x] | CAPX-FFI-23 | ~~QR code label generation~~ | Partsnap | P2 | S | CAPX-FFI-13 | **DONE 2026-03-03 — client/src/lib/qr-labels.ts: QR label data encoding (JSON→base64 URL-safe), SVG label generation with part info/location/QR placeholder, multi-label print page layout (A4/Letter, 2×4 grid), decode/validate round-trip. 36 tests.** |
| [x] | CAPX-FFI-24 | ~~Component damage assessment~~ | PartScout | P2 | M | CAPX-FFI-07 (camera) | **DONE 2026-03-03 — client/src/lib/damage-assessment.ts: DamageAssessor class with 6 damage categories, 12 component types with custom indicator checklists and weighted scoring. Grade system (A-F), usability threshold, actionable recommendations. Assessment history with localStorage persistence. useDamageAssessment hook. 76 tests.** |
...
| [x] | CAPX-FFI-26 | ~~In-app Kanban task board~~ | OmniTrek | P2 | M | None | **DONE 2026-03-03 — client/src/lib/kanban-board.ts: KanbanBoard singleton with 4 default columns (backlog/todo/in-progress/done), custom column management, task CRUD with priority/tags/assignee/dueDate, architecture node linking, column filtering, localStorage persistence, JSON import/export. useKanbanBoard hook. 78 tests.** |
...
| [x] | CAPX-FFI-29 | ~~Assembly cost estimator — before finalizing, see board cost + component cost + assembly cost per unit at various quantities~~ | Wild | P2 | M | FG-15 (distributor APIs) | **DONE 2026-03-04 — client/src/lib/assembly-cost-estimator.ts: AssemblyCostEstimator with quantity tiers, PCB fab + component + assembly labor costs, 3 assembly house profiles (JLCPCB/PCBWay/manual-DIY), NRE tracking, cost optimization suggestions, margin calculator, currency support. 127 tests.** |
...
| [x] | CAPX-FFI-31 | ~~LCSC/JLCPCB part number mapping — auto-map BOM to distributor-specific part numbers for assembly ordering~~ | Wild | P2 | S | FG-15 (distributor APIs) | **DONE 2026-03-04 — client/src/lib/lcsc-part-mapper.ts: LcscPartMapper with 154 built-in component mappings, fuzzy matching (MPN/value+package/description), SI prefix parser, JLCPCB basic/extended classification, BOM CSV and CPL CSV export for JLCPCB assembly. 109 tests.** |
| [x] | CAPX-FFI-32 | ~~Embeddable schematic viewer — generate embed link/iframe for interactive schematics in blogs, docs, forums (like CodePen for circuits)~~ | Wild | P2 | M | None | ~~Sharing and education. "Here's my circuit, embedded in my blog post."~~ **DONE 2026-03-04 — EmbedManager (embed-viewer.ts): circuit data serialization + pako compression + base64url encoding, short URL support via server API, theme parsing (dark/light/accent/grid/labels); EmbedViewerPage: read-only ReactFlow viewer with watermark + "Open in ProtoPulse" link; EmbedDialog for sharing; server routes POST/GET /api/embeds; App.tsx: /embed/:data and /embed/s/:code routes outside AuthGate; 19+ tests** |
...
| [x] | CAPX-FFI-38 | ~~Standards compliance checking — IEC/ISO/DIN standards enforcement beyond manufacturer-specific DRC (auto/medical/aerospace domains)~~ | circuitmind-ai | P2 | L | None | **DONE 2026-03-04 — client/src/lib/standards-compliance.ts: StandardsComplianceEngine with 27 rules across 6 domains (Automotive AEC-Q100/Q101/Q200, Medical IEC 60601, Industrial IEC 61131, Consumer IEC 62368, Aerospace DO-254/MIL-STD, General EMC IEC 61000). Derating calculator, clearance/creepage calc per IEC 60664-1, multi-domain checking, compliance report generation. 120 tests.** |
| [x] | CAPX-FFI-39 | ~~Custom DRC rule scripting~~ | Wild | P2 | M | None | **DONE 2026-03-03 — client/src/lib/drc-scripting.ts: DrcScriptEngine with Function constructor sandbox (blocks 30+ dangerous globals). Script context API (report/warn/info/getConnected/hasProperty). 6 built-in templates (trace width, spacing, net naming, power rail, unconnected pins, duplicate labels). ScriptLibrary with localStorage. useDrcScripts hook. 60 tests.** |
| [x] | CAPX-FFI-40 | ~~Customizable keyboard bindings — configurable shortcut system, vim-like bindings for power users~~ | Wild | P2 | M | None | ~~Frequently requested by power users on HN/Reddit. Extends existing Ctrl+K command palette.~~ **DONE 2026-03-03 — client/src/lib/keyboard-shortcuts.ts (376 lines): KeyboardShortcutManager class with 19 default shortcuts across 4 categories (edit/view/tools/navigation), custom override support, conflict detection, localStorage persistence, platform-aware (Ctrl vs Cmd), comboToString/matchesEvent. useKeyboardShortcuts() React hook with input field suppression. 25+ tests in keyboard-shortcuts.test.ts.** |
...
| [x] | CAPX-FFI-44 | ~~In-app electronics knowledge hub~~ | OmniTrek | P2 | M | None | **DONE 2026-03-03 — client/src/lib/electronics-knowledge.ts: ElectronicsKnowledgeBase with 20 built-in articles (passive/active components, power, communication, PCB, techniques). Weighted full-text search. Contextual component lookup. Category/difficulty filtering. useKnowledgeBase hook. 74 tests.** |
...
| [x] | CAPX-FFI-46 | ~~BOM grouping by assembly step — organize BOM by SMT vs through-hole vs hand-solder for assembly house communication~~ | Wild | P2 | S | None | ~~Small but high-value DFM feature for assembly ordering.~~ **DONE 2026-03-03 — shared/schema.ts: assemblyCategory text column on bom_items ('smt'|'through_hole'|'hand_solder'|'mechanical'). ProcurementView.tsx: "Group by Assembly" view toggle, auto-detect assembly category from component descriptions (0805→smt, DIP→through_hole, JST→hand_solder, standoff→mechanical), category headers with item counts and assembly notes (e.g., "SMT: Requires solder paste stencil and reflow oven"). Tests in server/__tests__/bom-assembly.test.ts.** |
| [x] | CAPX-FFI-47 | ~~Net highlighting / cross-probing between all views — click a net in schematic → highlights on PCB, BOM, breadboard simultaneously~~ | Wild | P2 | M | None | **DONE 2026-03-03 — client/src/lib/cross-probe.ts: CrossProbeManager singleton with net/component highlighting, toggle for multi-select, deterministic net color via djb2 hash over 12-color palette, subscribe/unsubscribe pattern. useCrossProbe() React hook. 38 tests.** |
...
| [x] | CAPX-FFI-59 | ~~AI prediction engine — proactively predict what the user needs next based on design state and past patterns~~ | circuitmind-ai | P3 | L | None | ~~"You just added a microcontroller — you'll probably need decoupling caps and a crystal." Extends IN-05 (component suggestions).~~ **DONE 2026-03-04 — PredictionEngine singleton (ai-prediction-engine.ts): 32 rules across 5 categories (missing-component, design-improvement, safety, best-practice, optimization), pattern matching via node type/label analysis, confidence scoring, deduplication, dismiss/snooze; usePredictions hook; PredictionCard + PredictionPanel UI; 102 tests** |
| [x] | CAPX-FFI-60 | ~~Review queue for low-confidence AI actions~~ | Partsnap | P3 | M | CAPX-FFI-20 (confidence scoring) | **DONE 2026-03-03 — client/src/lib/ai-review-queue.ts: ReviewQueueManager singleton with configurable confidence threshold, auto-queue for low-confidence actions, 30-day expiry, approve/reject/dismiss workflow, localStorage persistence. Singleton+Subscribe+Hook pattern (useReviewQueue). 56 tests.** |
| [x] | CAPX-FFI-61 | ~~Inventory health score and dead stock detection~~ | PartScout | P3 | M | CAPX-FFI-13 (storage) | **DONE 2026-03-03 — client/src/lib/inventory-health.ts: InventoryHealthAnalyzer with 5 weighted scoring factors (stock coverage, diversity, freshness, documentation, organization). Dead stock detection (>180 days), low-stock alerts, category-level breakdown, actionable recommendations. 47 tests.** |
| [x] | CAPX-FFI-62 | ~~Component favorites / pinning — quick-access favorites for frequently used parts~~ | PartScout | P3 | S | None | **DONE 2026-03-03 — client/src/lib/component-favorites.ts: FavoritesManager singleton with localStorage persistence, add/remove/toggle, max 50 limit with oldest-eviction, corrupt data resilience. useFavorites() React hook with SSR safety. 33 tests.** |
...
| [x] | CAPX-FFI-64 | ~~AI architecture analysis mode~~ | OmniTrek | P3 | M | None | **DONE 2026-03-03 — client/src/lib/architecture-analyzer.ts: ArchitectureAnalyzer class with heuristic keyword-based analysis. Detects 8 subsystem types, 6 circuit patterns (voltage divider, H-bridge, decoupling cap, pull-up/down, RC filter, voltage regulator), signal flow tracing (BFS), power architecture analysis, design type classification (9 types), educational notes, and improvement suggestions. No AI API required. 90 tests.** |
