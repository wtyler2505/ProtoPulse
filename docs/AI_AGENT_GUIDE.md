...
Legacy app: 725 test files, ~30.5k tests passing (≈421 env-dependent failures on `main` — tracked in `ROADMAP.md` off-vision items), using Vitest 4. Engine (`packages/`): full suite all green, in its own CI workflow — that workflow is the merge gate. (Test-case counts live in `.ref/project-dna.md`, regenerated by live scan — never restate them here.)
...
Sixteen workspaces: `graph` (the core), `parts` (18 seed parts, provenance tiers, part packs), `erc`, `drc` (versioned fab decks), `route` (walkaround/shove/pours), `export` (netlist, BOM, Gerber, drill, pick-and-place, panelization — all byte-exact contracts), `sim` (ngspice-WASM), `emu` (ATmega328P + RP2040 + a from-scratch ESP32-S3 Xtensa LX7 core that boots real esptool `.bin` app images), `cosim` (the closed firmware↔analog loop), `review` (versioned review decks), `relay` (zero-conflict sync), `renderer` (WebGL2, SDF text, dual picking), `ai` (the six-member crew runtime), `cli` (`check`/`export`/`import-legacy`, exit 0/1/2), `content` (decks, the 88-article wiki, curriculum), `app` (the editor, port 5174). Full test suite all green (counts: `.ref/project-dna.md`); own CI at `.github/workflows/packages-ci.yml` (typecheck, lint, tests, AI-crew golden eval, golden smoke, builds; 100% branch coverage gate on the graph core). Full map: `packages/README.md`; status: `ROADMAP.md`.
...
npm run test:packages            # all engine tests (counts: .ref/project-dna.md)
