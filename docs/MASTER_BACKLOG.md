...
| BL-0868 | **Breadboard `color-contrast` axe rule disabled pending Plan 03 Phase 9 tokens** — 2026-04-23 Plan 03 Phase 5 (E2E-494) axe-core Playwright scan authored `e2e/p1-a11y-scan.spec.ts` + `e2e/a11y-helpers.ts`. Breadboard decorative rails (power strip red/blue, tie-point backgrounds) produced color-contrast noise that is a function of the current Tailwind palette rather than markup. Phase 9 (concurrent) is standardizing color tokens in `client/src/index.css` and will re-address. Action: once Phase 9 lands, remove the `disableRules: ['color-contrast']` override on the `breadboard` scan in `e2e/p1-a11y-scan.spec.ts:102` and confirm zero critical/serious. | OPEN | C2 | Plan 03 Phase 5 execution 2026-04-23 |
