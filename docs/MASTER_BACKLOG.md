...
| BL-0866 | **Pre-existing test suite drift — 49 failures across 10 files discovered 2026-04-22** — audit during Plan 01 eslint-unblock (BL-0865 verification) surfaced this; NOT caused by zve upgrade (logs from 2026-04-11 show same files green). Failures: (a) `server/__tests__/settings-routes.test.ts` — 2 fails, test sends `provider: 'anthropic'` but commit `855b1626` on 2026-04-18 removed `'anthropic'` from the schema enum; fix = change test to use `'gemini'`. (b) `server/__tests__/ai.test.ts` — 2 fails, assertion-order issue with API key redaction (sk-ant..., AIzaSy...). (c) `scripts/__tests__/check-api-types.test.ts` — 2 fails, `npx tsx script/generate-api-types.ts` throws. (d) `client/src/__tests__/a11y.test.tsx` — 20 fails (!) across multiple views: DashboardView "history is not iterable", ArchitectureView missing TooltipProvider, BreadboardView+ProcurementView missing useBom mock, CircuitCodeView "Worker is not defined", ArduinoWorkbenchView cannot read .find. (e) Various ECONNREFUSED 127.0.0.1:3000 — tests expect live dev server. (f) `client/src/components/circuit-editor/__tests__/BreadboardPartInspector.trustTier.test.tsx` — 10/10 failing. (g) lifecycle-badges-integration, collaboration-crdt-integration, procurement-sub-components each have a few fails. Per Tyler's "fix everything" rule these all need attention, but triaging multi-file drift is its own dedicated task. Should split into smaller BLs per file when prioritized. | OPEN | C3 | BL-0865 eslint verification 2026-04-22 |
| BL-0865 | **ESLint blocked by zod-validation-error@3.5.4 missing `/v4` subpath** — 2026-04-22 Plan 01 execution: `npx eslint .` fails at startup with `ERR_PACKAGE_PATH_NOT_EXPORTED: Package subpath './v4' is not defined by "exports" in .../zod-validation-error/package.json`. Root cause: `eslint-plugin-react-hooks@7.0.1` calls `require('zod-validation-error/v4')` at runtime (see `node_modules/eslint-plugin-react-hooks/cjs/eslint-plugin-react-hooks.development.js:20`), and its peer-range `^3.5.0 \|\| ^4.0.0` allows either, but only the `^4.x` line ships the `/v4` subpath (`zod-validation-error@3.5.4` does not). Project is pinned to `zod-validation-error: ^3.4.0` in `package.json` line 125. **Fix:** bump `zod-validation-error` to `^4.0.2`, then in all 18 source files that import `from 'zod-validation-error'` (server/routes/*.ts mostly, including parts.ts:18, validation.ts:3, chat-branches.ts:3, design-preferences.ts:3, component-lifecycle.ts:2, projects.ts:2, agent.ts:3, bom-templates.ts:14, settings.ts:5, architecture.ts:3, design-history.ts:3, knowledge-vault.ts:15, arduino.ts:4, components.ts:3, rag.ts:3, pcb-zones.ts:2) change the import to `from 'zod-validation-error/v3'` because the project still uses Zod 3 (v4 subpath uses `zod/v4/core` types). Verify `npm run check` + `npm test` still green after the bump. This is BLOCKING the eslint quality gate for Plan 01 Phase 2+. | OPEN | C1 | Plan 01 Phase 2 dispatch 2026-04-22 |
