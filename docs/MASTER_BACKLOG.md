...
| P1 | **4** | 73 | 3 opened 2026-06-28 by the AI-first audit (BL-0902→0904); BL-0907 opened 2026-06-29 (genkit/otel HIGH vulns). Prior: all resolved (Waves 54-67). |
...
| **Total** | **6** | **508** | **514 items tracked** — 5 opened 2026-06-28 (AI-first audit), 1 opened 2026-06-29 (BL-0907 security); 508 done. |
...
| BL-0903 | **Version engine AI prompts + add missing `packages/ai/README.md`** — `@protopulse/ai` system/crew prompts live inline in code; extract them into versioned prompt files. Add the `packages/ai/README.md` that CLAUDE.md's engine section already references, documenting **model pins (provider+model+version)** and a **cost/latency budget**. Add a one-line "legacy `server/ai.ts` is frozen; new AI work → `@protopulse/ai`" note to AGENTS.md. **PARTIAL (2026-06-29):** ✅ `packages/ai/README.md` authored (6-persona crew, model pin `claude-sonnet-4-6`/8192 from `anthropic.ts:28-29`, `CONTEXT_BUDGETS` + per-agent `maxTurns` + `CostClass` budget section, prompt-location map, FakeProvider/eval); ✅ AGENTS.md frozen-`server/ai.ts` pointer added (engine section). **Remaining:** prompt extraction into versioned modules — deferred as *architecture-class* (6 agent files + their `*.test.ts` assert on prompt content; structure of a `prompts/` layer is an ADR-worthy decision, not a blind move). Carve to follow-up once the extraction shape is decided (advisor/peer-review + ADR). | PARTIAL | C3 | AI-first audit 2026-06-28 |
