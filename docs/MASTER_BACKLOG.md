...
| BL-0875 | **a11y test cluster (drift-updated 2026-05-09)** — `client/src/__tests__/a11y.test.tsx` **17 fail / 10 pass / 27 total** at 2026-05-09 (BL-0866 originally said 20 fails; drift has continued). Failures span ~24 views including Architecture, ArduinoWorkbench, AuditTrail, Board, Breadboard, Calculators, CircuitCode, Community, ComponentEditor, Dashboard, DesignHistory, DesignPatterns, DigitalTwin, GenerativeDesign, Kanban, Knowledge, PartAlternatesBrowser, PartUsageBrowser, PCBLayout, PcbOrdering, Procurement, Schematic, Validation, VaultBrowser. **Three distinct failure modes observed:** (a) `Tooltip must be used within TooltipProvider` (e.g., ValidationView wrapping in test setup is missing) — test-infra fix; (b) `ECONNREFUSED 127.0.0.1:3000` — tests hit a dev server that isn't running, overlaps with BL-0876; (c) **real axe-core violations** (button-name, label, aria-valid-attr-value, aria-prohibited-attr) in CalculatorsView, DesignPatternsView, KanbanView, KnowledgeView, BoardViewer3DView, CommunityView etc. — these are SOURCE CODE bugs, not test bugs. **Fix:** triage per-view; expect a mix of test-setup wrappers + source code accessibility fixes. Should split further into per-view BLs when a session has dedicated capacity. Diagnostic log: `logs/tests-a11y-latest.log`. | OPEN | C1 | BL-0866 split + drift-update 2026-05-09 |
| BL-0876 | **ECONNREFUSED 127.0.0.1:3000 in tests (BL-0866 cluster)** — Various tests assume a live dev server on :3000. **Diagnosis 2026-05-09:** root cause is `useQuery` calls in QueryClientProvider-wrapped views (a11y test setup) triggering real `fetch` against the missing dev server. **Staged fix 2026-05-09:** added a default `vi.stubGlobal('fetch', ...)` in `client/src/test-setup.ts` (`beforeEach`) that returns `404` JSON `null` so `useQuery` resolves cleanly to "no data" instead of TCP-failing. Per-test fetch mocks still work via `vi.spyOn` overrides. **Pending verification** in a fresh session: re-run the a11y suite + lifecycle-badges-integration + procurement-sub-components and confirm zero `ECONNREFUSED` lines in the log. If a11y violations remain, those are real source-code bugs (BL-0875), not BL-0876. | STAGED | C2 | BL-0866 split 2026-05-09 |
