...
| ID | Description | Status | Complexity | Source |
|----|-------------|--------|------------|--------|
| BL-0772 | E2E-235 🔴 BUG (CONFIRMED) — 3D View board = 100×80 mm. PCB tab board = 50×40 mm. Same project, same circuit, two different board sizes. Source-of-truth split. | OPEN | C2 | E2E walkthrough (3d-view-meticulous) — refs E2E-235 |
...
| BL-0789 | E2E-270 🔴 BUG (PATTERN) — Order PCB Width/Height = 100×80mm matches 3D View (100×80) but NOT PCB tab (50×40). Three different boards in same project: PCB / 3D / Order. Source-of-truth must converge. | OPEN | C2 | E2E walkthrough (order-pcb-meticulous-baby-step) — refs E2E-270 |
...
| BL-0838 | E2E-228 🔴 BUG (board size mismatch) — PCB tab shows board 50×40 mm. 3D View tab default board 100×80 mm. Two different defaults for same project's board. | OPEN | C2 | E2E walkthrough (pcb-meticulous) — refs E2E-228 |
...
| BL-0849 | \| E2E-228/235/270 \| 🔴 P1 \| PCB/3D View/Order PCB \| THREE different default board sizes for same project (50×40 / 100×80 / 100×80) \| Single source-of-truth for board geometry \| | OPEN | C2 | E2E walkthrough (tl-dr-top-p0-p1-bugs-action-items) — refs E2E-228 |
...
| BL-0855 | **`/extract` must also read `ops/queue/user-suggestions.md`** — `vault-inbox` skill writes user-surfaced note suggestions to a separate queue (`ops/queue/user-suggestions.md`) distinct from `ops/queue/gap-stubs.md`. `/extract` currently consumes only `gap-stubs.md`. Wire `--include-user-queue` flag (described in `vault-inbox/SKILL.md` as future extension) + moderation gate (user-suggested stubs enter `triage_status: pending-review` and require a human/AI-reviewer pass before extraction). Infrastructure work, not knowledge content. | OPEN | C2 | vault-inbox smoke test 2026-04-19 — surfaced by /vault-inbox extract |
| BL-0856 | **`/extract` skill must emit v2-schema-compliant frontmatter** — 2026-04-19 T4 content push landed 62 new notes via 8 `/extract` runs; 60/62 fail `/vault-quality-gate` with 4 systematic issues: (a) `description` > 140 chars (60 notes), (b) `topics` not linked to any existing MOC (49 notes), (c) `type` field missing or using v1 values (22 notes), (d) confidence claim without matching provenance (5 notes). Fix: update `.claude/skills/extract/SKILL.md` + its script to (1) truncate descriptions to 140 chars, (2) validate each topic against the MOC registry + fall back to `general` when unsure, (3) always emit `type: {claim|methodology|tension|moc|question}`, (4) require provenance when confidence is asserted. Then run gate remediation on the 60 existing notes. Catch-up work; content is sound, only frontmatter drift. | OPEN | C3 | /vault-quality-gate dry-run 2026-04-19 on 62 extracted notes |
| BL-0857 | **Vault MOC auto-stub cleanup** — 5 new MOCs created this session (`moc-a11y-wcag-aria`, `moc-electronics-math`, `moc-component-metadata-fields`, `backend-persistence-patterns`, `wcag`) — some overwrote auto-generated stubs from the `auto-create-parent-moc.sh` hook. Audit the hook's stub-generation output vs hand-curated MOC quality. Either upgrade the hook to produce richer stubs, or add a "promote-stub-to-real-MOC" workflow. | OPEN | C2 | T4 content push 2026-04-19 |
