...
| BL-0861 | **Extract `<VaultInfoIcon>` shared component (DRY threshold reached)** — 2026-04-19 Task 62 code review: 12 consumers of the `<VaultHoverCard>` primitive all replicate the same 5-prop trigger span (BookOpen w-3 h-3 + `inline-flex items-center cursor-help opacity-60 hover:opacity-100 transition-opacity` + data-testid + aria-label). Pattern has been stable across 11 prior implementations; ready to extract. Proposed API: `<VaultInfoIcon slug testId ariaLabel />` (~15 lines, collapses 9-line block to 1 at each call site). File under `client/src/components/ui/vault-info-icon.tsx`. Update `docs/design-system/vault-primitives.md` to reference it. Should land BEFORE Plan 15 wave-2 and any further consumer wirings so the pattern doesn't solidify further. | OPEN | C1 | Task 62 code review 2026-04-19 |
| BL-0862 | **Shared `createTestQueryClient()` test utility** — 2026-04-19 Task 62 code review: 4 test files (`ArchitectureView.test.tsx:183`, `storage-manager.test.tsx:141`, `GenerativeDesignView.test.tsx:9`, `CircuitCodeView.test.tsx:59`) each invented their own inline `QueryClient` with 3 different config variants (`retry: false` / `retry: false + gcTime: 0` / other). Extract `client/src/test-utils/createTestQueryClient.ts` with canonical defaults so the next ~10 consumer tests (one per Phase 8 consumer) reference a single source. Not a flakiness risk but it IS a drift vector. | OPEN | C1 | Task 62 code review 2026-04-19 |
