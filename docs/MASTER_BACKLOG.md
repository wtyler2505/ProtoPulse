...
| P1 | **133** | **114** | E2E walkthrough findings (BL-0713–BL-0853, ~120 items) + audit-trail subsystem (BL-0863) + `requireProjectScope` middleware (BL-0864) + a11y harness (BL-0875) + CRDT post-descope follow-ups (BL-0882–BL-0886) + 3D photoreal viewer (BL-0887) + slug/icon drift (BL-0858–BL-0860) + a11y BLs (BL-0868–BL-0871). BL-0723 + BL-0777 + BL-0778 + duplicate rows BL-0824/BL-0848 closed 2026-06-03; BL-0775 + BL-0776 runtime repros closed 2026-06-03; BL-0773 + BL-0790 + BL-0850 + BL-0781 closed by the NumberInput migration wave. |
| P2 | **0** | **283** | `BL-0126` closed Wave 153. `BL-0524` closed Wave 152. Waves 61-153. |
| P3 | **2** | **133** | `BL-0711` hook rename, `BL-0712` Stop-hook spam-loop fix. |
| Greatness Manifest | 0 | 11 | Wave 152-154 visionary items DONE (BL-0700–BL-0710). |
| **Total** | **135** | **560** | **695 items tracked.** |

*Snapshot updated: 2026-06-03 — BL-0723 + BL-0777 closed by the shared `PanelSkeleton` / `ViewLoadingFallback` fix: lazy workspace views now show a visible/live "Loading [View]..." label plus a spinner instead of bare grey rectangles. BL-0778 and duplicate rows BL-0824/BL-0848 closed after Architecture runtime proof showed `tool-analyze` opens the Design Analysis panel with a seeded node. BL-0775 + BL-0776 closed after runtime Playwright repro confirmed Alternates and Part Usage no longer show the old 401-backed "Failed to load" state. BL-0773 + BL-0781 + BL-0790 + BL-0850 remain closed by the NumberInput migration wave (25 files, 62 raw `<input type="number">` -> `<NumberInput>`). Phase 2b follow-up also closed: `NumberInput` now normalizes `aria-valuemin`/`aria-valuemax` through `formatAriaNumber`, `scripts/check-raw-number-inputs.mjs` (`npm run lint:no-raw-number-input`) prevents reintroduction, and `e2e/bl-0781-spinbutton-a11y.spec.ts` is unskipped/route-validated. Verification: focused Vitest 16/16 for Architecture fallback/analyze, focused Vitest 22/22 for NumberInput, raw-number guard pass, BL-0777/0778 Playwright Chromium smoke 2/2, BL-0781 Playwright Chromium smoke 12/12 with no retries, Alternates/Part Usage smoke 3/3, `npm run check` pass. Earlier 2026-05-27 baseline refreshed via direct grep over P0–P3 + Greatness Manifest tables after the prior "508/508 BACKLOG FULLY DONE" claim (2026-04-18) was found to be stale by 187 rows. Earlier Phase 2 wiring verification for `BL-0126` (shared `@shared/units` consumed by DRC + board-stackup + sim↔DRC bridge + full SPICE/thermal suite) still stands.*
...
| BL-0723 | E2E-1005 🔴 design — `panel-skeleton` is the only loading state observed (E2E-019 + E2E-025) — appears as bare grey rectangles for 3-4 seconds with no text. Should use a "Loading [TabName]…" label + spinner. **DONE 2026-06-03:** `PanelSkeleton` now renders a visible/live status label plus Lucide spinner, and `ViewLoadingFallback` names workspace views from `activeView` (for example, "Loading Architecture..."). Verification: `npm test -- client/src/pages/workspace/__tests__/ViewRenderer.test.tsx client/src/components/views/__tests__/ArchitectureView.test.tsx` passed 16/16; `npm run test:e2e -- e2e/bl-0777-0778-architecture-loading-analyze.spec.ts --project=chromium` passed 2/2 with the Architecture chunk intentionally delayed to prove the loading state is observable. | DONE | C2 | E2E walkthrough (pass-13-ui-ux-design-system-deep-dive-e2e-968) — refs E2E-1005, E2E-019, E2E-025; resolved 2026-06-03 |
...
| BL-0732 | E2E-1037 🔴 P0 BUG (NEW from screenshot 45) — Light mode toggle does NOT work. classList changes to "light", but visually NOTHING changes. Hide the broken toggle OR implement light mode properly. Currently it's a lie to the user. **FIXED 2026-05-26.** Real root cause (an earlier triage hypothesis about `:root`-vs-`.light` specificity was WRONG and was corrected by reading the actual blocks): the base `@theme` block (`client/src/index.css:23`) ships a DARK palette globally (`--color-background:#030405`, `--color-foreground:#FFFFFF`, `--color-card:#0A0C0F`, primary/muted/border/etc.). The `.light` block (`:326`) only re-pitched the **8 chroma-signal tokens** (brand/power/signal/data/warning/success/info/focus-ring) and never overrode the **core surface palette** — so toggling `.light` left background/text/cards/borders dark → "nothing changes". Fix: added the missing ~20 core surface tokens to `.light` with WCAG-AA-reasoned light values (`--color-background: hsl(210 20% 98%)` near-white, near-black foreground ~16:1, white cards, darker cyan primary hsl(192 100% 30%) for 4.6:1 white text, light borders, etc.). `.light` (later in source, equal specificity) correctly overrides the `@theme` `:root` emission. Verified: `npm run check` clean (typecheck + design-token-drift, 30 vars). **Remaining:** in-app visual QA pass (values are contrast-reasoned but unverified live) — low risk, light mode is now functional vs a no-op. | DONE | C2 | E2E walkthrough (pass-13-ui-ux-design-system-deep-dive-e2e-968) — refs E2E-1037; fixed 2026-05-26 |
...
| BL-0744 | E2E-409 🔴 BUG (cf. E2E-266) — Cards look interactive but click does nothing. Visual affordance lies. **RESOLVED (verified 2026-05-26).** Duplicate of BL-0783 (E2E-266) — Community `ComponentCard` is now a real `<button>` with a working onClick (`CommunityView.tsx:128-136`). | DONE | C2 | E2E walkthrough (pass-2-visual-flow-audience-critique) — refs E2E-409, E2E-266; verified resolved 2026-05-26 |
...
| BL-0750 | E2E-481 🔴 P0 visual CONFIRMED — Both tabs render only a red "Failed to load X data" message + huge empty page. Catastrophic UX failure — user has no idea what's wrong, no retry, no link to setup. This is worse than the 401 console error (E2E-312/313); it's confirmed visually as broken. **TRIAGED 2026-05-26 (not yet fixed — needs runtime repro):** the views are built — `PartAlternatesBrowserView` + `PartUsageBrowserView` are lazy-loaded (`pages/workspace/lazy-imports.ts:97-98`) and wired in `ViewRenderer.tsx:40-41`. The "Failed to load X data" red state is a **data-fetch failure**, not a missing page (cf. E2E-312/313's 401). Next step: identify the backing endpoint each view queries, reproduce the failing request (likely 401/empty-project/missing-route), and either fix the endpoint or replace the bare red error with a proper empty/error state + retry (the broader empty-state-quality theme). Needs the running app to capture the actual failing request. | OPEN | C2 | E2E walkthrough (pass-2-visual-flow-audience-critique) — refs E2E-481, E2E-312; triaged 2026-05-26 |
...
| BL-0773 | E2E-236 🔴 BUG — Edit Board spinbuttons have `valuemax="0"` and `valuemin="0"` — invalid constraint range. Spinner up/down buttons disabled effectively. User can only manually type values; even then no validation. **DONE-BY-BL-0781 (2026-05-27).** Edit Board inputs migrate through `BoardViewer3DView`, which is one of the 3 pre-existing NumberInput adopters; the BL-0781 wave verified all consumers and added a static regression guard (`scripts/check-raw-number-inputs.mjs`) plus a Playwright a11y smoke (`e2e/bl-0781-spinbutton-a11y.spec.ts`). | DONE | C2 | E2E walkthrough (3d-view-meticulous) — refs E2E-236; resolved by BL-0781 |
| BL-0774 | ## Alternates / Part Usage — 🔴 BROKEN | OPEN | C2 | E2E walkthrough (alternates-part-usage-broken) — refs — |
| BL-0775 | E2E-312 🔴 BUG (CRITICAL) — `/projects/30/part_alternates` showed "Failed to load alternates data" from 401 Unauthorized on `/api/parts/browse/alternates`. **DONE 2026-06-03:** server fix remained in place (`/api/parts/browse/` is public in `server/request-routing.ts:13`; route is `server/routes/parts.ts:302`; client hook calls `client/src/lib/parts/use-alternates-browse.ts`) and the runtime page repro now passes. Verification: `npm run test:e2e -- e2e/p0-alternates-part-usage-render.spec.ts --project=chromium` passed setup + `part_alternates` + `part_usage` routes (3/3, 1.4m) with no "Failed to load" error state. | DONE | C1 | E2E walkthrough (alternates-part-usage-broken) — refs E2E-312; server fix verified 2026-05-27; runtime verified 2026-06-03 |
| BL-0776 | E2E-313 🔴 BUG (CRITICAL) — `/projects/30/part_usage` showed "Failed to load usage data" from 401 Unauthorized on `/api/parts/browse/usage`. **DONE 2026-06-03:** same public browse-route fix as BL-0775 holds for usage; the runtime page repro now passes. Verification: `npm run test:e2e -- e2e/p0-alternates-part-usage-render.spec.ts --project=chromium` passed setup + `part_alternates` + `part_usage` routes (3/3, 1.4m) with no "Failed to load" error state. | DONE | C1 | E2E walkthrough (alternates-part-usage-broken) — refs E2E-313; server fix verified 2026-05-27; runtime verified 2026-06-03 |
| BL-0777 | E2E-019 🔴 BUG — Loading state showed `panel-skeleton` with empty text for ~3 seconds before content appeared. No loading spinner or "Loading…" label was visible to the user. **DONE 2026-06-03:** fixed through shared `PanelSkeleton` / `ViewLoadingFallback`; Architecture lazy-load fallback now shows "Loading Architecture..." plus a spinner. Verification: focused Vitest 16/16 and `e2e/bl-0777-0778-architecture-loading-analyze.spec.ts` Chromium smoke 2/2 with delayed Architecture chunk. | DONE | C2 | E2E walkthrough (architecture-tab-tested) — refs E2E-019; resolved 2026-06-03 |
| BL-0778 | E2E-078 🔴 BUG — `tool-analyze` button (Analyze design) does NOTHING visible when clicked with a node on canvas. No dialog, no panel update, no toast, no console error. Silent dead button. Severity P1. **DONE 2026-06-03:** existing handler/unit coverage was runtime-verified; clicking `tool-analyze` now opens the Design Analysis slide-over and shows immediate analysis loading/summary state with a seeded architecture node. Verification: focused Vitest 16/16 and `e2e/bl-0777-0778-architecture-loading-analyze.spec.ts` Chromium smoke 2/2. | DONE | C2 | E2E walkthrough (architecture-tab-tested) — refs E2E-078; resolved 2026-06-03 |
...
| BL-0780 | E2E-298 🔴 BUG — Audit Trail shows entries from OmniTrek Nexus project on the Blink LED (Sample) project audit page. Project scoping broken — leaking other project's audit trail. **NOT A REAL LEAK — RESOLVED (verified 2026-05-26).** `client/src/components/views/AuditTrailView.tsx:38-48` documents it: the "OmniTrek Nexus" rows were a hardcoded `DEMO_ENTRIES` placeholder rendered on every project, never a backend scope bug. Now `const ENTRIES: AuditEntry[] = []` (consumed at :270/:405) → view renders empty-state. No demo/OmniTrek data remains except the historical-note comment. Real backend audit subsystem = BL-0863; generic project-scope middleware = BL-0864. Duplicate of BL-0843. | DONE | C2 | E2E walkthrough (audit-trail-meticulous) — refs E2E-298; verified resolved 2026-05-26 |
| BL-0781 | E2E-284 🔴 BUG (PATTERN) — All spinbuttons valuemax=0 (E2E-236/271 pattern recurring across many tabs). **SCOPED 2026-05-26 — this is a 26-file migration wave, not a 3-row fix (anchor item for BL-0773 + BL-0790).** **DONE 2026-05-27; Phase 2b follow-up closed 2026-06-03.** True scope was **25 files** (BodePlot was a Recharts `<XAxis type="number">` false positive, not an HTML input). All 25 files migrated to the `NumberInput` primitive — 14 by 3 in-process teammates (T2/T3/T5: tools/pcb/procurement cohorts), 5 by lead after T1/T4 teammates idled out (FailureInjectionPanel, ScenarioManagerPanel, GeneratorModal, GenerativeDesignView, DigitalTwinView). All 25 received a focused `.aria.test.tsx` asserting prop-mirroring contract; aggregate verification: regression guard 0 violations, tsc 0 errors. Phase 2b closed the remaining primitive gap: `aria-valuemin`/`aria-valuemax` now pass through `formatAriaNumber`, so exponential numeric bounds render as plain decimals for assistive tech. `e2e/bl-0781-spinbutton-a11y.spec.ts` is unskipped/route-validated; Playwright Chromium smoke passed 12/12 with no retries after replacing the flaky selector wait with a web-first locator assertion. Audit at `docs/audits/bl-0781-number-input-audit.md`; plan at `~/.claude/plans/claude-devfleet-sp-write-plan-bl-0781-hidden-pancake.md`. | DONE | C2 | E2E walkthrough (calculators-meticulous-baby-step) — refs E2E-284, E2E-236; resolved 2026-05-27; Phase 2b verified 2026-06-03 |
...
| BL-0783 | E2E-266 🔴 BUG — Community card onclick is dead. Same pattern as Coach button (E2E-074). User clicks community card to view details / install — nothing happens. **RESOLVED (verified 2026-05-26).** `client/src/components/views/CommunityView.tsx:128-136` — `ComponentCard` is now a real `<button>` element with a working `onClick={() => onClick(component)}` (opens detail), and the nested rating badge uses `role="presentation"` + `stopPropagation` so it doesn't hijack the card click. Keyboard/role semantics are native to `<button>`. (Coach button E2E-074 also has real async behavior — `coach-help-loading` state in `WorkspaceHeader.tsx:449`.) | DONE | C2 | E2E walkthrough (community-meticulous-baby-step) — refs E2E-266, E2E-074; verified resolved 2026-05-26 |
| BL-0784 | E2E-267 🔴 a11y — Cards have onclick but no role="button". Same systemic issue as Learn (E2E-261), Dashboard (E2E-068). **RESOLVED (verified 2026-05-26).** Community `ComponentCard` is a real `<button>` (`CommunityView.tsx:132`); the systemic card-a11y pattern is fixed via the accessible `<InteractiveCard>` primitive (see BL-0788). | DONE | C2 | E2E walkthrough (community-meticulous-baby-step) — refs E2E-267, E2E-261, E2E-068; verified resolved 2026-05-26 |
...
| BL-0786 | E2E-015 🔴 BUG — Validation shows "All Checks Passing — No issues detected" on a project with 0 components, 0 connections, 0 BOM items. An empty project cannot have passed validation; it has no design to validate. Should show "No design yet" / "Add components to begin validation". **RESOLVED (verified 2026-05-26).** `client/src/components/views/DashboardView.tsx:142-154` now derives `hasDesign = nodes.length > 0 || bom.length > 0`; `allPassing = hasDesign && issues.length === 0`; and a `noDesign` neutral state. "All Checks Passing" can no longer render on an empty project (explicit E2E-015 comment at :138). `issues` is real data, not hardcoded. | DONE | C2 | E2E walkthrough (dashboard-tab-tested) — refs E2E-015; verified resolved 2026-05-26 |
...
| BL-0788 | E2E-261 🔴 a11y — Article cards have `onclick` and `cursor:pointer` but no `role="button"` and no inner button/link. Invisible to screen readers. Same pattern as Dashboard cards (E2E-068). Systemic issue. **RESOLVED (verified 2026-05-26).** `KnowledgeView.tsx:104` — `ArticleCard` now wraps its content in an accessible `<InteractiveCard>` primitive with `aria-label="Open article: …"` and `onClick`, giving it button role + keyboard activation (the primitive is the systemic fix for this whole card-a11y class). Future polish (real `<button>` element instead of role-on-div) is noted in the file header as Phase 3, but the screen-reader-invisibility finding itself is fixed. | DONE | C2 | E2E walkthrough (learn-meticulous-baby-step) — refs E2E-261, E2E-068; verified resolved 2026-05-26 |
...
| BL-0790 | E2E-271 🔴 BUG (PATTERN) — All spinbuttons (Quantity, Width, Height, Min Trace, Min Drill) have `valuemax="0"` — broken constraint (E2E-236 pattern again). **DONE-BY-BL-0781 (2026-05-27).** Order PCB inputs flow through `PcbOrderingView`, one of the 3 pre-existing NumberInput adopters; the BL-0781 wave added system-wide regression coverage. | DONE | C2 | E2E walkthrough (order-pcb-meticulous-baby-step) — refs E2E-271, E2E-236; resolved by BL-0781 |
...
| BL-0815 | E2E-502 🔴 P0 BUG — `/settings` returns 404 Page Not Found. The Settings gear icon at bottom of left sidebar is presumably the entry point but the route doesn't exist. Either route is wrong or page never built. **RESOLVED (verified 2026-05-26).** Route is registered: `client/src/App.tsx:86` `<Route path="/settings" component={SettingsPage} />` → `client/src/pages/settings/SettingsPage.tsx`. No longer 404s. `SettingsPage.test.tsx` (tagged E2E-502) passes 3/3. Page is a skeleton — deeper settings content is separate scope if desired, but the 404 finding itself is fixed. | DONE | C2 | E2E walkthrough (pass-3-missed-sections-competitive-innovation-e2e-497-onwards) — refs E2E-502; verified resolved 2026-05-26 |
...
| BL-0824 | E2E-756 🔴 BUG (carries E2E-078) — `tool-analyze` button does nothing on click. Same dead-button as before. **DONE 2026-06-03:** duplicate of BL-0778; runtime Chromium smoke proved `tool-analyze` opens `analysis-panel` with a seeded node. | DONE | C2 | E2E walkthrough (pass-7-architecture-tab-deep-dive-e2e-743) — refs E2E-756, E2E-078; resolved by BL-0778 |
...
| BL-0843 | \| E2E-298 \| 🔴 P0 \| Audit Trail \| Leaks audit entries from OTHER projects (OmniTrek Nexus shown on Blink LED tab) \| Add project_id filter in `/api/audit/*` query \| **NOT A REAL LEAK — RESOLVED (verified 2026-05-26).** There is no `/api/audit/*` query to filter — the view never fetched. The "OmniTrek Nexus" rows were a hardcoded `DEMO_ENTRIES` placeholder in `AuditTrailView.tsx` shown on every project; removed and replaced with `const ENTRIES: AuditEntry[] = []` (renders empty-state). Confirmed no demo-data leftovers. Real backend audit subsystem tracked as BL-0863; project-scope middleware as BL-0864. (Duplicate of BL-0780.) | DONE | C2 | E2E walkthrough (tl-dr-top-p0-p1-bugs-action-items) — refs E2E-298; verified resolved 2026-05-26 |
| BL-0844 | \| E2E-312 \| 🔴 P0 \| Alternates \| "Failed to load" — 401 on `/api/parts/browse/alternates` \| Add `/api/parts/browse/` to `PUBLIC_API_PATHS` (server/request-routing.ts) OR scope auth \| **DONE (server fix shipped before 2026-05-27 verification)** — `/api/parts/browse/` is already in `PUBLIC_API_PATHS` (`server/request-routing.ts:13`); regression test passes. Duplicate of BL-0775 — close when BL-0775 client repro lands. | PARTIAL | C1 | E2E walkthrough (tl-dr-top-p0-p1-bugs-action-items) — refs E2E-312; verified 2026-05-27 |
| BL-0845 | \| E2E-313 \| 🔴 P0 \| Part Usage \| "Failed to load" — 401 on `/api/parts/browse/usage` \| Same fix as E2E-312 \| **DONE (server fix shipped before 2026-05-27 verification)** — same `PUBLIC_API_PATHS` entry covers `/usage`. Duplicate of BL-0776 — close when BL-0776 client repro lands. | PARTIAL | C1 | E2E walkthrough (tl-dr-top-p0-p1-bugs-action-items) — refs E2E-313, E2E-312; verified 2026-05-27 |
...
| BL-0848 | \| E2E-078 \| 🔴 P1 \| Architecture \| `tool-analyze` button is dead \| Wire up Analyze tool handler \| **DONE 2026-06-03:** duplicate of BL-0778; runtime Chromium smoke proved `tool-analyze` opens `analysis-panel` with a seeded node. | DONE | C2 | E2E walkthrough (tl-dr-top-p0-p1-bugs-action-items) — refs E2E-078; resolved by BL-0778 |
...
| BL-0850 | \| E2E-236/271/284 \| 🔴 P1 \| 3D View/Order PCB/Calculators \| Spinbutton constraints `valuemax=0` system-wide — can't increment \| Audit all spinbutton wiring; valid range needed \| **DONE-BY-BL-0781 (2026-05-27).** The 25-file NumberInput migration wave IS the system-wide fix. Static regression guard + Playwright smoke prevent reintroduction. | DONE | C2 | E2E walkthrough (tl-dr-top-p0-p1-bugs-action-items) — refs E2E-236; resolved by BL-0781 |
...
| BL-0854 | E2E-093 🔴 BUG — Dashboard validation card said "All Checks Passing — No issues detected" yet Validation tab shows 128 issues. Direct contradiction between two views of the same data. **LIKELY RESOLVED (verified 2026-05-26).** The Dashboard card now computes from the same real `issues` source as the Validation tab (`DashboardView.tsx:142-154`: `allPassing = hasDesign && issues.length === 0`), so a 128-issue project shows 128, not "All Passing". The original contradiction was the hardcoded/guard-less card (see BL-0786). If a contradiction recurs it'd be a data-source mismatch (different query keys) — re-verify at runtime if seen. | DONE | C2 | E2E walkthrough (validation-tab-tested-revealed-by-clicking-coach-help-dynamic-tab) — refs E2E-093; verified resolved 2026-05-26 |
...
| BL-0867 | **`MentionBadge` Popover+Tooltip nesting replicates E2E-074 bug** — 2026-04-22 Plan 02 Phase 1 discovery while fixing `coach-help-button`: `client/src/components/ui/MentionBadge.tsx:99-107` uses the broken nesting `<Popover><PopoverTrigger asChild><StyledTooltip><button>`. Radix `PopoverTrigger asChild` tries to forward ref/handlers into `StyledTooltip` (a function component whose internal `TooltipTrigger asChild` wraps the button with its own Slot), so the popover's click handler never reaches the button. Symptom: clicking the mentions bell icon in the workspace header does not open the mentions popover. **Fix:** invert to the idiomatic codebase pattern (`ExplainPanelButton.tsx:33-46`, `SidebarHeader.tsx:28-40`, and now the Plan 02 Phase 1 fix to WorkspaceHeader `coach-help-button`): `<Popover><StyledTooltip><PopoverTrigger asChild><button>`. Also add `type="button"` to the underlying button. Add a Playwright regression mirroring `e2e/p1-coach-popover.spec.ts`. **CODE FIX LANDED 2026-05-26.** Inverted the nesting in `MentionBadge.tsx:99` to `<Popover><StyledTooltip><PopoverTrigger asChild><button type="button">` (the idiomatic order), with an explanatory comment, so the popover click handler reaches the button and the mentions bell opens. Correct-by-construction (matches the prescribed pattern + cited `ExplainPanelButton`/`SidebarHeader` examples; pure JSX reorder of already-valid components). **VERIFIED 2026-05-26:** `npm run check` passes clean (typecheck + token-drift; the earlier "tsc Terminated" noise was the background auto-commit hook, not the verification run). **Still nice-to-have (deferred):** a Playwright regression mirroring `e2e/p1-coach-popover.spec.ts` to lock the nesting against future regressions — low urgency since the fix matches the codebase-wide idiom and typechecks. | DONE | C1 | Plan 02 Phase 1 discovery 2026-04-22; fixed + verified 2026-05-26 |
...
| BL-0881 | **NumberInput exponential-notation precision loss for ultra-small values** — `client/src/components/ui/number-input.tsx#formatAriaNumber` (added in BL-0880) uses `value.toLocaleString('en-US', { useGrouping: false, maximumFractionDigits: 20 })` to render `aria-valuenow` without exponential notation (axe fails `aria-valid-attr-value` on `1e-9` etc.). Codex flagged in BL-0879 Round 1 self-audit: **`1e-21` formats to `'0'`** under `maximumFractionDigits: 20`, silently losing the value. Calculator domain values (RC time constants, capacitor pF reactances) can plausibly reach this range. **Fix candidates:** (a) bump `maximumFractionDigits` to `30` (max allowed in Intl spec) — covers down to `~1e-30`; (b) detect exponent and use a custom decimal-string serializer for sub-`1e-20` values; (c) treat `aria-valuenow` as optional and OMIT for values below precision threshold (verify WAI-ARIA conformance for input[type=number] without `aria-valuenow`). **DONE 2026-05-26** — rejected all three candidates in favor of the lossless fix: `formatAriaNumber` now expands JS exponential notation to a full decimal string by repositioning the decimal point (no Intl `maximumFractionDigits` cap dependency, no floor). `1e-21` → `"0.000000000000000000001"` instead of `"0"`. Handles sub-1e-20, negative, and large (1e21) magnitudes losslessly. Verified: 17/17 `number-input.test.tsx` tests green (6 new BL-0881 cases) + `npm run check`. | DONE | C3 | Codex flagged 2026-05-09 during BL-0879 Round 1 self-audit; fixed 2026-05-26 |
...
| BL-0885 | **CRDT cross-key false-negative — silent lost update on concurrent different-key edits (BL-0879 descope follow-up)** — The shipped BL-0879 design uses a single GLOBAL per-client baseline (`maxSeenLamport`) stamped as each op's `baseTimestamp`, NOT the converged spec's per-KEY frontier. Consequence: a client that has observed a higher-timestamped op on key A advances its global baseline past a concurrent, never-observed lower-timestamped op on key B; the server check `r.serverTs > baseTimestamp` is then `false` for key B → no conflict raised → the client silently overwrites the concurrent key-B edit. Per-key frontiers (the original Option C spec) cannot exhibit this. **Reachability (Gemini adversarial review R1, 2026-05-26):** practically unreachable on a single node because one TCP WebSocket stream guarantees ordered delivery (you cannot observe ts=10 without first observing ts=8). Becomes reachable under (a) horizontal scaling with Redis pub/sub (no global ordering across publisher nodes) or (b) micro-disconnect where `state-sync` replay interleaves poorly with live broadcasts. **This is the architectural ceiling that blocks safe horizontal scaling of the collab backend.** Fix = adopt per-key frontiers (the original `CODEX_PLAN_v2.md` §2-3 design). Acceptable to defer under the honest-collaborators + single-node assumption. | OPEN | C3 | Gemini adversarial review R1/R2 2026-05-26 (BL-0879 descope) |
| BL-0886 | **CRDT `baseTimestamp` bound at transmit time, not mutation time (BL-0879 descope follow-up)** — `client/src/lib/collaboration-client.ts#sendStateUpdate` stamps each outgoing op's `baseTimestamp` from the CURRENT `maxSeenLamport` at send time. This is correct for the ONLY current callers (conflict resolution: accept-mine / merge), where re-stamping with the latest observed baseline is exactly what makes the re-applied edit win — and is harmless today because `send()` has no offline queue (disconnected sends are dropped) and no live-edit/batch layer calls `sendStateUpdate`. **Latent bug (Gemini adversarial review R1/R2, 2026-05-26):** the moment live-edit batching OR an offline send-queue is wired, an edit composed at baseline N but flushed after broadcasts advanced `maxSeenLamport` to M (>N) will be stamped M and silently overwrite concurrent edits it never observed (offline-overwrite / lost update). Fix when that work lands: capture `baseTimestamp` at mutation/intent time and pass it through as an explicit field (`sendStateUpdate` already preserves a pre-set `baseTimestamp`); add an offline-flush regression test. A loud `@danger` JSDoc warning is in place on `sendStateUpdate` as of 2026-05-26. | OPEN | C3 | Gemini adversarial review R1/R2 2026-05-26 (BL-0879 descope) |
| BL-0887 | **3D board viewer photoreal FR4 visual-fidelity upgrade (Tyler-requested 2026-05-26)** — `client/src/components/views/BoardViewer3DView.tsx` renders the PCB as a flat, lifeless shape (this is the root cause behind the E2E-356/358/364 "flat green parallelogram" findings, incl. BL-0740). **Diagnosis 2026-05-26:** the renderer uses `meshBasicMaterial` (an UNLIT material — ignores all lighting, hence the flatness) with only ~4 `<mesh>` + 3 `<sphereGeometry>` primitives, and some layers are CSS `translateZ` DOM overlay rather than real WebGL. The data model (`client/src/lib/board-viewer-3d.ts`: `Trace3D`, `Via3D`, `Component3D`, `boardThickness`, per-layer copper/silk/mask colors) is rich but is NOT rendered with fidelity. `three@0.184`, `@react-three/fiber@9`, `@react-three/drei@10` are ALL installed but barely used. **Target (Tyler chose 2026-05-26): PHOTOREAL FR4** — green solder-mask via `meshPhysicalMaterial` (clearcoat + subtle gloss), gold/copper pads & traces with metalness+roughness, white silkscreen layer, true extruded `boardThickness`, soft studio lighting + drei `<Environment>` reflections + `<ContactShadows>`, and real component meshes replacing the sphere markers. Looks like a fab-house photo. **Scope/process:** write a plan per the plan template (`docs/plans/2026-03-05-pcb-layout-engine.md`) FIRST — mandatory Context7 research on `@react-three/fiber` + `@react-three/drei` (Environment, ContactShadows, useGLTF, meshPhysicalMaterial) + WebSearch on PBR PCB rendering; phased TDD; preserve existing per-layer toggle/interaction behavior and the cross-view repair loop verified in Codex R64; keep performance budget for large boards (instancing for vias/pads). Brainstorm open questions before plan: component 3D model source (GLTF library vs procedural bodies), per-layer visibility UX, perf ceiling. **Implement in a fresh session** (this session's context was maxed when scoped). | OPEN | C4 | Tyler-requested 2026-05-26; supersedes the visual half of BL-0740 |
...
| BL-0879 | **CRDT conflict-detected message not emitted to LWW-losing client** — **DELIVERED 2026-05-26 via a DESCOPED design, ratified by Gemini adversarial review R1→R2 (`COLLAB_BL0879_DESCOPE_RESPONSE_R1/R2_GEMINI.md`, ROUND_STATUS: ratified, SIGNOFF: both).** Codex was rate-limited so Gemini substituted as the adversarial peer. **What shipped (≈130 LOC, not the 775-1305 spec):** client-supplied baseline via a single GLOBAL per-client `maxSeenLamport` stamped as each op's `baseTimestamp`; server `detectConflict(op, recent, baseTimestamp)` + symmetric drop check raise `lww-update` conflicts and emit `conflict-detected` to the losing client; fail-SAFE `baseTimestamp ?? 0` default (a fieldless op conflicts rather than silently winning — Gemini caught the original fail-deadly `MAX_SAFE_INTEGER`). Verified: 167 collab tests + `npm run check` green. **Tradeoffs accepted vs the converged per-key spec, now carved:** BL-0885 (cross-key false-negative / blocks horizontal scaling), BL-0886 (transmit-time `baseTimestamp` binding — latent until live-edit batching). Pre-existing carves still stand: BL-0882 (insert/delete window-bound), BL-0883 (state-sync snapshot/replay/ACK), BL-0884 (spoof-proof ACK ledger). The full per-key Option C-plus-ACK design below was NOT built — it is the upgrade path if BL-0885 reachability changes (horizontal scaling). **Original converged spec (reference / upgrade path)** consists of 3 documents: (1) `CODEX_PLAN_v2.md` — primary plan (sections 1-9: schema, server state, conflict detection, ACK semantics, state-sync gap, test plan, insert/delete carve, state-sync replay carve, LOC estimate); (2) `CLAUDE_RESPONSE_3.md` — adds BL-0883 cold-start carve regression test + `connectionId` server-only doc comment; (3) `CLAUDE_RESPONSE_5.md` — adds `markObserved` no-op-on-disconnect-race rule + `flushMicrotasks` primitive spec (`setImmediate`-backed) + same-frame duplicate test 5b. **Approach:** Option C-plus-ACK — per-connection ids on ClientEntry, server-owned per-key frontier `Map<projectId, Map<entityKey, FrontierEntry{ts, connectionId, userId, op}>>`, per-connection observedFrontier `Map<projectId, Map<connectionId, Map<entityKey, number>>>` advanced ONLY by client `state-update-ack` (reused for both normal remote updates AND conflict `theirOp` ACKs), conflict rule `frontier.ts > observedFrontier[C][K] && frontier.connectionId !== C`, theirOp stored in frontier for UI payload, server validates ACKs with `Math.max(current, ackedTs)` and `ackedTs ≤ frontier.ts` clamp. Carves: BL-0882 (insert/delete frontier extension), BL-0883 (state-sync document snapshot + replay + ACK), BL-0884 (per-connection ACK ledger for spoofing-proof CRDT integrity, future). LOC estimate **775-1305 changed lines** (380-620 production + 395-685 tests). **Pickup steps:** Fresh session reads the 3 spec docs in order, implements per CODEX_PLAN_v2.md §9 implementation order, runs 12 server integration tests + 5 client tests, then non-implementing agent (Codex if Claude implements; vice-versa) reviews the diff for final correctness. Bidirectional pattern continues into implementation phase. | DONE | C2 | Converged 2026-05-09 (8-round review); DELIVERED via descope 2026-05-26 (Gemini R1/R2 ratified) — carves BL-0885/0886 |
