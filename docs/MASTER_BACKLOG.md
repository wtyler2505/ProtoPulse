...
| BL-0126 | Shared unit/scale contract across sim + DRC engines | PARTIAL — Phase 1 shipped: `shared/units.ts` (657 lines) centralizes SI + SPICE parsers, branded length phantom types (`Length_mm`/`Length_mil`/`Length_meter`/`Length_inch` + `as*` constructors + `*To*` conversions), `parseSiValue`/`parseSpiceValue`/`formatSpiceValueFixed`/`formatSpiceValueCompact`/`formatSiValue`/`convertTo`/`spiceToSiNumber`/`siToSpiceString`. Tests: `shared/__tests__/units.test.ts` — 110 passing. Typecheck clean. **Phase 2 pending:** wire `Length_mil` brand through `shared/drc-engine.ts` DRC interfaces (replace `// mils` comments with typed fields on `DRCRule.params`/`DRCViolation`/trace/pad APIs); migrate `client/src/lib/board-stackup.ts` to branded types; migrate sim↔DRC boundary in `client/src/lib/simulation/pcb-geometry-bridge.ts` to `milToMm`/`mmToMeter` (remove any remaining inline `* 0.0254`); ensure `cross-engine-parity.test.ts` covers the branded boundary. | C4 | MF-078 |
