...
| BL-0879 | **CRDT conflict-detected message not emitted to LWW-losing client** — architectural fix specified via 7-round Claude↔Codex bidirectional adversarial review. **CONVERGING (Round 7 in flight):** Spec is `CODEX_PLAN.md` (v1, Codex Round 4) + `CLAUDE_RESPONSE_3.md` (Round 5 additions: BL-0883 carve regression test + connectionId server-only doc) + `CODEX_RESPONSE_FINAL.md` (Codex Round 6 — caught real send-vs-observe race on conflict-detected) + `CLAUDE_RESPONSE_4.md` (Round 7 acceptance) + `CODEX_PLAN_v2.md` (Round 7 in flight, dispatched 2026-05-09 via Codex `be073i3ox`). Round 6 hole: server advancing observedFrontiers on conflict-detected SEND (without client ACK) reintroduces the same send-vs-observe race the plan eliminated for state-update broadcasts — full-duplex WebSocket can deliver two stale same-key updates from connection C before the first conflict-detected reaches its browser, and the second update is then accepted as "intentional overwrite". Fix: client ACKs conflict `theirOp` via existing `state-update-ack` schema; server only advances observedFrontiers on receipt; same code path as state-update ACKs. Approach: per-connection ids on ClientEntry, server-owned per-key frontier `Map<projectId, Map<entityKey, FrontierEntry{ts, connectionId, userId, op}>>`, per-connection observedFrontier advanced ONLY by client ACK, conflict rule `frontier.ts > observedFrontier[C][K] && frontier.connectionId !== C`, theirOp stored in frontier for UI payload. Carves: BL-0882 (insert/delete frontier extension), BL-0883 (state-sync document snapshot + replay + ACK). LOC estimate ~605-1015 changed lines. **Pickup steps when Codex Round 7 completes:** read `CODEX_PLAN_v2.md`, run Round 8 adversarial review; if converged, fresh-session implementation against the v2 spec. | OPEN | C2 | Bidirectional review 2026-05-09 |
