...
| Hardware bridge / probe (v0.5 / v0.7) | Partial | Emulator side of v0.5 shipped: ATmega328P (timers, SPI, TWI, EEPROM, watchdog) + RP2040 + a from-scratch ESP32-S3 under one McuCore contract. The S3 now includes Xtensa LX7 code-density/windowed-ABI execution, app-image/XIP loading, level-1 interrupts + interrupt matrix, UART/TIMG/GDMA/LEDC/RMT/RTC/touch/ADC/PCNT surfaces, RTC sleep/wake and reset/fault slices through ESP32-S3 slice 97, including the RTC slow-clock mux/divider, elapsed-time rebase for RWDT and the RTC main timer, host-injected WiFi/BT light-sleep wake source bits, and a first PCNT pulse-counter register/input-matrix path. Firmware panel core picker and co-sim bench with per-core ADC channels are live. Still Partial because unmodified IDF/FreeRTOS remains an open long tail, WebSerial/WebUSB flashing needs real hardware, and the v0.7 Probe has not started. |
