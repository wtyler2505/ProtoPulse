...
**Date:** 2026-03-27 (Tables 1–3) · **Updated:** 2026-06-16 (Table 4 — engine redesign addendum, refreshed through the v0.1–v0.6 landings)
...
| Hardware bridge / probe (v0.5 / v0.7) | Partial | Emulator side of v0.5 shipped: ATmega328P (timers, SPI, TWI, EEPROM, watchdog) + RP2040 + a from-scratch ESP32-S3 under one McuCore contract. The S3 now includes Xtensa LX7 code-density/windowed-ABI execution, app-image/XIP loading, level-1 interrupts + interrupt matrix, UART/TIMG/GDMA/LEDC/RMT/RTC/touch/ADC/PCNT/I2C/SPI/MCPWM/TWAI/SHA/AES surfaces, RTC sleep/wake and reset/fault slices through ESP32-S3 slice 163 (including deep-sleep timer wake that resets the core with DEEPSLEEP_RESET, and GDMA IN_CHECK_OWNER / OUT_AUTO_WRBACK / OUT_CHECK_OWNER descriptor-owner/writeback gating), including the RTC slow-clock mux/divider, elapsed-time rebase for RWDT and the RTC main timer, host-injected WiFi/BT light-sleep wake source bits, a first PCNT pulse-counter register/input-matrix path with an APB-cycle input glitch filter (FILTER_EN/FILTER_THRES dropping sub-threshold pulses), a first I2C master command/FIFO path with I2C_EXT0/1 interrupts, a first GPSPI2/GPSPI3 CPU-FIFO master path with SPI2_DMA/SPI3_DMA interrupts, a first MCPWM timer/operator/generator GPIO-output path with PWM0/PWM1 interrupts, a SYSTIMER with both 52-bit/16-MHz counters (UNIT0/UNIT1, UPDATE/VALUE_VALID latch read — what esp_timer polls) and all three comparators (TARGET0/1/2) with one-shot and periodic alarms, per-comparator unit selection (TIMER_UNIT_SEL), and INT_RAW/ST/ENA/CLR latches routed through the interrupt matrix (sources 57/58/59) to CPU interrupts, and TWAI/CAN register/self-test, host injection, host TX drain, acceptance filtering (single and dual filter modes, standard and extended frames), virtual peer-bus delivery, ACK/no-ACK TEC movement with ACK bus-error events, listen-only TX suppression, RX FIFO overrun error events, bus-off host events, bus-off recovery state-change events, bitwise arbitration with arbitration-lost capture (ALI/ALC, first-loss latch with read re-arm), non-destructive retransmit, single-shot (TR|AT) drop-on-loss with a transmit-complete (TCS) status bit, host-drained TX/RX/error/state-change events, and CAN_INT interrupts, plus eFuse read/program-done interrupt routing through the interrupt matrix (source 36) and SHA/AES/RSA completion-interrupt routing (sources 77/76/75, matrix maps at the explicit silicon offsets 0x138/0x134/0x130). The SHA accelerator covers SHA-1/224/256/384/512 with multi-block, a done interrupt, and a GDMA-fed DMA hashing path for the full SHA family (incl. SHA-512); the AES accelerator covers AES-128/192/256 ECB encrypt and decrypt with a done interrupt, plus AES-CBC (encrypt/decrypt), AES-CTR, AES-OFB, AES-CFB8 (encrypt/decrypt), AES-CFB128 (encrypt/decrypt), and AES-GCM (authenticated encryption with AAD, via a from-scratch GHASH, verified for AES-128, AES-192, and AES-256 keys) through the GDMA-fed AES-DMA path with a completion interrupt; the RSA/MPI accelerator covers modular exponentiation (Z = X^Y mod M), modular multiply (Z = X·Y mod M), and full-width multiply (Z = X·Y) with the done-status handshake and a matrix-routed done interrupt; the HMAC accelerator computes HMAC-SHA256 with an eFuse-sourced key (single-block message path); the Digital Signature (DS) accelerator computes an RSA signature Z = X^Y mod M by AES-256-CBC-decrypting the encrypted private-key parameter block, reading Y and M from their little-endian regions, and running the BigInt modexp (the MD integrity check and the HMAC-downstream key derivation are follow-ons); a hardware RNG (WDEV_RND_REG) backs esp_random() with a deterministic-from-reset xorshift32 stream; and the USB-Serial-JTAG controller provides a bidirectional console (EP1 FIFO + WR_DONE flush for TX, host-injected RX with OUT_EP_DATA_AVAIL, and matrix-routed RX (SERIAL_OUT_RECV_PKT) and TX-empty (SERIAL_IN_EMPTY) interrupts) for the default S3 console path. Firmware panel core picker and co-sim bench with per-core ADC channels are live. Still Partial because unmodified IDF/FreeRTOS remains an open long tail, WebSerial/WebUSB flashing needs real hardware, and the v0.7 Probe has not started. |
