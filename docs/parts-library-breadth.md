...
- **PARKED (need Tyler's physical board / a clean diagram before modeling):**
  - [x] `core:osepp-bth-01` — **OSEPP BTH-01 — UNPARKED + done, photo-verified 2026-06-22.** Tyler supplied board photos (`images/OSEPP-BTH-01*.jpeg`): it's the **Arduino MAIN board** (ATmega328P @16MHz + Bluegiga WT11 Bluetooth + PL2303), full Uno form factor. Modeled with the standard Uno R3 header pinout (32 pins, identical nets to `core:arduino-uno-r3`); D0/D1 shared with the onboard WT11. The catalog had conflated it with OSEPP's separate **BTM-01** (the BC417/HC-06 serial module) — which Tyler may or may not also own; if a small 4/6-pin BC417 module turns up, model THAT separately as BTM-01.
