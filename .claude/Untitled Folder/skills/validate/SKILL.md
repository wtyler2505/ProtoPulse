...
- If target is "all" or "notes": validate all notes in knowledge/ directory
...
1. Check the note's location — notes in knowledge/ use the standard note template
...
| Topics | Must link to at least one topic map | Check for `topics:` in YAML or `Topics:` section in footer. Must contain at least one entry. BOTH formats are valid: bare slug (`- power-systems`, preferred) and wiki link (`- "[[power-systems]]"`, accepted-legacy → WARN, never FAIL) — see `ops/config.yaml` `_schema` |
...
1. Discover all .md files in knowledge/ directory
