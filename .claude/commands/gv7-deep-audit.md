...
4. Document result (Works / Broken / Issue)
...
# OR for Python: safety check --json > ${AUDIT_DIR}/reports/safety.json 2>&1 || true
```
...
### 3.4 Uncommitted Secrets Check (Desktop Commander)
...
**Hard cap: max 6 concurrent agents; spawn them with `run_in_background: true` and consume results as they complete.** Then consolidate findings before UI audit (needs sequential interaction).
