...
```
metrics.snapshot = {
  timestamp, loc, file_count, languages,
  complexity_hotspots[], test_file_count,
  recent_commit_count, doc_files[]
}
```
...
**Diff metrics:**
```
Δ LOC: +1,200 (was 8,500 → now 9,700)
Δ Complexity: 2 new hotspots in src/services/
Δ Test files: -1 (regression?)
Δ TODO count: +5 (debt accumulating)
```
...
Reflection saved. Next run will diff against this snapshot.
...
mcp__memory__create_entities --entities '[{
  "name": "project_reflection_[project]_[date]",
  "entityType": "reflection",
  "observations": ["metrics_json", "findings", "recommendations"]
}]'
```
