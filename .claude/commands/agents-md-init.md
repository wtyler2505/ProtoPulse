...
## Current Status
!`test -f AGENTS.md && echo "   AGENTS.md already exists" || echo "  Ready to create AGENTS.md"`
...
**CRITICAL**: Document the EXACT script names from package.json, not generic placeholders. For example:
...
### Testing Philosophy
**When tests fail, fix the code, not the test.** Key principles:
...
### MANDATORY: Always Delegate to Specialists & Execute in Parallel
...
**Performance Impact:**
Parallel tool execution is 3-5x faster than sequential calls, significantly improving user experience.
