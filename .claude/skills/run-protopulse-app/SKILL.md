...
- **`earlyoom` kills heavy processes under memory pressure.** This was diagnosed on a ~7 GB box already running a full desktop + Chrome + MCP servers (`free -h`: ~1-2 GB available, swap near-full; `systemctl is-active earlyoom` → `active`). earlyoom SIGTERMs the largest-RSS process, so a memory-hungry Vite dev/build is the first to die (graceful `code 143`). A backgrounded dev server is also reaped across tool calls. That's why Run launches + drives + tears down in a single shell, and why the driver's `nav` retries: it catches the live window. If a capture comes back blank/refused, re-run — a fast boot (server serving in ~2s) lets the lighter editor render before any kill. On a host with adequate free RAM none of this applies; the server stays up indefinitely. **Do not raise `--max-old-space-size`** to fight it — a bigger heap ceiling lets RSS grow further before GC and trips earlyoom *sooner*.
...
- **Production build is memory-heavy.** `vite build` transforms a large module graph (multi-GB peak); on the memory-pinned box above, earlyoom SIGTERM'd it mid-`transform` (raising heap to 8 GB made it *worse*, per the rule above). Don't build to run it — use the dev server, or build on a host with several GB free.
