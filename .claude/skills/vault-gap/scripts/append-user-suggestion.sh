...
# different priority class per the vault-gap `--source user` spec (formerly vault-inbox)).
#
# Usage:
...
Ordered append-only log of user-submitted vault suggestions captured by `.claude/skills/vault-gap/` (`--source user` mode, formerly vault-inbox).
