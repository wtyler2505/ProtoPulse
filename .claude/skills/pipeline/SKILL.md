---
name: pipeline
description: End-to-end source processing -- seed, reduce, process all claims through reflect/revisit/verify, archive. The full pipeline in one command. Triggers on "/pipeline", "/pipeline [file]", "process this end to end", "full pipeline".
version: "1.0"
...
Report:
```
$ Extracted: {N} insights, {M} enrichments
...
$ Processing insight 1/{total}: {title}
  ...
  $ reweave... done (2 insights updated)
  $ verify... done (PASS)
```
...
- List of created insights with titles
...
Processing:
  insights created: {N}
  Existing insights enriched: {M}
  Connections added: {C}
  topic maps updated: {T}
  Older insights updated via reweave: {R}
...
- Extracted {N} insights and {M} enrichments
...
- Show the user what was created (list of insights)
