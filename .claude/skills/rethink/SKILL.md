...
|-------------|---------|---------------|--------|
| PROMOTE | Reusable insight worth keeping as a permanent insight | General principle across sessions. Would work as a claim note. Crystallized insight, not operational guidance. | Create insight in {vocabulary.notes}/, set observation `status: promoted`, add `promoted_to: [[title]]` |
...
**For PROMOTE items:**
1. Create insight with prose-as-title in {vocabulary.notes}/
...
- Add each promoted insight to the queue with `current_phase: "reflect"` (the note already exists, so skip create)
