...
Read `${CLAUDE_SKILL_DIR}/reference/interaction-constraints.md` (vendored from the arscontexta plugin — this skill is not plugin-hosted, so `${CLAUDE_PLUGIN_ROOT}` is not set here) and check:
