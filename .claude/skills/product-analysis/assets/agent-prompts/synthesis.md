...
- Search "{{DOMAIN}} industry benchmarks {year}" — contextualize the project's metrics (is 50 test files good or bad for a project this size?)
- Search "{{DOMAIN}} product maturity model" — frame the project's current stage
- Search "technical debt benchmarks {language}" — put the `lizard`/`scc` numbers in industry perspective
- Search "{{DOMAIN}} startup success factors" — help prioritize the checklist based on what matters most for market success
