...
fd -g '*Page*' -g '*View*' -g '*Screen*' -g '*Layout*' {{PROJECT_ROOT}} -g '!node_modules' -g '!dist' 2>/dev/null
...
rg 'aria-' {{PROJECT_ROOT}} --stats -g '*.tsx' -g '*.jsx' -g '!node_modules'
rg 'role=' {{PROJECT_ROOT}} --stats -g '*.tsx' -g '*.jsx' -g '!node_modules'
rg 'tabIndex' {{PROJECT_ROOT}} --stats -g '*.tsx' -g '*.jsx' -g '!node_modules'
rg 'alt=' {{PROJECT_ROOT}} --stats -g '*.tsx' -g '*.jsx' -g '!node_modules'
...
rg 'toast\.|notification\.|alert\(' {{PROJECT_ROOT}} --stats -g '!node_modules' -g '!dist'
rg 'error.*message|Error.*message' {{PROJECT_ROOT}} -i --stats -g '!node_modules' -g '!dist'

# 5. Loading/skeleton states
rg 'loading|isLoading|isPending|Skeleton|Spinner' {{PROJECT_ROOT}} --stats -g '*.tsx' -g '!node_modules'

# 6. Keyboard shortcuts
rg 'useHotkeys|onKeyDown|onKeyPress|keyboard.*shortcut|Keyboard' {{PROJECT_ROOT}} -i --stats -g '!node_modules'
```
...
1. `resolve-library-id` for the project's UI libraries (e.g., the component library, styling framework, routing library from {{TECH_STACK}})
...
- Search "{{DOMAIN}} UX best practices" — what do expert users expect from tools in this domain?
- Search "{{DOMAIN}} workflow design patterns" — how do the best tools handle common workflows?
- Search "WCAG 2.1 checklist web application" — reference accessibility standards
- Search "{persona type} software expectations" for each persona in {{PERSONAS}} — what do these users value?
- Search "{{TECH_STACK}} accessibility patterns" — framework-specific a11y guidance
...
For each persona in {{PERSONAS}}, trace these workflows:
...
```markdown
# Phase 3: UX & Workflow Evaluation -- {{PROJECT_NAME}}

> Generated: {{DATE}}
> Personas evaluated: {{PERSONAS}}
