...
> Shared by `/connect` (reflect) and `/revisit` (reweave). Extracted 2026-06-11 from the connect skill during the progressive-disclosure restructure (docs/audits/2026-06-11-skill-audit.md Top-10 item 10). Vocabulary placeholders (`{vocabulary.*}`) resolve per the invoking skill's runtime configuration. The qmd three-tier fallback and lock-dir serialization here are the canonical copies — do not fork them back into individual skills.
