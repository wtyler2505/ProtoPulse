...
[![Engine Tests](https://img.shields.io/badge/Engine_Tests-all_green-6da13f?style=flat-square&logo=vitest&logoColor=white)](packages/README.md)
...
npm run test:packages   # Engine suite, all green (counts: .ref/project-dna.md)
...
**A fully green test suite** (counts live in [`.ref/project-dna.md`](.ref/project-dna.md) — regenerated by live scan), own CI (`.github/workflows/packages-ci.yml`): typecheck, lint, tests, golden eval, golden smoke, builds.

```bash
npm run check:packages           # typecheck every package
npm run test:packages            # all engine tests
...
<td align="center"><strong>16</strong><br><sub>engine packages, tests all green</sub></td>
...
- The engine adds its own **full test suite** (`npm run test:packages`; counts in [`.ref/project-dna.md`](.ref/project-dna.md)), a **golden-regression eval for the AI crew** (`npm run eval:ai`), byte-exact golden-file export tests in `tools/golden/`, and a **100% branch coverage gate** on the graph core — all enforced in a separate, fully green CI workflow
