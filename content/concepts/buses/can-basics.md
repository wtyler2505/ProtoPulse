...
On the virtual CAN bus, run a single node and watch the error counters climb to bus-off — then add a second node and watch the same firmware succeed. Then delete one terminator and re-run over a modeled long harness: clean frames at the near node, corrupted at the far one.
