...
async function gatherProjectSnapshot(projectId: number) {
  ...
  return {
    ...
    nodes: nodes.map(n => ({ type: n.type, label: n.label, data: n.data })),
...
fs.writeFileSync('server/batch-analysis.ts', replacement, 'utf8');
console.log("Patched server/batch-analysis.ts");
