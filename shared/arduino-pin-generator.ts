...
/**
 * Format pin constants as C++ constexpr expressions.
 */
export function formatAsConstExpressions(
  constants: readonly PinConstant[],
  options: PinGeneratorOptions,
): string {
  if (constants.length === 0) {
  ...
  if (options.groupByCategory) {
    ...
    for (let gi = 0; gi < groupEntries.length; gi++) {
      ...
      for (let ci = 0; ci < group.length; ci++) {
        lines.push(formatConstExpr(group[ci], maxNameLen, options.includeComments));
      }
    }
  } else {
    for (const c of constants) {
      lines.push(formatConstExpr(c, maxNameLen, options.includeComments));
    }
  }
...
function formatConstExpr(
  ...
  if (constant.category === 'unmapped') {
    const base = `// constexpr uint8_t ${constant.name}${padding}= ${pinStr};`;
